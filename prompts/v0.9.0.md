...
Use `pkg/` prefix for external packages (from `ailang-packages` or other package repos):
...
**Setup:** `ailang init package --name myorg/myapp` → `ailang add --path ../ailang-packages/packages/gcp-auth` → `ailang lock`

**Available packages** at `github.com/sunholo-data/ailang-packages`: `sunholo/gcp-auth` (OAuth2), `sunholo/auth` (API keys), `sunholo/http-helpers` (request builders), `sunholo/logging` (JSON logs), `sunholo/config` (env loading), `sunholo/testing-utils` (assertions).
