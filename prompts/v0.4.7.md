# AILANG v0.4.7 - AI Teaching Prompt (Inline Tests + Cross-Function Dependencies)
...
**NEW in v0.4.7:** Inline tests with cross-function dependency support (M-TESTING-DEPS):
...
### Inline Tests (v0.4.7+ - Executable Documentation)

**NEW:** Inline tests provide executable documentation directly in function definitions. Tests show expected input/output pairs that are automatically verified.

**Syntax:**
```ailang
pure func name(params) -> ReturnType
  tests [
    (input1, expected1),
    (input2, expected2),
    ...
...
**Multi-argument function (use tuple for inputs):**
```ailang
...
**Cross-function dependencies (v0.4.7 M-TESTING-DEPS):**
Functions that call other user-defined functions are automatically supported. The test harness includes all dependencies.

```ailang
-- gcd is defined above
...
{
  (a * b) / gcd(a, b)  -- lcm depends on gcd
}
```
...
- **Self-documenting:** Expected behavior is visible in function definition
- **Executable specification:** Tests verify the documentation is accurate
- **Regression protection:** Catch breaking changes immediately
- **AI training:** Clear examples of correct patterns

**Running inline tests:**
```bash
ailang test file.ail        # Runs all inline tests
```

**Requirements:**
- Function must be `pure func` (no effects)
- Tests are tuples of `(input, expected_output)`
- Multi-arg functions use nested tuples: `((arg1, arg2), expected)`
