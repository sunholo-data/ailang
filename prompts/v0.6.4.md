# AILANG v0.6.4 - AI Teaching Prompt
...
**⚠️ IMPORT REMINDER:** When using stdlib functions, you MUST add the import statement!
- Using `floatToInt`? Add `import std/math (floatToInt)`
- Using `getString` for JSON? Add `import std/json (decode, getString, getInt, ...)`
- Missing imports cause "undefined variable" errors
...
import std/json (encode, decode, getString, getNumber, getInt, getBool, getArray, getObject)  -- v0.6.4 helpers!
import std/list (map, filter, foldl, length, concat, sortBy, take, drop)  -- sortBy(cmp, xs), NO flatten!
import std/string (split, trim, stringToInt, stringToFloat, contains, find, substring, intToStr, floatToStr)
