...
-- Use map instead of writing recursive transform functions
let doubled = map(\x. x * 2, [1, 2, 3])  -- [2, 4, 6]

-- Use filter instead of writing recursive select functions
let evens = filter(\x. x % 2 == 0, [1, 2, 3, 4])  -- [2, 4]

-- Use foldl instead of writing recursive accumulators
let sum = foldl(func(acc: int, x: int) -> int { acc + x }, 0, [1, 2, 3])  -- 6
```

**Note:** These are pure functions. For effectful operations (e.g., calling AI per element), use recursion for now.
...
Add preconditions and postconditions to functions for runtime verification:
...
```bash
ailang run --verify-contracts --experimental-binop-shim --caps IO --entry main file.ail
```

**Contract modes:**
- Default (no flag): contracts are parsed but not checked at runtime
- `--verify-contracts`: panics on first violation with clear error message

**Rules:**
- `requires { expr }` — checked before function body executes
- `ensures { expr }` — checked after, `result` refers to return value
- Contract expressions must evaluate to `bool`
- Contracts go BETWEEN the return type and the function body
