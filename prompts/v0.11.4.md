...
let m0 = M.empty()
let m1 = M.insert("alice", 30, m0)
let m2 = M.insert("bob", 25, m1)

match M.lookup("alice", m2) {
  Some(age) => println(show(age)),  -- 30
  None      => println("missing")
}

println(show(M.size(m2)))          -- 2
println(show(M.member("bob", m2)))  -- true
...
match verifyRS256(token, pemPublicKey) {
  Ok(claims) => if isExpired(claims) then "expired" else "valid",
  Err(msg)   => "invalid: " ++ msg
}
...
export func processBatch(items: [Item]) -> int ! {Trace, IO} {
  let span = spanStart("batch.process");
