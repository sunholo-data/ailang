...
| `concat(a, b)` for strings | `"${a}${b}"` interpolation (preferred) or `a ++ b` |
...
**For strings, prefer interpolation `"${...}"` over `++`. `++` still works for strings today,
but Phase 2 of M-CONCAT-DISAMBIG (v0.13.0) will restrict `++` to lists only.**

```ailang
-- PREFERRED: string interpolation
let msg = "Count: ${length(items)}"

-- Works today, but discouraged (will be list-only in v0.13.0):
let greeting = "Hello" ++ " " ++ "World"

-- List concatenation (unchanged, permanent)
