...
- **`PROVIDER=Codex`** (added 2026-07-16, Mark — the true-Fable lane): the `Codex` CLI takes FULL
...
- No design doc yet → **design-doc-creator** on the ROTATION designer (see the roles table: next
  entry after `~/.ailang/state/mission-designer-rotation`; Codex via `Codex-sub`, codex via the
...
6. **Every wait is bounded** (added 2026-07-12 after iteration 13 hung 4h in an unbounded
   `until COND; do sleep 30; done` — no worktree, no commit, Codex idle at 0% CPU with a live
