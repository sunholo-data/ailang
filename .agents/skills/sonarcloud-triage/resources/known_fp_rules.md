...
| `gosecurity:S6096` | `internal/pkg/tarball.go` (~L152-177, sink ~L167) | **Fixed + False Positive** | `ExtractTarball` (the `ailang install` extract path) was hardened 2026-07-14 to match `builtins/tar.go`: `isTarPathTraversal` + `filepath.Rel` containment (L152-163) + explicit symlink/hardlink rejection (L174-177). The fix did **not** auto-resolve the finding — Sonar's taint engine doesn't model `filepath.Rel` as a sanitizer, so it re-raised on the FS sink (same as the sibling row above). Correct disposition is **mark FP**. |
...
| `go:S4036` | `internal/executor/motoko/motoko.go:{655,665}` vuln-typed pair (keys AZ8T7J0blyXstVJKHcIT, AZ8T7J0blyXstVJKHcIU) — scope EXTENDED 2026-07-12 to internal/coordinator/, cmd/registry-validator/, internal/auth/gcp/, internal/executor/; EXTENDED 2026-07-21 to internal/eval_harness/ — pending keys (no SONAR_PAT in agent session): AZ9_V1aF2v84aGN84oqR/S/T (motoko/healthcheck.go:99,140,150), AZ9tWkujLG18L6h5L1Ry/z (gemini_evaluator_bridge.go:378,747). process_unix.go:35 was instead FIXED in code (fixed /bin/ps path, no PATH lookup) | **FP / Safe** | Running known system/dev tools (lsof, ps, git, gcloud, ailang) from PATH is standard for the single-tenant developer CLI, daemon, and rig executors. |
