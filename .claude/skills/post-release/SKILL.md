...
- **Step 2**: Agent eval — flagship + modern OS × AILANG+Python
  ...
  - **Tier system** (v0.14.0+): `smoke` (23), `core` (27), `stretch` (11), `vision` (9) — counts as of 2026-06
...
- **Step 3** (--lang-harness): Language × Harness sweep — cheapest models × 4 languages
  ...
  - **Tier: `core` only** (27 benchmarks) — stretch is skipped here even if `--tier core,stretch` was set globally
...
- Default (release): `--tier core,stretch` — 38 benchmarks (27 core + 11 stretch)
- Dev/fast mode: `--tier core` — 27 benchmarks (Core is the headline metric)
- Full audit: `--tier smoke,core,stretch` — 61 benchmarks (include smoke for sanity)
...
**Cost & time** (default tier `core,stretch` = 38 benchmarks):
