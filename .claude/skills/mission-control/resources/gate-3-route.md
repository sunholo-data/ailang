...
- **`PROVIDER=codex`** (executor role — the landed M1b lane; codex CLI at `/opt/homebrew/bin/codex`,
  ...
  4. **Fallback (never wedge the loop) — follow the RATIFIED CHAIN, never a straight drop to
     ...
     **COMMA-SEPARATED CHAIN walked left to right**, with opus as the implicit tail — so the
     ...
     executor `pi:ollama/deepseek-v4.1-flash:cloud,pi:openrouter/deepseek/deepseek-v4.1-flash`;
...
`$MISSION_PLANNER_ANTHROPIC_FALLBACK` (default `codex:gpt-6-sol`), not the already-failed
...
**⚠ WHEN A MILESTONE SHIPS A TOOL THAT PARSES ANOTHER TOOL'S OUTPUT, SAY IN THE DIRECTIVE THAT THE
FIXTURE MUST BE *GENERATED BY THAT TOOL* AND *PINNED AGAINST EVERY TRANSFORMATION BETWEEN THEM* —
A HAND-TYPED FIXTURE MAKES THE WHOLE SUITE GREEN AGAINST INPUT THE TOOL WILL NEVER SEE** (added
2026-09-07 V1 iteration 348; three first-party instances in ONE sprint, each caught only by a judge
or by real CI, each with the full unit suite green). The designer rule above demands a Verification
Log row per codebase claim, and the executor recipe demands mutation proof per test row. Both are
about the code. Neither asks where the test's *input* came from — and for a parser, the input IS the
contract. The failure mode is uniquely quiet: hand-typed fixtures are self-consistent, so every gate
in this protocol passes, and the defect surfaces only where the real bytes are.
Measured on `tools/ci/headroom`, which parses `go test` output. **(1)** The parser anchored on
`^ok\t`; Go pads the status column, so a real line is `ok` + TWO SPACES + TAB. Six hand-typed
fixtures — copied from the design doc's own illustrative example — passed, while the tool matched
**zero** records against real output and reddened a passing build on both CI legs. **(2)** Fixed by
checking in genuinely captured output; a real `(cached)` line then turned out to carry **no duration
at all**, and the repo's `.gitattributes` (`* text=auto`, `eol=lf` pinned for `*.golden`/`*.sh` but
not for the new fixture) rewrote it on Windows checkout — LF **128 records / 105 cached** vs CRLF
**23 / 0**, one variable changed. **(3)** Closed only by fixing BOTH sides, which are not
alternatives: the parser tolerates the transformation at runtime (a log the runner writes has no
`.gitattributes`), and the fixture is pinned so it cannot change shape in transit.
**Rules for the directive. (a)** Name the command that GENERATES the fixture, and require it to be
checked in as produced — never transcribed, never adapted from a doc's illustration. **(b)** Require
a SECOND fixture from a different real invocation (cache-warm vs cold, failing vs passing) — the
shapes that differ are exactly the ones nobody imagines. **(c)** Enumerate the transformations
between generator and parser (VCS checkout attributes, shell redirection, log collectors, encoding)
and require the parser to tolerate them AND the fixture to be pinned against them. **(d)** Require
the anti-vacuity guard — a parser that extracts zero records from non-empty input must fail LOUDLY,
never return an empty report and exit 0. That guard is what made all three of these cheap; without
it they ship as a green job reporting nothing. Mission-independent: every mission on this rig ships
tools that read CI logs, `git` porcelain, `go test` output or provider JSON. The tell: a milestone's
deliverable parses something, and every fixture in its test file was typed by whoever wrote the test.
