...
:::tip IMPLEMENTED (v0.5.11)

Shared semantic state is now available in AILANG:

**SharedMem Effect** - Key-value cache with atomic CAS (compare-and-swap):
- `_sharedmem_get`, `_sharedmem_put`, `_sharedmem_delete`, `_sharedmem_keys`
- `_sharedmem_cas` - Atomic compare-and-swap for conflict-free multi-agent updates

**SharedIndex Effect** - Similarity-based retrieval:
- SimHash (Tier 1) - Fast, deterministic text similarity
- Neural Embeddings (Tier 2) - True semantic similarity via Ollama

**sem_frame Schema** - Standard semantic frame type in `std/sem`:
- `make_frame_at`, `store_frame`, `load_frame`, `update_frame`
- `is_similar` for near-duplicate detection

See the **[Semantic Search Guide](/docs/guides/semantic-search)** for setup instructions.

**Try it:** `ailang run --caps IO,SharedMem --entry main examples/sharedmem_cache.ail`
...
## Implementation: DX-15/16/17 (Complete)

The shared semantic state vision has been implemented across three design milestones:

### Core Components (All Implemented ✅)

| Component | Status | Description |
|-----------|--------|-------------|
| `SharedMem` effect | ✅ | Key-value cache with get/put/delete/cas operations |
| `SharedIndex` effect | ✅ | Similarity index with SimHash and embedding search |
| `sem_frame` type | ✅ | Canonical payload in `std/sem` module |
| `_ollama_embed` | ✅ | Generate 768-dim embeddings via Ollama |
| `_simhash` | ✅ | Fast 64-bit locality-sensitive hashing |
...
- **[Complete Implementation Status](https://github.com/sunholo-data/ailang/blob/main/design_docs/implemented/v0_5_11/semantic-caching-complete.md)** - Definitive status document
- [DX-15: Semantic Caching MVP](https://github.com/sunholo-data/ailang/blob/main/design_docs/implemented/v0_5_11/DX-15-semantic-caching-MVP.md)
- [DX-16: SharedIndex + DX-17 Report](https://github.com/sunholo-data/ailang/blob/main/design_docs/implemented/v0_5_11/dx-16-sprint-completion-report.md)
- [Future Work (v0.6.0+)](https://github.com/sunholo-data/ailang/blob/main/design_docs/planned/v0_6_0/semantic-caching-future.md)
