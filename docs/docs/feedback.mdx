...
Categories: `bug`, `enhancement`, `documentation`, `design-discussion`, `dx`, `feedback`, `limitation`.

Submissions land in the `public-feedback` inbox queue and are triaged against existing design docs by the AILANG core team. Anonymous is fine — the value is the signal, not the identity.
