...
**Run it:**
```bash
ailang run --caps IO,FS --args-json '{"path":"README.md"}' --entry main examples/docs/read_file.ail
```
...
ailang run --caps IO,FS,AI --ai claude-haiku-4-5 \
  --args-json '{"path":"README.md"}' --entry main examples/docs/summarize_file.ail
```
...
**Run it:**
```bash
ailang run --entry main examples/runnable/conway_grid.ail
```
