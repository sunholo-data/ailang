...
**Already working in the AILANG repository?** The hooks are configured and ready to go! Just:

```bash
# Test it right now:
ailang agent send --to-user '{"message": "Hooks work", "status": "ready"}'
ailang agent inbox user

# Or trigger the SessionStart hook:
bash scripts/hooks/session_start.sh
```

See [HOOKS_SETUP.md](../../../HOOKS_SETUP.md) for more details.
:::
