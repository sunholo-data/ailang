...
- **[Z3 Verify Demo](https://www.sunholo.com/ailang-demos/verify.html)** — Watch Z3 mathematically prove correctness or find counterexamples across 4 real-world scenarios
- **[AI + Contracts Demo](https://www.sunholo.com/ailang-demos/contracts-ai.html)** — AI extracts data from documents, contracts validate every field
- **[Safe Agent Demo](https://www.sunholo.com/ailang-demos/streaming/safe_agent/)** — Contract-verified AI tool calling with path traversal and SQL injection prevention
:::
...
:::tip See also: IFC Labels
For prompt-injection prevention and information-flow control, see the
[**Information-Flow Control (IFC) Labels** guide](ifc-labels.mdx).
Labels (`T<label>` and `T{not LABEL}`) are a sibling feature to
contracts — they work at the type-system layer rather than the SMT
layer, and the two are designed to compose.
:::
...
- Numeric conversions: `intToFloat` → `to_real`, `floatToInt` → `to_int`
...
|---------|--------|-------------|
| Higher-order functions (unknown lambda) | `map(f, xs)` where `f` is a parameter | Requires whole-program analysis |
| Effectful functions | Nondeterminism breaks proofs | Fundamental — effects are inherently nondeterministic |
| Nested quantifiers | `forall i: ... => forall j: ...` | Future — single-level only for now |
