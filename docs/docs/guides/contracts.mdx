...
$ ailang verify examples/runnable/contracts/record_verify.ail
  ...
  ✓ VERIFIED getX            4ms
  ✓ VERIFIED manhattan       6ms

  2 functions: 2 verified
```
...
- Record return types are not yet supported in function signatures (parser limitation)
...
**Not yet encodable:** `_str_trim`, `_str_upper`, `_str_lower` (no Z3 equivalent), `_str_split`, `_str_chars` (return lists — requires Phase D).
...
**Currently supported:**
- `int`, `bool`, `string`, and enum/ADT parameters and return types
- Arithmetic: `+`, `-`, `*`, `/`
- Comparison: `>=`, `<=`, `>`, `<`, `==`, `!=`
- Logical: `&&`, `||`, `!`
...
AILANG has no `for` or `while` loops. The verifiable fragment simply rejects recursive functions, and the remaining non-recursive functions are exactly the decidable fragment that SMT solvers handle well.
...
The "verifiable fragment" (pure, non-recursive, no higher-order functions, quantifier-free logic) describes a **large and natural subset** of AILANG programs — business rules, validation logic, pricing functions, game mechanics. In Go or Python, the equivalent restriction would eliminate virtually all idiomatic code because most code depends on loops, mutation, and side effects.
