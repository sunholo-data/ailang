...
| `eq_String(a, b)` | `=` | Equality |
| `ne_String(a, b)` | `distinct` | Inequality |
| `lt_String(a, b)` | `str.<` | Lexicographic less-than |
| `le_String(a, b)` | `str.<=` | Less-than-or-equal |
| `_str_len(s)` | `str.len` | String length |
| `_str_find(s, t)` | `str.indexof` | Find substring |
| `_str_slice(s, i, j)` | `str.substr` | Substring extraction |
| `_str_startsWith(s, p)` | `str.prefixof` | Prefix check |
| `_str_endsWith(s, x)` | `str.suffixof` | Suffix check |

**Not yet encodable:** `_str_trim`, `_str_upper`, `_str_lower` (no Z3 equivalent), `_str_split`, `_str_chars` (return lists).
...
Z3 proved that `listLength` always returns a non-negative result, that `prependLength` increases length by 1, that head equals nth at index 0, and that a singleton list has length 1. For `brokenAllPositive`, Z3 found a list whose head is non-positive.

**Supported list operations:**

| AILANG | Z3 SMT-LIB | Purpose |
|--------|-----------|---------|
| `_list_length(xs)` | `seq.len` | List length |
| `_list_head(xs)` | `seq.nth xs 0` | First element |
| `_list_nth(xs, i)` | `seq.nth` | Element at index |
...
- Comparison: `>=`, `<=`, `>`, `<`, `==`, `!=`
- Logical: `&&`, `||`, `!`
- String operations: `++` (concat), `_str_len`, `_str_find`, `_str_slice`, `_str_startsWith`, `_str_endsWith`, string comparisons
- List operations: `_list_length`, `_list_head`, `_list_nth`, `::` (cons), `++` (concat), list literals
