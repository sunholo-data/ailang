...
2. **SessionStart Hook** (`scripts/hooks/session_start.sh`)
   ...
   - **Actions**:
     - Checks user inbox for messages from autonomous agents
     - Displays notification with unread count and preview
     - **Forwards messages to Claude Code assistant inbox** (makes them visible to AI)
     - Marks forwarded messages as read
...
The hook also forwards the message to the Claude Code assistant inbox, making it visible to the AI in the current session.

**To verify forwarding worked:**
```bash
tail -10 ~/.ailang/state/hooks.log
# Should show: "Successfully forwarded message msg_... from cli-sender to claude-code"
```

## How It Works

### Message Forwarding (SessionStart Hook)

When you start a Claude Code session, the SessionStart hook automatically:

1. **Checks user inbox** for unread messages at `~/.ailang/state/messages/inbox/user/_unread/`
2. **Displays notification** to you in the terminal
3. **Forwards messages** to Claude Code assistant inbox (project-local: `.ailang/state/messages/claude-code/`)
4. **Marks as read** by moving messages to `~/.ailang/state/messages/inbox/user/_read/`

This ensures that both you AND the AI assistant are aware of messages from autonomous agents.

**Message Format (forwarded to Claude Code):**
```json
{
  "original_message_id": "msg_...",
  "from_agent": "sprint-planner",
  "original_timestamp": "2025-10-25T12:00:00Z",
  "content": {
    "status": "completed",
    "milestone": "5/5",
...
**Note:** Due to Claude Code architecture, the AI assistant can only see forwarded messages if you explicitly show them (e.g., by running `cat .ailang/state/messages/claude-code/*.json` or asking the assistant to check). The hook prepares messages for AI consumption but doesn't automatically inject them into the conversation context.
