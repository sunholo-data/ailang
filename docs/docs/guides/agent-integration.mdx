...
📖 **<a href={`/docs/prompts/${ACTIVE_PROMPT}`}>Teaching Prompt {ACTIVE_PROMPT}</a>** - Complete AILANG teaching prompt
...
**Essential reading:**
- **<a href={`/docs/prompts/${ACTIVE_PROMPT}`}>Teaching Prompt {ACTIVE_PROMPT}</a>** - Complete syntax guide (MUST READ before coding)
...
- https://ailang.sunholo.com/docs/prompts - Teaching prompts
...
- [ ] Read the full teaching prompt (run `ailang prompt` or see [Teaching Prompts](/docs/prompts))
...
1. Run `ailang prompt` to get the current teaching prompt
...
**For AI agents:**
- If syntax is unclear, run `ailang prompt` for the current syntax guide
- If examples fail, check limitations documentation
- When in doubt, generate simpler code

---

**Remember**: AILANG is designed for AI-assisted development. Your success rate will improve as you internalize the syntax patterns from the teaching prompt. Run `ailang prompt` to get the current syntax guide!
