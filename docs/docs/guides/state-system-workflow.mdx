...
Full CLI reference (send/list/read/ack/unack/forward/search) lives in the
[Agent Messaging guide](./agent-messaging.md).
...
Because state survives crashes and session loss, recovery is just "Resume
sprint M-TESTING-INLINE" in a fresh session — the skill reloads the JSON and
...
Messages not appearing → see the [Agent Messaging guide](./agent-messaging.md).
