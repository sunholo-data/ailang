...
```bash
# Quick dev check — ALWAYS scope with --models and/or --tier;
# a bare `ailang eval-suite` runs every dev model x every tier (real API spend)
ailang eval-suite --tier smoke --models gpt5-mini
...
- **[cost-and-speed-budgets.md](cost-and-speed-budgets.md)** - Cost-as-primary-gate eval semantics (v0.15.1+)

## Available Models

The model roster and pricing live in `internal/eval_harness/models.yml` — the
single source of truth. See [model-configuration.md](model-configuration.md)
for the current lineup, pricing, and how to add models. (Hard-coded model
lists in docs rot within months; this page deliberately doesn't carry one.)
...
| `frontier` | 0–40%         | minutes   | Frontier-defeating — at least one frontier model fails (v0.29.0+) |
