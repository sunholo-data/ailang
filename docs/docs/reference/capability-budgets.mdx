...
- **Per-invocation isolation**: Each call to an annotated function is bounded on its own — a caller's earlier effects are not charged against the callee's `@limit`
- **Composable / bubbling limits**: an effect charges the innermost frame **and** every active ancestor frame, so nested/delegated effects still count toward an annotated caller's budget (the tightest active `@limit` wins). See [Effects → Capability Budgets](/docs/reference/effects#capability-budgets) for the full hierarchical scoping rules.
