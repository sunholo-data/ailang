...
Generated by `make docs-env` from `internal/config` (231 variables). Do not edit; edit the `Var` beside the getter and regenerate.
...
| `PORT` | — | Cloud Run's port convention: when set, the daemon starts its HTTP server on it, `ailang server` and `ailang serve-api` bind 0.0.0.0 instead of 127.0.0.1 (DefaultBindHost; --bind overrides), and the registry validator listens on it (default 8080 there). |
...
| `MISSION_NAME` | — | Name of the mission whose fire this process belongs to (v1, docs, motoko, world, fleet); set by the mission driver. Default filer for `ailang mission ticket file`; the push scope guard keys on it. |
