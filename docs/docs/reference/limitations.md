...
**Prevention guide**: Run `ailang devtools-prompt | grep -A 80 "WASM-Friendly Patterns"` for concrete before/after examples of the five compounding patterns to avoid in WASM-targeted modules. (The guide lives in the devtools/toolchain prompt rather than the syntax prompt because it's a runtime-target concern, not a syntax concern. The language syntax prompt has a short cross-reference.) The patterns also produce more idiomatic AILANG even for CLI-only code — the workarounds (flatten matches, extract unpack helpers, qualified imports, single-effect functions, let-bind intermediate records) are good style regardless of target.
