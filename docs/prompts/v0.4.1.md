...
- List: `[]` | `x :: xs` (sugar v0.4.1+, expressions only) | `match xs { [] => ..., ::(h, t) => ... }` (patterns use canonical)
...
**Sugar:** `x :: xs` (infix cons for lists, **expression contexts only**)
**Canonical:** `::(x, xs)` (function call)
**Right-associative:** `1 :: 2 :: []` → `::(1, ::(2, []))`

**⚠️ Important:** S-CONS sugar only works in **expressions**. In **patterns**, you must use the canonical form `::(h, t)`.
...
-- ⚠️ Pattern matching requires CANONICAL form
match list {
  [] => 0,
  ::(h, t) => h + sumList(t)  -- Must use ::(h, t), NOT h :: t
}
```
