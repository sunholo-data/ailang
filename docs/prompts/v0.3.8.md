...
**⚠️ CRITICAL: Zero-argument functions**

AILANG uses ML-style call syntax (juxtaposition), NOT C-style `f()` syntax:

```ailang
-- ❌ WRONG - This is a parse error!
getMessage()      -- Parser doesn't support empty parens

-- ✅ CORRECT - Pass the unit value with a space
getMessage ()     -- Calls function with unit argument

-- ❌ WRONG - No space means parse error
f()

-- ✅ CORRECT - Space between function and unit
f ()
```

**Why?** In AILANG:
- Function calls use juxtaposition: `f x` (not `f(x)`)
- Parentheses group expressions, they don't mark function calls
- Zero-argument functions actually take the **unit value** `()` as their only argument
- `f ()` means "apply f to the unit value"
- `f()` has no meaning (parser sees it as malformed grouping)

**Design note**: This is intentional (not a missing feature). Parenthesized call syntax `f(x, y)` may come as optional sugar in v0.5.x, but currently you must use the canonical ML syntax.
