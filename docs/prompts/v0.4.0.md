---
layout: page
title: AI Prompt - v0.4.0
parent: AI Prompts
nav_order: 1
---

# AILANG v0.4.0 - AI Teaching Prompt (Environment Variables + Net Effects)
...
- **Zero-arg call: `f ()` (SPACE required!)** NOT `f()` (parse error)
...
Modules, functions, lambdas, pattern matching, ADTs, records, effects (`! {IO, FS, Net, Env}`), recursion, numeric conversions, HTTP with headers, JSON encoding, **property-based testing**, **environment variables**.
...
**Many benchmarks require making HTTP POST requests with custom headers and JSON body. Here's the complete pattern:**
