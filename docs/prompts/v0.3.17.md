---
layout: page
title: AI Prompt - v0.3.17
parent: AI Prompts
nav_order: 1
---

# AILANG AI Teaching Prompt (v0.3.16 - Entry-Module Prelude)
...
**NEW in v0.3.16**: Entry modules and REPL now have automatic `print` builtin - no import needed!
...
## Current Version: v0.3.16 (October 2025)
...
- ✅ **Entry-module prelude** - `print` builtin available in entry modules (no import needed!)
...
- ✅ **HTTP headers** - `httpRequest()` with custom headers and Result-based error handling (since v0.3.9)
- ✅ **JSON encoding** - `std/json` module with `encode()`, `jo()`, `ja()`, `kv()` for building JSON

**⚠️ CRITICAL LIMITATIONS - What AILANG Does NOT Have:**

**Syntax that does NOT exist:**
- ❌ NO `for`/`while` loops → use recursion or map/filter
- ❌ NO list comprehensions `[x*2 for x in list]` → use recursion or `map`
- ❌ NO function type syntax `func(T) -> U` → use lambda values `\x. expr`
- ❌ NO JSON literals `{"key": "value"}` → use `jo([kv("key", js("value"))])`
- ❌ NO mutable variables `var` or `let mut` → everything is immutable
- ❌ NO assignment `x = y` → use `let x = y in expr`
- ❌ NO error propagation `?` → use pattern matching on Result
- ❌ NO list spread `[x, ...rest]` → use Cons constructor in patterns

**Important notes:**
- `show` is a builtin - do NOT import it
- Higher-order functions use lambda values, not type annotations
