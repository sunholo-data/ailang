...
| DOCS-2-02 | Intro page names IFC Labels as `v0.16.0` while the latest prompt is `v0.16.6`. | 1 | MEDIUM | `check_versions.sh` reports `[STALE] intro.mdx references v0.16.0, latest is v0.16.6`. | Defer to a future item; `docs/docs/intro.mdx` is nested and outside this sprint's single-level `docs/*` allowlist. **REFUTED 2026-08-31 (docs-9, controller live-repro):** false positive — `intro.mdx`'s "Recent Additions" bullets intentionally show each feature's ship-version, not the current version; `v0.16.0` is correct for IFC Labels and diffing v0.16.0→v0.16.6 prompts shows no IFC content changed. The producing check (`check_versions.sh` Check 3) was structurally incapable of being correct here and has been removed. No `intro.mdx` edit was made or needed. |
