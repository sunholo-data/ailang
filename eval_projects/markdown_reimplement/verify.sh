#!/usr/bin/env bash
# verify.sh <ailang-parse-dir> — grade a (re)implementation of docparse/services/markdown_parser.ail
...
for g in "$GOLDEN"/*.golden; do
  ...
  got=$(cd "$AP" && ailang run --entry main --caps IO,FS,Env docparse/main.ail "data/test_files/$base.md" 2>/dev/null \
