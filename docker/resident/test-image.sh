...
have "ailang pi tool-profile ailang_only has no bash" 'case "$(ailang pi tool-profile ailang_only)" in "--no-extensions -e "*"--no-builtin-tools --tools read,edit,write,ailang_check,ailang_run,builtins_search,examples_search") true;; *) false;; esac'
...
have "  ...and allows exactly the ailang_only set" 'grep -q -- "--tools read,edit,write,ailang_check,ailang_run,builtins_search,examples_search" <<<"$ARGV"'
