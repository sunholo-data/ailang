...
6o. [**IN-SPRINT — BUILT, JUDGED AND VERIFIED 2026-09-03 (iteration 34); NOT LANDED, AND THE REASON IS NOT OURS.**
   ...
   **RESUME PREDICATE, to be RUN as a command and never transcribed: `test` AND `lint` green on `dev`.** Then land and close.
   **SCOPE, because a 96/100 does not cover it:** the `launchd drivers (bash 3.2)` target dies at the fmt_ab script before
   reaching the probe suite, so these arms have **never run in CI** — all evidence is darwin/arm64 local; ubuntu and windows
   legs unrun. Design + plan: `m-motoko-group-kill-and-lsof-containment.md` (+ sprint plan), quorum BLOCKED r1/r2, resolved
