...
5. [IN-PROGRESS — case CORRECTED and BOUNDED 2026-08-13 (iteration 3); filing gated on one
   ...
   **≥78,644** — *more* headroom than the 75k reserve `96542f8` adds, so upstream already beats our
   ...
   the waiting, not the offer. **Remaining before filing (one unit-level assertion, NOT a rig run):**
   the disposition's **R8**, re-sharpened this iteration to the single open question — is the
   70→95 band reachable in practice? Both functions are `pure`, so it is a direct call, not a
   provider A/B · ≤1 iteration
