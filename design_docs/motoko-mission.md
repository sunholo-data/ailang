...
## STATUS 2026-09-06 — ITERATION 37: COMPLETE: **THE REMAINING SUITE-BOUND DERIVATION LANDED, AND THE JUDGE FALLBACKS WERE TREATED AS EVIDENCE RATHER THAN EXIT CODES.** Row **6p M2/M3** landed in PR [#1066](https://github.com/sunholo-data/ailang/pull/1066), merge [`6c03639f5`](https://github.com/sunholo-data/ailang/commit/6c03639f518fa45569b879bfa73c2d31e5b3d62f). M2 consumes the measured wall-clock class, enforces the 100/s floor, and refuses `p_obs > 4.70`; M3 replaces the discovery arm's hardcoded node ceiling with the derived value. Ordinary and forced-rate suites pass **59/59**; the forced 200/s diagnostic is exact (`scale=2 arm_cap=240s node_ceiling=3200 floor=enforced`); independent M2 and M3 mutations each red their own census gate; the production probe stayed byte-identical at `f0b5e024…aabc99`. A 10-pair loaded A/B produced zero reds in either arm and is therefore **UNINFORMATIVE**, not proof of benefit. The final independent evaluator gave **PASS 84/100** and reproduced per-milestone non-vacuity; its delayless control first hit the 240s cap, then passed with marker_count=230, while factor-one red exactly at 200 nodes. Generator≠judge holds at the agent/model level but not provider level: primary `pi:ollama/minimax-m3:cloud` timed out after 1,806s with no verdict; fallback `pi:openrouter/minimax/minimax-m3` authored a substantive PASS 90 report but the runner correctly rejected it as `empty_worktree` because the controller sent it to ignored `.ailang/`; the final required Agent-tool judge was `codex:gpt-6-astra`, distinct from the `codex:gpt-5.6-sol` executor, and the same-provider exception is **FLAGGED**. All four roles were spawned with the Agent tool: designer freshness pass, planner refresh, sequential executor M2/M3, final evaluator. Gate 3b is green on the merge SHA: **20 checks, 0 pending, 0 failed**, including `launchd drivers (bash 3.2)`. Goal unmoved — this is `[HARNESS]` completion, not epic progress. Gate-5: no skill edit; the ignored-path, typed-timeout, nonempty-artifact and fallback rules already existed and fired. Next: row **16**, then row **7**.
...
6p. [**LANDED 2026-09-06 · iterations 35 + 37** — M1 in PR [#1048](https://github.com/sunholo-data/ailang/pull/1048); M2/M3 in PR [#1066](https://github.com/sunholo-data/ailang/pull/1066), merge [`6c03639f5`](https://github.com/sunholo-data/ailang/commit/6c03639f518fa45569b879bfa73c2d31e5b3d62f). The suite now measures and consumes its wall-clock class, enforces the floor and `p_obs` refusal, and derives the discovery node ceiling. Ordinary and forced suites pass 59/59; evaluator PASS 84/100; merge SHA 20/20 green including bash 3.2. Historical M1 record follows.**]
   [**M1 OF 3 LANDED 2026-09-05 · iteration 35** — PR [#1048](https://github.com/sunholo-data/ailang/pull/1048),
   ...
   **M2 AND M3 WERE THE ROW'S RESIDUAL AND LANDED IN ITERATION 37** — the historical handoff is retained here;
   the implemented design and sprint plan are archived under `design_docs/implemented/v0_35_1/`.
...
6s. [**STILL PARKED 2026-09-07 · iteration 39 — the ruling's two conditions are DONE and the row is parked again, on a NEW and narrower question.** `D-MOTOKO-CARVEOUT-1` was answered **(B) OVERRULE**, and iteration 39 did exactly what it ordered: the residual audit was RUN (full file 1..1212 at base `878939117`; it found precondition **P1**, the 14 self-re-exec sites three quorum rounds had missed) and a FRESH round-3 quorum was run. It **BLOCKED 3/3, all present** — `gpt6-astra` first dropped out on `budget` after the doc grew 276 → 520 lines and was re-run alone at a raised cap, returning REJECT. `oc-glm-5-2`'s remedy 2 now ships verbatim (its remedy 1, a real Linux host, is measured unavailable: six container runtimes absent, control `git` present), so the round-2 PATH-shadowed `expected_arms=56` is gone. What blocks now is **P2**, filed as `D-MOTOKO-P2-1`: the exact count depends on the loopback-socket arm near line 640 staying UNINFORMATIVE, and astra's and gemini's remedies for it CONTRADICT, so the loop may not choose. Independent evaluator **PASS 90/100, zero blocking** (`docs/sprint-retros/motoko-iter39-evaluation-round1.md`) — it reproduced every audit row first-party and ran two mutants the doc had only specified. Design revision `f6750002d` on `sprint/motoko-iter39-armcount-r3`; the M1 code is unchanged and still unmerged. Historical iteration-38 tag follows —] [**PARKED `needs-human-review` 2026-09-07 · iteration 38 — the WORK IS DONE AND UNMERGED; what is parked is a process question about how it got approved.** Design, sprint plan, implementation and mutation matrix all exist on branch `sprint/motoko-iter38-changelog-arms` (design [`0656ac174`](https://github.com/sunholo-data/ailang/commit/0656ac174), plan [`11572ac59`](https://github.com/sunholo-data/ailang/commit/11572ac59), implementation [`bd4ee3d87`](https://github.com/sunholo-data/ailang/commit/bd4ee3d87)) and the gate is measured to work: pristine `PASS: 60`, rc=0, and four mutants each red — removal 59, **addition 61** (the only row that proves the gate LOOKS rather than merely fires), self-arm deletion 59, zero-vacuity — with the controller independently re-running the addition and self-arm rows OUT OF SANDBOX and restoring to `52ab531e…296df7` each time. **It is parked because the INDEPENDENT EVALUATOR failed the controller, not the code: FAIL 68/100, two BLOCKING findings, both about Gate 2 conduct.** The quorum BLOCKED 3/3 in both rounds; the controller then invoked the narrow-refinement carve-out for a bounded second revision instead of parking. The judge checked the carve-out's own condition — apply the reviewers' text VERBATIM, never a controller-invented resolution — and found it violated twice: `gpt6-astra`'s fix said *"Mark the prior quorum objection unresolved until this audit is recorded"* and only its cosmetic half was applied; `oc-glm-5-2` offered two remedies (run on a real Linux host, or gate the non-Darwin path behind an explicit `UNVERIFIED` refusal) and the controller shipped a third — a PATH-shadowed simulation, honestly labelled, that glm was never asked to accept. Confirmed first-party against the controller's own directives. **The ask is one word** (see `D-MOTOKO-CARVEOUT-1`); nothing else about the row is in doubt. Original filing kept below as the trail —]
...
18. [NEW — filed by iteration 39's Gate-3b red, from a first-party CI failure this iteration caused
   ...
   **Gate 4's STATUS rotation moves a heading from an UNLINTED file into a LINTED one, so every
   mission's next rotation is a repo-blocking red waiting to happen** · loop health ·
   `TestMissionDocHeadingsStayCanonical` lints `*-mission-log.md`, `*-mission-log-archive.md` and
   `*-mission-status-archive.md`, and deliberately **excludes charters** — the comment says so:
   *"Charters are excluded deliberately: they are curated prose, and a lint that demanded canonical
   headings there would be linting an essay."* Gate 4's rotation step then carries the now-4th STATUS
   stamp from the charter into the archive, i.e. across that boundary. Measured 2026-09-07 on
   motoko's own record PR: `test` and `test-windows` both `failure` with *"non-canonical record
   headings rose to 13 (limit 12)"*, on a docs-only change that never touched the offending heading's
   text. Canonical is `## STATUS <date> — ITERATION <n>: <title>` (`canonicalStatusRe`,
   `internal/mission/normalize.go:37`); the fleet writes `ITERATION <n> COMPLETE:` without the colon.
   Motoko fixed its own four stamps in place, so its next three rotations are safe. **V1, World and
   docs are still loaded**, and `ailang mission normalize --apply` is not the fix a single mission may
   run — it rewrites 9 headings across siblings' archives in one go. **Scope:** either make Gate 4's
   rotation step emit the canonical heading (cheapest, and it is one line in the gate), or have the
   normaliser offer a `--mission` scope so each loop can fix only its own. The first is better: a
   heading should be born canonical rather than repaired after it crosses a lint boundary ·
   1 iteration
