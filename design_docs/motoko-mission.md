...
6h. [**NEXT** — split out of 6f's triage on first-party evidence, per the rule that a confirmed defect a
...
6i. [NEXT after 6h — filed by iteration 24's OWN mutation drill, per rule 3n: anchor the enumeration to the diff,
   not to the defect] **The production `run_lane` process-group kill is pinned by nothing — reverting the whole
