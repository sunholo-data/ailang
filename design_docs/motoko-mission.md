...
6. [**M1 + M4 LANDED · M2, M3, M5 are the RESUME POINT, not dropped.**
   ...
   **Remaining after iteration 15:** **M2** `AC-D1-live` — one fmt-lane run reaching
   ...
   since the preflight no longer refuses the fmt lane); **M3** D1b counterbalanced Wednesday block
   — **now unblocked**, because AC-M3-4 asserts the §5.3 gate's predicate by calling M4's
   order-integrity checker on synthesised rows, and M4 has landed; **M5** smoke-bank wiring
   (depends on M3). M4 was taken ahead of M2/M3 deliberately: the plan declares it *parallel-safe
