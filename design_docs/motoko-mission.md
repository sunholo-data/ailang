...
6. [**M1 (D1) LANDED 2026-08-20 · iteration 14 · PR [#794](https://github.com/sunholo-data/ailang/pull/794) → `bc0b5a8d4` · Gate 3b GREEN on the PR head `fb1ef41b5` (21 checks, 0 not-green, 4/4 required, `mergeStateStatus=CLEAN`) · evaluator PASS 84/100, zero blocking. M2–M5 are the RESUME POINT, not dropped** —
   plan: [m-motoko-fmt-remeasurement-instrument-sprint-plan.md](planned/m-motoko-fmt-remeasurement-instrument-sprint-plan.md),
   sprint JSON records M1 `completed`/`passes=true` and M2–M5 `not_started`.
   ...
   **Remaining, in plan order:** **M2** `AC-D1-live` — one fmt-lane run reaching `localhost:11434`
   with ZERO `openrouter.ai` connections, asserted on the CONNECTION and paired with an
   OpenRouter-lane known-positive control (needs the rig; its circularity is now broken, since the
   preflight no longer refuses the fmt lane); **M3** D1b counterbalanced Wednesday block; **M4** D2
   censored-pair analyzer; **M5** smoke-bank wiring. Note **M3's AC-M3-4 depends on M4's
   order-integrity checker**, which is why M3 was not shipped alone this iteration — doing so would
   have left a milestone with an unclosable acceptance criterion.
