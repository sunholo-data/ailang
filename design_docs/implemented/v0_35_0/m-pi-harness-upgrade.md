...
| V33 | **0.85.1 differential (M3.0)** — three live captures on the rig 2026-09-16 (`ollama/glm-5.3-flash:cloud` fizzbuzz + tool-use; `openrouter/z-ai/glm-5.3-flash --thinking medium` for reasoning), `pi --version` 0.85.1 | top-level key sets per event type, assistant `message_end.usage`, `toolcall_start` fields, `agent_end.willRetry`, `message_update` sizes | **Every row of the 0.73.1→0.84.4 table holds unchanged at 0.85.1**: `session{cwd,id,timestamp,type,version}`, `message_update{assistantMessageEvent,type,usage}` avg 277 B / max 520 B, `toolcall_start{contentIndex,id,toolName}`, `message_end.usage{input,output,cacheRead,cacheWrite,reasoning,totalTokens,cost}`, `rawStopReason` (`tool_calls`/`stop`), `agent_end{messages,type,willRetry}`, `agent_settled{type}`, `tool_execution_end{isError,result,toolCallId,toolName}`. No new event types. **D5 verified on real usage**: `totalTokens 4261 = input 3516 + output 41 + cacheRead 704 + cacheWrite 0`, with `reasoning 8` NOT added — reasoning is inside `output`. Captures are the `testdata/v0_85_1/` fixtures |
| V34 | The stdin trap is live on 0.85.1 | first capture run with the harness's stdin (a pipe, never written): `pi` produced **zero bytes and never exited** until killed; re-run with `</dev/null`: 33 lines, rc=0 | **Confirmed** — the resident doc's finding (`docker/resident/lib/pi.mjs`) holds; `pi.go` relies on Go's nil-Stdin → null device |
