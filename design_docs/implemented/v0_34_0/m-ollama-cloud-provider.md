...
**Status**: **ALL 9 ACCEPTANCE CRITERIA MET** — Phase 0 AND Phase 1 PASSED on the live rig 2026-08-26**; a full motoko agent benchmark runs on Ollama Cloud (V31) (signed in as `m@sunholo.com`,
...
- [x] **AC6 DONE (V41)**: Cloud rows are not force-clamped to `--parallel 1` (D4), and honour the tier
      concurrency cap
- [x] **AC7 DONE (V22, V51)**: A quota-exhausted request banks the **existing** `error_category` `quota_exhausted`
...
- [x] **AC8 DONE (V22, V51)**: `isRetryableError` returns **false** for quota exhaustion and **true** for a
...
| V22 | **ANSWERED — the exhaustion contract, captured verbatim by deliberately spending a session window (Mark, attended).** This unblocked AC7 and AC8, which had been open since the design was written | parallel burst of kimi-k3 requests until the limit; raw status + body dumped | **HTTP 429**, body: `{"error":{"message":"you (marked) have reached your session usage limit, upgrade for higher limits: https://ollama.com/upgrade or add extra usage: ... (ref: <uuid>)","type":"api_error","param":null,"code":null}}`. **The status and type are BOTH shared with a transient rate-limit** — 429, and `api_error` rather than a distinct quota type — so neither can discriminate. **Only the message text can.** This is precisely why the quorum was right to refuse an earlier draft that guessed the shape: a carve-out keyed on status would have been wrong |
| V50 | **POS — THE DENOMINATOR IS 1.0.** V26's central gap is closed | usage read each round of the V22 burst | Trajectory `0.613 → 0.792 → 0.972 → **1**`, then hard 429. So `limits.session.usage` is a FRACTION OF 1.0, not an absolute count — every earlier ratio can now be read as an absolute. Concretely: deepseek-v4-flash full frontier ≈ 0.10 ⇒ **~10 frontier passes per 5h session**; the full 56-benchmark suite ≈ 0.18 ⇒ **~5 per session**; kimi-k3 frontier ≈ 2.4 ⇒ **cannot fit in one window at all**. Weekly is a SEPARATE, much larger denominator (0.291 after the same work) — V37 stands |
| V51 | **POS — AC7 and AC8 IMPLEMENTED against the measured shape, not a guess** | new shared `isQuotaExhaustion`; carve-out placed BEFORE the 429 rule in `isRetryableError`; 6 tests using the verbatim V22 body | Before: `error_categorizer` matched `monthly limit`/`quota exceeded`/`billing` — **none of which match `session usage limit`** — so exhaustion banked as `rate_limit` or `api_error`; and `isRetryableError` returned **true** on any `429`, retrying into a bucket that cannot recover for 5 HOURS (weekly: days). After: exhaustion banks `quota_exhausted` and is NOT retried, while transient 429s still ARE. One shared definition so the categoriser and the retry predicate cannot drift. 15 packages pass |
