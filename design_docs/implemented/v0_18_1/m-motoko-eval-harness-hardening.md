...
**Result**: 7 of 7 acceptance-gate conditions met. End-to-end smoke verified: `Success=true, Cost USD=$0.000730, Finish reason=stop`.
...
- motoko_agent `motoko-bisect-gap1` (PR target): `7997c8f` (M1c JSONL drain), `faada61` (M2a profile mirror), `7d595a4` (M2b/M2c CLI flags), `1055976` (M4a-c session_id unification), `77e7de0` (M5b cost env override), `af9cd68` (M5 follow-up: TS childEnv MOTOKO_COST_* passthrough)
- ailang-packages `main`: `5e2eaef` (drop dotted tool aliases for Anthropic Bedrock compat — 28 aliases across omnigraph, context-mode, exa-search)
**Dependencies**:
