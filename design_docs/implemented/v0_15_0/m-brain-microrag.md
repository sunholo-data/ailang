...
5. **Frontend D — Gemini CLI hooks (`microrag_context.sh`, `microrag_lint.sh`).** Added in v0.15.0 sprint M-EXEC-EXPAND / M4A. Byte-identical to Claude Code shims apart from session-id env var (`GEMINI_SESSION_ID` instead of `CLAUDE_SESSION_ID`). Registered in `.gemini/settings.json`. Templates at `.claude/skills/microrag/frontends/gemini/`.
6. **Frontend E — Codex CLI hooks (`microrag_context.sh`, `microrag_lint.sh`).** Added in v0.15.0 sprint M-EXEC-EXPAND / M4A. Byte-identical to Claude Code shims apart from session-id env var (`CODEX_SESSION_ID` instead of `CLAUDE_SESSION_ID`). Registered in `.codex/hooks.json`. Templates at `.claude/skills/microrag/frontends/codex/`. Codex CLI's hook stdin schema (`.tool_name` + `.tool_input.file_path` + `.tool_input.content`/`.new_string`) matches Claude Code's to-the-field, so the shim and output envelope are reusable.
7. **Frontend F — opencode TypeScript plugin (`microrag-plugin.ts`).** Added in v0.15.0 sprint M-EXEC-EXPAND / M7A. opencode uses a TypeScript module plugin system (not bash hooks), so this frontend is a TS file exporting `preToolUse`, `postToolUse`, and `sessionStart` functions. `preToolUse` shells out to `ailang micro-rag context` via `child_process.spawnSync`, identical logic to the bash shims. Session isolation via `AILANG_MICRORAG_SESSION` set from opencode's session ID in `sessionStart`. Install: copy `microrag-plugin.ts` to `~/.config/opencode/plugins/` and register in `opencode.jsonc`. Source at `internal/executor/opencode/plugins/`. Toggle: `AILANG_MICRORAG_ENABLED=0`.
