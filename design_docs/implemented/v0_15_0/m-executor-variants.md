...
> **Implemented** (v0.14.2) — per-agent executor image variants shipped. Landed 2026-04-24 in
> `721550fdb` (M1 core Go wiring + base Dockerfiles) + `c07bf73c1` (codex + opencode images) +
> `af36a00a1` (gemini + eval images + CHANGELOG). `ExecutorVariant` field live in
> `internal/coordinator/agent_registry.go` + `cloud_dispatcher.go`. This full design doc replaces
> the earlier rough-draft stub that lived at this path. Swept planned→implemented in the V1
> mission backlog re-triage batch 3 (iter 87).
