...
| V31 | B1's carry-over boundary — `cross-project-messaging.mdx`'s *Automated Feedback (Advanced)* section is a real H2, adjacent to the next H2, and its body is the "~15-line bash block" claimed (D-3's condition: verify EVERY section-cut boundary the brief asserts, not only the B3/B4/B5 that reviewers happened to name — B1 is the one Phase-B item that also defines a boundary and had no V-row) | `grep -nE '^##+ ' docs/docs/guides/cross-project-messaging.mdx`; read lines 232–256 | `232:## Automated Feedback (Advanced)` immediately followed by `257:## Semantic Search` — both genuine H2s (`## `, not H3/bold), no heading in between; body is one intro line + a single ` ```bash ` fence spanning lines 236–253 (15 lines of bash between the fences, matching "the ~15-line bash block under that heading"), then a `---` separator before the next heading — B1's boundary (232 up to, not including, 257) is exact, and the carried-over content is exactly the fenced block, not the whole section. |
