...
| 8n | marker present, stdin `null` (valid JSON, no fields) | `deny`, reason contains `fail-closed:payload-unparsable`, rc 0 | drop `-e` from the payload gate's `jq -e .` (null AND empty input then pass the gate) → **arms 8e and 8n die** (measured: `printf '' \| jq .` also exits 0; the first draft of this row claimed 8n alone) |
