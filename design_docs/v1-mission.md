...
## STATUS 2026-08-31 — ITERATION 309: **The approved sprint's own headline unit would have regressed production routing, and the loop measured that before landing it.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == origin via the RESOLVED symlink (inode-checked against the pin's own copy); dev required workflows green at `8bb92ae2b`, only the standing non-required SonarCloud red (inherited — present on `4bd58bef6`). Canonical cloud inbox read with the invalid-store control FIRING (`unknown message store mode`, rc=1) on a freshly `-ldflags`-built binary; **200 unread, 178 of them docparse task-status noise**, and the first `--json` enumeration returned only 20 — a truncated enumeration re-read at `--limit 500`. 0 directives since watermark on #972 and #852; decision ledger **50 rows, 0 OPEN**. Pick = `m-coordinator-child-env-opencode-retry-storm`, on Mark's attended D-50 `execute sprint` authorization, which outranks the queue head. **The iteration's finding: the authorized unit is textually clean and semantically a regression.** `git cherry-pick --no-commit 3500db0a7` is rc=0 with zero conflicts at HEAD — and `Dispatcher.Dispatch` calls `coordinator.ValidateExecutionRoute` (dispatcher.go:184) BEFORE `checkVariantProviderAgreement` (:195), so M1 becomes the OUTER gate and the dispatchable set collapses to the intersection the instant it lands. Enumerated over both REAL functions: **104 cells, 39 over-rejects, 0 under-rejects**, four causes — `managed_agents` (13), empty provider (13), `eval`/`eval-go` wildcard images (12), `pi-go` (1). Confirmed by three independent instruments (controller replica, codex executor's `route_matrix_test.go` driving both real functions, planner's analytic 49−10). The designer had found 2 of the 4 causes; the quorum reviewer who demanded a cell-by-cell table was right, and the measurement is what found the other 2. **Blast radius on the DEPLOYED config is ZERO** — live prod carries 35 agents (pi×32, codex×2, motoko×1), all inside the 10-cell accept set — so this is latent, not active, and there is no urgency. Quorum **BLOCKED twice** (r1 3/3 present, all reject; r2 `oc-glm-5-2` absent, degraded N−1). All five objections were premise objections and were MEASURED, not forwarded: two refuted, one converted to a normative constraint, one confirmed-and-enlarged, one confirmed — `ailang coordinator config diff` validates **`local` only, inside the DIFFERS branch only**, so the deploy-code-with-unchanged-config case returns nil having validated nothing; the doc's rollout safety net is vacuous where cited. My own first version of that finding said "no route evaluation whatsoever" and the PLANNER refuted it — recorded as a correction, not smoothed. **Disposition: code merge PARKED, record LANDED.** Evaluator `sonnet` **74/100 PASS** with two blocking findings, both acted on in-iteration: the zero-risk subset was landed (`accept_set_pin_test.go`, pinning dev's own 49/104 accept-set, 3 mutants killed — add-variant 112≠104, widen-accept 50≠49, empty-enumeration fires the anti-vacuity floor — each BUILDS, restore byte-identical) and the planner's analysis was committed rather than left in `/tmp`. Executor refused round 1 on a genuine self-contradiction in MY directive (forbade all git writes, then required a cherry-pick) — the correct call; controller pre-applied and re-ran. Evidence branch `mission/iter309-route-authority-parity` @ `8c8c29864` holds M1+probe UNMERGED. metered=**$0.2377**.
...
**[IN-SPRINT &mdash; iter-311: Sprint 1 **M1 LANDED**; M2&ndash;M5 remain (~3.5d)] `m-registry-interface-hash-blind-to-signatures` &mdash; a BREAKING API change is classified `patch` to every downstream consumer.**
