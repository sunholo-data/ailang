...
**[NEXT &mdash; iter-316, first-party on CI with the divergence control fired; small, and it taxes every PR until fixed] `m-message-watcher-windows-wallclock-flake` &mdash; `TestMessageWatcherStart` reds on the Windows runner from an absolute wall-clock bound.**
...
**[NEXT &mdash; iter-317, found by the sonnet evaluator on PR #1013 and reproduced by it with the mutant asserted LANDED+BUILDS; small, and it leaves the fix that just cleared a dev red unprotected] `m-probe-derace-has-no-killer` &mdash; a full revert of the process-tree de-race passes the whole suite, so CI cannot see the fix disappear.**
