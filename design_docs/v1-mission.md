...
**[NEXT] [Mark, attended 2026-09-01 — design APPROVED by Mark in session; fleet infra, this loop owns the shared skill] `m-spawn-pin-enforcement` — a `provider:model`-pinned role can be silently spawned on an Anthropic alias (sonnet/opus) because the Agent tool's enum (`sonnet|opus|haiku|fable`) cannot carry a `provider:model` pin, and the skill's advisory text about that limitation has been skipped twice — measured.**
