...
- **[UNPARKED — `D-15` ANSWERED 2026-08-14 (Mark, attended): `view`. Resume = `#698` part 1 M4 (`passes: null`, `blocked_on: human_decision` → clear), implementing the recommended ~230 LOC scope; `eval-*` errors loudly on `--remote`. Was: PARKED — `D-15`, needs Mark]** `#698` part 1: opt-in remote READ — how far should `--remote` reach, `view` or `eval`?** The ratified freeze says every `chains`/`eval-*` consumer inherits the option; the code refutes that as wiring-only work. `ListEvalChains` is a 5-line wrapper over `ListChains` (on `Backend`, implemented by Firestore), so it is servable — but **`QueryEvalResults` is `*Store`-only**, and Firestore stores `eval_assessment` as an **opaque JSON string**, so its six `json_extract` filters cannot be served remotely without a schema change. Options priced by the planner from the code: **`view`** (~230 LOC/~4h — remote read for the `Backend`-shaped `chains` commands; `eval-*` stays local and errors loudly on `--remote`, turning assumed demand into a dated signal) — **recommended**; `eval` (~550 LOC/~9h — adds a narrow `ChainReader` plus a full-collection Firestore scan a later schema sprint would replace); `Backend`-promotion (~595 LOC/~10h — rejected by the planner as boilerplate across 5 impls for no gain over `ChainReader`). Choosing `view` NARROWS a ratified freeze item, which is why it is Mark's call and not the loop's. Plan + JSON already landed with M4 `passes: null`, `blocked_on: human_decision`.
