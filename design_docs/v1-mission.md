...
- **[DOC LANDED + QUORUM-CLEARED 2026-08-04 (iter-137) — PR #582 → squash `2629ad8fa`, dev CI GREEN
  ...
  **Two reviewer claims MEASURED, not forwarded**: (1) "per-request SDK servers break SSE" REFUTED on
  ...
  that in-process Go callbacks cannot be forcibly terminated. **NEXT: route to sprint-planner** — the
  doc declares `**Planner-Lane**: opus-required` and `derive-planner-lane.sh` agrees
  (`opus declared:opus-required`), ~17h across 3 milestones. Was: NEXT — the QUEUE HEAD after
  m-planner-codex-lane LANDED at iter-136.
