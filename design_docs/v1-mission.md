...
## STATUS 2026-09-08 — ITERATION 349: **This slot's first attempt died at Gate 3b holding a green PR; attempt 2 judged it, landed it, and found the whole fleet has been running frozen driver code for 43 commits.** The pick was **inherited, not chosen**: attempt 1 (fire 00:02:48, killed 01:20:55 by the stall watchdog, `rc=143`, `at=gate-3b`) had run the entire inner loop and opened PR [#1107](https://github.com/sunholo-data/ailang/pull/1107), then died waiting on CI it had already passed — leaving **zero** charter rows, zero log entries and zero STATUS stamps. Gate 2's trace (a) found the orphan PR; trace (b) attributed it to this mission first-party, since `sprint/v1-iter349-launchd-drain-budget` has a worktree in **this clone's** list (proof of ownership in a way `--author` is not, because V1 and motoko share the push identity). **It had no independent evaluation on record** — no verdict artifact anywhere, and attempt 1's judge worktree sits clean at the reviewed commit, so a judge was spawned and its verdict died with the slot. Landing on that would have been a generator-only landing, so this attempt verified before adopting: `test_driver_notify.sh` at `abaa190f2` reproduces **40 passed, 0 failed** first-party, and the PR's live-rig spool claim reproduces exactly with a known-absent spool as the negative control. **Shipped** [`b5513ccdf`](https://github.com/sunholo-data/ailang/commit/b5513ccdfb7b9e014c197e9021d27be5272ebbe1), 21 checks on the PR head with **zero not-green**: `MISSION_DRAIN_BUDGET` (default 90s) caps the WHOLE drain in the one phase of a fire with no deadline behind it but the 6h `HARD_TIMEOUT`, re-spooling the current row and every still-unattempted row unchanged, so an outage costs a delay and never the record. **Independent judge `sonnet` PASS 88/100, zero blocking** — and it measured rather than confirmed, because I withheld both my candidate findings and my method: it re-derived the base (38/0 at `ead709c31`, so the gate measures M3 and not the repo), ran a mutation drill **anchored to the diff** in which blanking only the diagnostic log line kills only the diagnostic assertion (39/1) — the reading that proves the two assertions are independently anchored — and established non-vacuity the strict way, `patch -R` reverting **M3's own hunk alone** with M1/M2 kept and byte-identity verified, both assertions going red. **Three non-blocking findings, all reproduced by me before disposition, two of them about my own side of the record:** a real `deferred` counter over-count when a malformed spool row precedes the deferral point (queued as `m-drain-deferred-counter-overcounts`, deliberately NOT fixed in-flight — a one-line un-reviewed edit to a just-merged green path is the change this loop keeps closing); the PR body's own Measurements table claiming `grep -c MISSION_DRAIN_BUDGET` → `0 → 2` where the true count is **1** (control `NOTIFY_TIMEOUT`=7 fires), **corrected in the PR body before merging** and stated as a correction; and a missing changelog entry against a standard the sibling M1/M2 landing observed (39 lines), repaired here. **The iteration's real finding is bigger than its pick.** `AILANG_DRIVER_REF` is a **fixed SHA** for all four missions — `48c4a6e49`, attended deployment stamp `20260907T151903` — unmoved for **43 commits**. Three of those touch `tools/launchd/`, including `e5a325a20`, which is **M1/M2 of this very sprint**: the driver on the rig does not contain the D-60 bounding M3 builds on, so **this merge is inert until the pin moves**. `pin-root.sh` cannot report it — `PIN_DRIFT` counts commits the clone is behind **the ref**, so a SHA pin that is an ancestor of the clone reads `0` forever, and this fire logged `driver pin drift: 0 below warning threshold 25` with 43 commits of drift. Zero prior mentions of `AILANG_DRIVER_REF` in the charter or the entire log. Queued as `m-fleet-sha-pin-freezes-every-driver-fix` and asked as **D-61**. **Gate-5 skill edit:** the Gate-1 staleness `cmp` covers `SKILL.md` and nothing else, while the rules now live in twelve `resources/*.md` that every gate stub tells you to read by **relative** path — which resolves against the pin worktree, not the checkout `SKILL.md` came from; measured `SKILL.md` clean, `gate-3-route.md` 75,448 vs 78,390 B and `gate-3b-ci-green.md` 30,155 vs 33,494 B, with **ten of twelve matching**, which is what makes it quiet. **Ruled out:** *"`NOTIFY_TIMEOUT` may be unset where the new guard reads it, making the fix vacuous in production while the tests stay green"* — **REFUTED** (`:514` assigns, `:1018` calls, `set -uo pipefail` would abort loudly); and *"tonight's `Eval Suite 0/2` is the new standard-mode gate firing"* — **REFUTED by timestamp**, that run predates #1104 by 37 minutes. **M4 still owed** — the sprint's 11-row M1–M3 sweep was never executed. Designer/planner/executor deliberately NOT spawned (verify-and-land of an already-quorumed sprint); evaluator `agent-tool sonnet` 131,510 tok / 73 tool uses / 1,821 s in its own worktree. N=12, goal unmoved (HARNESS). Metered **$0.00** of $5. Ledger 61 rows, **ONE open (D-61)**. Full record: v1-mission-log.md iteration 349.
