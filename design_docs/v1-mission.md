...
- **[NEXT] `#692` — batch mode silently drops `Debug` ghost-effect output** (`flushDebugOutput` never runs per item; filed by iteration 192 alongside `#691`). [cli-DEMAND]; P2; small fix, no new doc — same direct-fix lane, and the acceptance must include a batch fixture whose per-item Debug output survives, plus a neutering mutation that reds it.
- **[NEXT] `#706` — `exit(0)` in a serve-api route handler returns HTTP 500** (found by the iteration-197 evaluator, reproduced first-party by the controller). No host special-cases `*embed.ExitError`: `routes_dispatch.go:145`, `a2a.go`, `mcp.go` all treat any non-nil error as a hard failure, so `#691`'s documented "hosts branch on `Code == 0`" is satisfied by nobody. **Not a regression** — that path previously crashed the whole serve-api process — so this is an incomplete improvement, not a defect introduced by `#691`. Needs a one-word contract call: is `exit(0)` from a handler a SUCCESS response (recommended, matching the CLI batch contract) or an error with a better message? Three call sites → three separate arms, per the guard-the-helper-miss-the-call-site shape that produced this whole chain.
- **[NEXT] `#703` — `govulncheck-filter` silently drops module-level findings** (filed by iteration 196; the gate prints a confident green whose denominator excludes a whole class, and one dropped finding is a real unallowlisted Ollama advisory, `GO-2026-5750`/`CVE-2026-7020`, with no upstream fix). P1 gate-integrity: a vacuous green on the security gate is worse than a red.
