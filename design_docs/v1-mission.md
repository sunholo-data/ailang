...
| D-55 | RESOLVED | **Does the compile-cache artifact-verification design have to bound ADVERSARIAL gob-decode work, or is the accidental-corruption threat model enough to unblock it?** `m-compile-cache-unverified-artifacts` (iteration 328) is BLOCKED after one revision and one re-quorum. `gemini-3-1-pro` passed both rounds; `oc-glm-5-2`'s objection is CLOSED (the judge invoked the CLI and the three serve-api flags are real and work). The single live objection is `gpt5-6-sol`, rejecting twice on the same point: SHA-256 beside the blobs proves CONSISTENCY, not that the bytes came from our compiler, so the doc's "compiler-origin" trust assumption is not enforced by key, stamp or hash. **Options.** (a) RULE THE THREAT MODEL SUFFICIENT — accept accidental-corruption scope, unblock, the doc goes straight to the planner next iteration, and adversarial hardening is filed as its own backlog row. (b) REQUIRE ADVERSARIAL HARDENING NOW — the doc gains a decode-bounding section and a third quorum round, costing roughly one extra iteration before any fix ships. (c) SPLIT — land the correctness fix now under (a) and open a separate hardening doc immediately. **Loop's recommendation: (a).** The live user-facing defect is silent wrong-program execution; adversarial cache hardening is a separate problem that HEAD is already strictly worse at, and blocking a strict improvement on it leaves users exposed to the confirmed bug meanwhile. **Default if unanswered: (a), applied at the next iteration** and recorded as a controller routing call rather than as a ruling, with the hardening row filed alongside it.  **ANSWERED — C: ship the accidental-corruption correctness fix and open a separate adversarial-hardening design immediately. Explicitly remove compiler-origin guarantees: a co-located hash proves consistency, not trusted provenance. Proceed through planning and normal implementation/evaluation gates.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Measured first-party, iteration 328, 2026-09-05, both directions. PRE-EXISTING and REDUCED by the doc: at HEAD `internal/pipeline/cache_store.go`'s `LoadArtifacts` reads unbounded and gob-decodes with **0** byte ceilings and **0** hash checks in the whole file (known-positive control `os.ReadFile` = 5 hits in that file), while the doc adds a 16 MiB per-blob ceiling AND hash-verification before any decode. Judge-verified independently: Go's `encoding/gob` already caps preallocation against remaining input, so a 16 MiB-capped blob cannot be amplified arbitrarily. AGAINST: a poisoned cache blob is decoded on every routine `ailang run` **without the victim knowingly compiling anything**, unlike a malicious `.ail` file — though the cache sits in the user's own project directory, where anyone able to write it can already edit the sources the compiler reads. Quorum artifacts `.ailang/state/mission-quorum/m-compile-cache-unverified-artifacts-2026-09-05T11-31-14Z.json` (round 1) and `-2026-09-05T11-43-52Z.json` (round 2). Defect reported at [#1046](https://github.com/sunholo-data/ailang/issues/1046). Evaluator `sonnet` PASS 92/100 concurred that PARK was the correct disposition, on the ground that this objection needs controller judgement and carries no verbatim fix.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-56 | RESOLVED | **`gpt-6-astra` is BOTH a designer-rotation entry and a quorum reviewer, so on its turn it judges the doc it just wrote — which rule should the loop follow permanently?** Added by Mark (attended 2026-09-05) as a third rotation entry, and on the same day it straight-swapped `gpt5-6-sol` out of the design-quorum roster (`cmd/ailang/design_quorum.go`). The rotation pointer now sits at `claude:claude-fable-5-1`, so **the NEXT iteration's designer is astra** and the collision is live rather than hypothetical. This is the strong, model-level form of the self-marking defect, not the weaker vendor-level one, and your stated principle is *"ideally no model provider marks its own work"*. The mission-control skill records an interim workaround (author with the next rotation entry, or substitute `gpt5-6-sol` back into the OpenAI seat for that doc only) and a PROPOSED-not-ratified fix. **Options.** (a) RATIFY THE PROPOSED FIX — the OpenAI quorum seat becomes *the OpenAI model that did not author this doc*: astra by default, `gpt5-6-sol` when astra designed it. Keeps three independent vendors AND every reviewer independent of the author; costs one small change to the reviewer-selection code. (b) DROP ASTRA FROM THE ROTATION, keep it as a reviewer — simplest, but it undoes your 09-05 decision to vary between two fable-class authoring lanes. (c) DROP ASTRA FROM THE QUORUM, keep it as a designer — restores `gpt5-6-sol` as the permanent OpenAI reviewer; also simple, but you swapped it out deliberately. (d) ACCEPT THE COLLISION and let the loop note it each time. **Loop's recommendation: (a).** It is the only option that keeps both of your 09-05 decisions intact. **Default if unanswered:** the loop keeps applying the skill's interim workaround — author with the next rotation entry on astra's turn and record the substitution — which is correct but silently costs astra its authoring slot every cycle.  **ANSWERED — Exclude the entire origin vendor of the artifact author from its independent judges, not just the exact model. Keep Astra in the designer rotation. Select replacement reviewers from other origin vendors explicitly; do not disguise the same origin behind OpenRouter. Preserve the required quorum size with independent vendors; unavailable independent capacity is a recorded wait, never an absent-judge pass. This supersedes the interim same-vendor substitution.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iteration 334, 2026-09-06. First-party: the mission-control roles table names astra as rotation entry 2 AND flags the quorum collision on the same line; `ailang design-quorum --help` reports the default reviewer set as `gpt6-astra,gemini-3-1-pro,oc-glm-5-2`; this iteration's own two quorum artifacts both list `gpt6-astra` as a reviewer. Rotation pointer `~/.ailang/state/mission-v1-designer-rotation` = `claude:claude-fable-5-1` after this iteration, i.e. astra is next.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-57 | RESOLVED | **Which cache module-directory naming direction should resume after the bounded re-quorum rejected the hybrid?** (a) RETAIN the 38-byte ASCII prefix plus 16-hex full-ID hash, explicitly accepting limited readability and collision resistance rather than injectivity; smallest correction, existing stamp mismatch rejects wrong-ID artifacts. (b) PURE HASH; remove the slug and choose digest width in the design (full SHA-256 means 64 hex characters), trading readable names for simplicity. (c) BASENAME/PARENT REDESIGN; specify portable component parsing and test representative Windows paths, requiring another design pass. **Recommendation: (a)** — the prefix is a limited hint, never an identity guarantee; GLM’s claim that a single separator produces two underscores is false under the measured byte algorithm. This counterevidence does not override its requested change of direction. **Default if unanswered: HOLD implementation immediately and indefinitely; no runtime snapshot copy.** A human ruling, completed design gate, and planner synchronization are all required before M1.  **ANSWERED — A: retain the 38-byte ASCII prefix plus 16-hex full-ID hash as a limited readability hint and collision-resistant name, never an injective identity. Retain stamp validation. Complete the design correction and synchronize the planner before implementation.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iteration336: both quorum rounds 2 rejects/1 pass, all three external reviewers present; raw artifacts in `docs/sprint-retros/iter336-cache-module-id-encoding-quorum/`. Sol’s final source-attribution objection answered with an eight-file exact-commit diff (zero changes); GLM’s remaining alternatives dispute the direction, so no narrow-refinement carve-out or third quorum. Independent evaluator reviews the park, not permission to override quorum. PR [#1061](https://github.com/sunholo-data/ailang/pull/1061).  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-58 | RESOLVED | **Pi runner content-delta contract:** (A) authorize one fresh designer revision and quorum to settle snapshot failure ordering, bounded comparison and physical artifact aliases; (B) keep this approach parked and commission a Git-native content-comparison design. Recommend A. Default: HOLD immediately until a human ruling; neither option skips design/planner/executor/evaluator gates.  **ANSWERED — A: authorize one fresh scoped designer revision and quorum covering snapshot failure ordering, bounded comparison and physical artifact aliases. Continue through normal planner/executor/evaluator gates; do not reuse the blocked design as approval.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iteration337, two complete quorum rounds rejected by all three external reviewers. Narrow-refinement considered but not used: remaining controller blockers require design judgment beyond verbatim reviewer fixes. See planned/v0_35_2/m-pi-runner-worktree-assertion-vacuous-on-revision.md and docs/sprint-retros/iter337-pi-runner-quorum-r2.json. Approval inbox_1788680529734_00877707 delivered byte-exact.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-59 | RESOLVED | **Pi shell-suite candidate at the three-round evaluator cap:** (A) authorize a fresh scoped iteration after the existing SMT #602 gate is green, repairing the one pre-fork temp-directory residue before a new independent evaluation sequence; (B) abandon/archive candidate `2eb17d026` and leave the coverage item unshipped. **Recommendation: A** — the shell contract independently passes focused19/19 and wiring37/37 with zero process survivors, while the final hard failure came from unchanged SMT code; the temp residue is concrete and small but still must be repaired under a new gate. **Default if unanswered: HOLD indefinitely; no push, PR, fourth round, or merge.**  **ANSWERED — A: authorize a fresh scoped iteration after the SMT #602 prerequisite is demonstrably green. Repair the pre-fork temporary-directory residue and obtain a fresh independent evaluation sequence. No fourth evaluation round on the old sequence and no waiver of mandatory tests.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iteration339 independent scores FAIL83 → FAIL88 → FAIL70. The final judge observed mandatory `make test` nonzero on unchanged `TestSolve_HardTimeout_FakeSolverIgnoringT` (#602), then an isolated exact pass; evaluator skill has no waiver. It also observed one `run-test-suite.*` directory after injected pre-fork mkfifo failure. Candidate branch and independent verdict record are preserved.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-60 | RESOLVED | **How broad must the launchd notification recovery be?** (A) keep `m-launchd-notify-subshell-observation` test-only: repair the shell spy so its observation survives command substitution, restore the seven inherited notification assertions, and queue subprocess bounding separately; (B) expand this design before implementation so all three unbounded external paths are brought under the existing bounded-call discipline: direct `ailang messages send`, drain-time `ailang messages send`, and GitHub notification calls. **Recommendation: B** — bounded waits are a hard mission invariant, and the same source already contains `_mc_bounded`; fixing only the observation harness would green CI while leaving the production hang surface the quorum identified. **Default if unanswered: HOLD indefinitely.** No sprint plan, production edit, push, or merge may occur until a human rules, the designer revises the chosen scope, quorum passes, and planner/executor/evaluator gates run.  **ANSWERED — B: expand the design to bound direct message sends, drain-time message sends and GitHub notification calls using existing bounded-call discipline; repair the shell observation spies and all seven inherited notification assertions. Revise design, obtain independent quorum, then plan/execute/evaluate. Prefer this reliability repair early in the week.** (Mark Edmondson, attended 2026-09-07, recorded directly in this ledger.)| Iteration344 reproduced exact base `c308b2a0a`: pin suite 54/54, notification suite 20/27 with seven named failures. The immediate cause is test-only: `_out=$(ailang …)` runs the shell stub in a subshell, so its mutation of `TRACE` is unobservable; production still performs the send. Two complete quorums rejected the design with all three reviewers present. R2 Sol disputes direction and requires bounded direct-send, drain-send, and GitHub calls; Gemini and GLM require narrower fixture/exit-status evidence. The narrow-refinement carve-out cannot settle a direction dispute. Astra designed, Sol planner/executor failed closed, and independent gpt-5.5 evaluator scored the PARK disposition PASS92 (implementation score 0 because none was authorized). Configured `pi:ollama/minimax-m3:cloud` could not be selected by the Agent tool; the exact failure and fallback are recorded in iteration344.  **Attended ruling 2026-09-07** — recorded in-session under the ATTENDED LEDGER EDITS contract, not via the bookkeeping issue. Provenance is the ATTENDED SESSION, not the commit author: this script stamps a fixed attended identity for EVERY caller (ATT_NAME/ATT_EMAIL are defaults, not derived from the invoker), and nothing in scripts/mission_decisions.sh or the mission-control skill reads the commit author of a ledger resolution (verified 2026-09-04, positive-controlled). The control is the charter rule that the UNATTENDED loop may not resolve a row on its own behalf. Mark explicitly delegated these rulings in the attended 2026-09-07 session: "please make the rulings so we are all unblocked". Codex selected and recorded this scoped ruling under that delegation. Decisions authorize the stated next gates, not fabricated execution or evaluation results.|
| D-61 | RESOLVED | **RULED (A) — attended 2026-09-08 (Mark)** — the loop MAY return a mission pin to the `origin/dev` default, never pin it forward and never pick a new SHA, only after verifying the pinned SHA is an ancestor of `origin/dev`, recording the change in its record. Rationale as filed: returning to the documented default is the one pin edit that cannot select a worse tree, and (B) means a driver fix can sit inert indefinitely with the only symptom a log line reading `drift: 0`. **Scoped as a bridge, not a permanent grant**: when binary mission control owns iteration it owns pinning too, and this authority retires with the shell driver. **Executed the same session, attended**: all four `~/.config/ailang/mission-*.env` had `export AILANG_DRIVER_REF=48c4a6e49` and `AILANG_DRIVER_PIN_DIR=…-ollama-gauge-48c4a6e49` removed after `git merge-base --is-ancestor` confirmed the SHA was an ancestor of `origin/dev` and **100** commits stale; `AILANG_DRIVER_PIN=1` is unchanged, so pinning stays ON and only the frozen SHA is gone. Backups at `mission-<m>.env.pre-unpin-<stamp>`. Verified the driver recreates an absent pin worktree (`pin-root.sh` `else` branch) and that its onboarding gate passes, since the gate accepts when the SOURCE clone is onboarded and `~/dev/sunholo-data/ailang` reports `true`. **May this loop repoint `AILANG_DRIVER_REF` back to `origin/dev` itself, once a SHA-pinned attended deployment's fix has merged — or is every change to the fleet's `~/.config/ailang/mission-*.env` pins attended-only?** (A) the loop MAY return a mission's pin to the `origin/dev` default (never pin it forward, never pick a new SHA) once it has verified the pinned SHA is an ancestor of `origin/dev`, recording the change in its record; (B) attended-only, and the loop's disposition is to REPORT the drift each iteration and stop there. **Recommendation: A**, narrowly — returning to the documented default is the one pin edit that cannot pick a worse tree, and (B) means a driver fix this loop lands can sit inert for an unbounded time with the only symptom a log line reading `drift: 0`. | Filed iteration 349, first-party. All four missions carry `export AILANG_DRIVER_REF=48c4a6e4975632e1ac3c1452ebbb55e2de52c80f` (attended deployment stamp `20260907T151903`); `origin/dev` is 43 commits ahead, **3 of them touching `tools/launchd/`** including `e5a325a20` — M1/M2 of the sprint whose M3 iteration 349 just merged, so that merge is inert on the rig. `pin-root.sh` defines `PIN_DRIFT` as commits the clone is behind **the ref**, so a SHA pin that is an ancestor of the clone reports `0` forever: the driver logged `driver pin drift: 0 below warning threshold 25` on the same fire. Zero prior mentions of `AILANG_DRIVER_REF` in the charter or the whole log. Queue row `m-fleet-sha-pin-freezes-every-driver-fix`. |
| D-62 | RESOLVED | **RULED — attended 2026-09-08 (Mark): "I don't really get the obsession with making sure all steps need to be on different models/vendors. its a nice to have, not a blocker."** `generator != judge` is now PREFERRED at vendor level, REQUIRED at model level. A model may not judge its own output (self-review, refused hard). A different model from the same vendor is ALLOWED and FLAGGED (`Candidate.SameVendorAsAuthor` on the receipt), and cross-vendor candidates sort first so the property still holds whenever it can be had. | Forced by a conflict the skill fix exposed: the only harness that loads the `sprint-evaluator` skill is claude, so vendor-level exclusion made a claude-authored work item have NO routable evaluator at all. Measured 2026-09-08: the docs canary evaluator failed 3/3 on the cross-vendor pi lane precisely because it had no methodology, while every recorded fleet verdict (hundreds, NN/100) came from a claude lane. A same-vendor judge WITH its rubric beats a cross-vendor judge without one. Precedent: motoko already ran "generator != judge model-level only, and FLAGGED". |
| D-63 | RESOLVED | **RULED — attended 2026-09-08 (Mark): "we are retiring the no single provider role its a nice to have not a code red situation."** NO-SINGLE-PROVIDER-ROLE — every role must name at least two providers across its fallback chain — is RETIRED. Cross-provider breadth in a role chain is a preference, not a release gate. The exact-chain assertion pinning the evaluator to `pi:ollama/minimax-m3:cloud,pi:openrouter/minimax/minimax-m3,codex:gpt-6-astra` is removed from `tools/launchd/test_mission_routing.sh`; the evaluator keeps its live guards (D-62 codex-stays-last, and the anthropic role pre-flight). Do NOT reinstate an exact-chain grep — it re-freezes a routing decision that is meant to move. | The driver moved the evaluator to `claude:claude-sonnet-4-6,claude:claude-haiku-4-5,opus` and the assertion was never updated, so the `launchd drivers (bash 3.2)` CI job went red for EIGHT consecutive runs — including the v0.35.3 release itself — while guarding a policy nobody intended to keep. A gate that is red for a policy you have abandoned stops carrying information about the ones you keep: it masked two genuinely new failures (`test`, `test-windows`) introduced on `a097d7718` during the v0.35.4 stdlib work. Same reasoning as D-62 and ruled the same day. After removal: 83 passed, 0 failed. |
<!-- decision-ledger:end -->
...
## STATUS 2026-09-14 — ITERATION 354: **`PIN_AGE` landed — a pinned driver now reports how far its target is behind `origin/dev`; the red `dev` `test` context from a concurrent commit was fixed forward first; both pi executor links hit the 30-minute wall on plan-sized milestones and the controller finished the arms.** Gate 0/1: kill switch armed, billing CLEAN, gh `sunholo-voight-kampff`; `SKILL.md` + 11/12 resources byte-identical to origin in the resolved copy — `gate-1-observe.md` DIFFERS (origin carries iteration 353's `--allow-escape-sequences` rule that the main checkout lacks; the resolved copy was followed); **0 directives** on #1163 (new this week) and #1072; `dev` `9422ff628` green on every context. **Cross-mission**: World row 73 / [#1160](https://github.com/sunholo-data/ailang/issues/1160) ghost-disciplined REAL on V1 (`#1072` carries a self-authored rc=143 notice the directive read is blind to) → queued as `m-gate0-self-crash-notice-read` **[world-DEMAND]**, acknowledged on ailang-world#138 and #1160. **Pick** `m-pin-drift-blind-under-sha-pin` (top `[NEXT]`; measured first-party: `HEAD..48c4a6e49` = 0 while that pin is 266 behind `origin/dev`). **Designer** `codex:gpt-6-astra` (rotation entry 2; probe rc=0; 427 lines, 17 verification rows) — astra recused from its own quorum, `gpt5-6-sol` substituted: **r1 BLOCKED** (sol: mutable `origin/dev` in the measurement — premise measured REAL, three other jobs fetch the shared clone; glm: `AILANG_DRIVER_REF` export unverified — premise measured FALSE at `pin-root.sh:322-323`; gemini pass), one protocol-mandated revision, **r2 BLOCKED 3/3 zero absentees** on the r2 test hook the controller's own directive had suggested (sol/glm: delete it, use a lab PATH `git` shim; gemini: V20) → all three fixes concrete, none disputing direction → carve-out r3. **Planner** `pi:ollama/kimi-k3:cloud` (resolver `agent-tool opus fail-closed:unparsable-path-entry`; pin followed per §2) verdict `ok` 960 s / 42 tools — kimi's **fourth consecutive** successful planner run; 14 re-verification rows, D-1/D-2/D-3 recorded. **Executor** `pi:ollama/deepseek-v4-flash:0731-cloud` M1 **`wall_timeout`** rc=13 at 1810 s with T1–T11 landed — measured outside the sandbox: pin suite **117/0** (all 35 plan-named lines + one split sibling), notify **50/0**, the executor's own "every tool returned empty" claim REFUTED by its NDJSON (3 of 172 results zero-length); nine-mutant drill run by the controller, all killed. Fallback `pi:openrouter/deepseek/deepseek-v4-flash-0731` M2 **`wall_timeout`** at 1810 s with all 31 arms written unrun (71/10 as delivered; every failing trace showed CORRECT production behaviour) → end-of-chain opus spawn is hook-denied → controller repaired 10 assertion needles (state-line ordering, title-on-the-AILANG-line, `arm_ok`'s single required arg) to **81/0**, ran the 13-mutant drill: **MUT-R SURVIVED** (prev=170 + age 30 → dedupe hid a deleted status guard) → ROW repaired (absent previous state) → killed. **Mid-iteration RED on `dev`**: `7423434b4` (concurrent direct-to-dev, 12:21) added `pi-or-minimax-m3` at 32000 without a `ceilingLimited` entry → REQUIRED `test` red on three commits → fixed forward as [#1165](https://github.com/sunholo-data/ailang/pull/1165) → `fd44be9ee` (22 checks zero not-green), sprint rebased (zero file overlap). **Shipped** PR [#1166](https://github.com/sunholo-data/ailang/pull/1166) → [`266cf23a2`](https://github.com/sunholo-data/ailang/commit/266cf23a2ea6c7247243ed9a687226639897b9e4), 22 checks on the PR head, ZERO not-green. **Independent judge** `agent-tool sonnet`, own worktree: **r1 PASS 96/100, zero blocking** — 7 of 22 mutants re-drilled matching the audit exactly, two diff-anchored mutants of its own, one SURVIVED (notice prose unasserted) → reproduced, row added, mutant now red; **r2 PASS 100/100**, all three r1 findings dispositioned. **Ruled out:** the executor's tool-output failure claim; the designer's r2 test hook (my suggestion); "the notify reds are production defects" (10/10 traces correct). Queued: `m-gate0-self-crash-notice-read` [world-DEMAND], `m-codex-quota-admission-test-reads-live-ledger` (pre-existing rig-only red, reads the live quota ledger), and the process row on the pi 30-minute cap (third instance → Gate-5 skill edit). N=12, goal unmoved (HARNESS). Metered **$0.29** of $5 (two quorum rounds $0.20 + OpenRouter M2 $0.09). Ledger 63 rows, **ZERO open**. Full record: v1-mission-log.md iteration 354.

## STATUS 2026-09-14 — ITERATION 353: **First fire in six days (kill switch lifted); landed the macOS flake fix — the capture helper, not the assertions, was the defect — credited orphaned iteration 352, and acknowledged three attended rulings.** Gate 0/1: kill switch armed (cleared between the 06:28 and 07:58 fires), billing CLEAN, gh `sunholo-voight-kampff`, all 13 skill files byte-identical to origin in BOTH the resolved-symlink copy and the pin copy; **0 directives** on #1072 (15 comments); `dev` at `05a6457d1` green on CI/Build/Docs with the standing SonarCloud red inherited (walked back over 3 parents). **Three ATTENDED rulings acknowledged** (2026-09-08, Mark): D-61 (A) — pins returned to `origin/dev` in all four env files the same session, so `m-fleet-sha-pin-freezes-every-driver-fix` is RESOLVED by attended action and its instrument residue re-queued as `m-pin-drift-blind-under-sha-pin`; D-62 generator≠judge PREFERRED at vendor level, REQUIRED at model level; D-63 NO-SINGLE-PROVIDER-ROLE retired. Ledger 63 rows, **ZERO open**. **Gate 2 found an ORPHANED ITERATION 352** (2026-09-08 06:00–11:33Z): it ran the whole inner loop on `m-daemon-task-exec-run-untested`, merged PR [#1113](https://github.com/sunholo-data/ailang/pull/1113) → [`45f02deb3`](https://github.com/sunholo-data/ailang/commit/45f02deb3) with the four required contexts green and a banked round-1 judge PASS 85/100 (two surviving mutants closed in `25d317650`; no round-2 artifact exists), then left zero charter rows, zero log entries and no slot verdict — credited here, docs moved to `implemented/v0_35_3/` with iteration 351's. **Pick: `m-debugcacheforms-flaky-on-macos-ci`** (the TOP-OF-QUEUE row; a red V1 owns, actively costing re-runs — 4 of the last 25 `Build and Release` runs on `dev`, all on docs-only or tools-only commits, plus 352's own merge SHA). **Gate 2 measured the mechanism before routing**: `capturePipelineStderr` closed the pipe's READ end before waiting for the copier goroutine (`r.Close(); <-done`), so a late-scheduled reader lost the tail — a standalone probe loses bytes **50/50** under that ordering and **0/50** with the wait first, and the raw job logs show exactly the shapes it predicts (three EMPTY warm captures at `:245`, one first-line-only cold capture at `:232`). The row's own hypotheses were refuted: the assertion is a prefix (no `cached 0s ago` timing), and a structural-counters oracle would read through the same truncated capture. Shipped as PR [#1161](https://github.com/sunholo-data/ailang/pull/1161) → [`3faaf47fa`](https://github.com/sunholo-data/ailang/commit/3faaf47fa06549c94eac95432a6933ee11df4cf6), **22 checks on the PR head, ZERO not-green**: a panic-safe named-return `defer` teardown (restore → close writer → drain-wait hook → bounded 10 s wait on the copier's completion channel → close reader → panic on a non-nil copy error), four helper tests pinned with a channel-gated reader (no sleeps), a six-mutant drill. **The sharpest measurement came from the drill:** with the old ordering restored and the copy error propagated, the production test is red LOCALLY on the first run — the read-on-closed-fd error was happening every run here too and being swallowed. **Designer** `claude:claude-fable-5-1` via `claude-sub` (rotation pointer advanced; probe rc=0; one authoring run + the one protocol-mandated revision — diet intact). Its first draft REFUTED my frequency claim ("only 1 of 6 failed macOS jobs is this test; the rest are an external cancellation mode") and the refutation was itself WRONG: `gh run view --log` errors on logs carrying terminal escape sequences and its empty grep was read as absence; re-derived from the raw job logs with `--allow-escape-sequences`, 4 of 4 failed jobs are this test and the `cancelled` legs are `fail-fast` siblings. The designer correctly refuted MY "14 test files call HangGuard" (14 lines, 1 external file — a 3b(ix) scope error). **Quorum r1 BLOCKED** (gemini reject: panic-safe named-return defer; astra ABSENT on budget → re-run alone at $0.40 → reject: a sleep-based slow reader cannot make MUT-1 deterministic, use a channel-gated reader + drain-wait hook and bound the wait; glm pass), **r2 BLOCKED 3/3 zero absentees** (astra: propagate `io.Copy`'s error, no silent truncation; gemini: a goroutine holding `os.Stderr` does NOT delay EOF, only an inherited fd does; glm: show the cold assertion) — every fix concrete, none disputing the direction → applied verbatim under the narrow-refinement carve-out (r3). **Planner** `pi:ollama/kimi-k3:cloud` (resolver said `agent-tool opus fail-closed:path-not-in-codex-allowlist`; the `provider:model` pin was followed per role-spawn-routing §2) verdict `ok` 709 s / 39 tools — kimi's **third consecutive** successful planner run; it found two doc/plan disagreements (four tests not three; the `## [Unreleased]` changelog section had been consumed by the v0.38.5 release) and its D-2 resolution (`## [v0.38.6]` header) was corrected to the repo's real convention. **Executor** `pi:ollama/deepseek-v4-flash:0731-cloud` verdict `ok` 802 s / 110 tools, non-empty diff — deepseek's **third consecutive** `ok`; it self-reported that its first gated reader wrapped a `strings.Reader` and would have made MUT-1 spuriously green, and fixed it before drilling. Every gate re-run by me outside the sandbox (AC1/AC2 four `--- PASS` lines/AC4 `-race`/AC5/gofmt/whole package `-race`/`unused` lint: all rc=0; MUT-1 ×5 rc=1 with `copier failed: read |0: file already closed`, landed/vet-clean/restored byte-identical). **Independent judge** `agent-tool sonnet`, own worktree: **r1 PASS 88/100, zero blocking**, five non-blocking findings each reproduced first-party before acting — one real: the post-loop `os.Stderr == nil` check could not fail (restore-skip mutant rc=0 → fixed to compare with the original, rc=1); **r2 on the r3 delta PASS 98/100, all five CLOSED**. Weekly sweep (rotation week): **7 orphans of 85 enumerated** (`gh` count control 85 = 85; positive control `#1111` fires at 4; negative control fired) — 5 daneel-filed issues (4 already have design docs landed 2026-09-13), #1096 (pi runner `empty_worktree` on a committing executor), and #1089 (the docs mission's own bookkeeping thread, not an orphan) — batched as `m-weekly-sweep-orphans-2026-09-14`. N=12, goal unmoved (HARNESS). Metered **$0.45** of $5 (two quorum rounds + one raised-cap astra re-run); pi lanes flat-rate. Ledger 63 rows, **ZERO open**. Full record: v1-mission-log.md iteration 353.

## STATUS 2026-09-08 — ITERATION 351: **Inherited iteration 350's orphaned design doc, landed it through planner/executor/judge — and the judge's round-1 FAIL was right: the sprint had quietly made production retry backoff uncancellable.** Iteration 350 CLAIMed `m-coordinator-test-parallelism` at 01:14Z and died mid-flight, leaving **zero** charter rows, zero log entries, no PR and no remote branch — only a worktree in **this clone's** list (Gate 2 trace (b), ownership proof `--author` cannot give on a shared push identity) carrying a 416-line **r3** design doc. That doc's quorum round 2 was **3/3 REJECT with zero absentees**, and all three verbatim fixes were already applied under the narrow-refinement carve-out; I re-read the three objections and confirmed none disputes the design DIRECTION, so the carve-out was legitimately spent and the doc needed no fourth round and no designer. **Verified before adopting** (V2 `t.Parallel()` **0** in the package against a repo-wide control of **17**; V11/V13/V15 the three hardcoded timers at the named lines; V16 `NewStoreBackedApprovalCheckpoint` **0** production callers; V20 `IsThrottled()` present; and the headline timing re-measured first-party — four tests **8.13 s** of a **12.65 s** summed total). Shipped as PR [#1111](https://github.com/sunholo-data/ailang/pull/1111) → [`5f95a3814`](https://github.com/sunholo-data/ailang/commit/5f95a3814399527c081b0b6b2109f3bc614fc0a7), **21 checks on the PR head with ZERO not-green** (SonarCloud included, after it caught a coverage gap): three timer seams as per-call config — `ExecuteOptions.RetryBaseDelay` + a cancellable `Wait`, `pollInterval`/`tick` constructor params, a `WithClock` option — with **no production default changed** and each set explicitly rather than by a zero-value fallback. Measured by me outside the executor sandbox: four tests **8.13 s → 0.23 s**, summed package time **12.65 s → 4.65 s**, wall **16 s → 6 s**. **The iteration's real value was the judge.** Round 1 `sonnet` **FAIL** on one blocking finding I reproduced first-party from the diff before acting: the seam had replaced `select { case <-time.After(delay): case <-ctx.Done(): … }` with an uncancellable `opts.Wait(delay)` plus a **non-blocking** ctx check, so a context cancelled 20 ms into a 1 s backoff went unnoticed for the full second (judge measured ~45x, and later measured pre-sprint **22.1 ms** / round-1 **1.001 s** / round-2 **22.077 ms** — exactly restored). It contradicted the design's own Non-Goal 4, the sprint plan had flagged it *"noted, not decided"*, and it shipped decided-by-default with zero coverage. The judge also found **one surviving mutant**: doubling `DefaultExecuteOptions().RetryBaseDelay` passed the ENTIRE suite, because injection guards assert an *injected* value is honoured — which stays true when the *default* moves — and the doc's grep backstop was provably broken (`RetryBaseDelay:.*time.Second` matches `2 * time.Second`; loose **1** / precise **0** on the mutant, precise **1** clean). Round 2 **PASS 97/100, zero blocking**. **And round 2's finding was about MY OWN code**: the defaults test I wrote to close the survivor claimed to pin three defaults and pinned one-and-a-half — the judge proved the poll sub-check vacuous by swapping both the constructor argument and the expectation to `999*time.Hour` and still passing. Narrowed the claim rather than widening the code: the vacuous assertion is deleted, the docstring now states exactly what is and is not pinned, and the two survivors are re-proven RED against compiling mutants with each file restored byte-identical. **Ruled out:** *"mission-motoko's cross-mission warning that Gate 4's STATUS rotation will red CI on V1's next fire"* — **REFUTED for V1**, measured: all three of V1's live stamps already carry the canonical colon and `TestMissionDocHeadingsStayCanonical` is rc=0 at exactly the ratchet; replied to motoko with the downward-arm warning. Also **NOT picked**: the physically top row `m-fleet-sha-pin-freezes-every-driver-fix` is blocked on **D-61**, which is still the ledger's one open row. **Queued from this iteration:** `m-ratelimit-window-default-unpinned`, `m-approval-poll-production-defaults-unexercised` and `m-daemon-task-exec-run-untested` — the last found by SonarCloud's new-code coverage gate (78.8% vs 80) with a firing negative control, and it carries the admission that the sprint's FIX 2 production-caller rebase is verified by code reading only, with no test executing it. `m-ratelimit-window-default-unpinned` (pre-existing, the design doc's own O2c, confirmed by the judge mutating the window to 500 ms with the whole package staying green). Designer NOT spawned (r3 doc inherited, complete); planner `pi:ollama/kimi-k3:cloud` verdict `ok` 547 s / 35 tool executions — kimi's **second consecutive** successful planner run; executor `pi:ollama/deepseek-v4-flash:0731-cloud` verdict `ok` twice (567 s / 110 tools, then 233 s / 32 tools) — deepseek's **second consecutive** `ok` with a non-empty diff, which meets the promotion bar; evaluator `agent-tool sonnet` two rounds, 202,569 tok / 25 tool uses in round 2, in its own worktree. N=12, goal unmoved (HARNESS). Metered **$0.00** of $5 (both pi lanes are flat-rate ollama-cloud). Ledger 61 rows, **ONE open (D-61)**. Full record: v1-mission-log.md iteration 351.
...
**[LANDED 2026-09-07 (iter-345) — PR [#1074](https://github.com/sunholo-data/ailang/pull/1074) → [`16f0cb741`](https://github.com/sunholo-data/ailang/commit/16f0cb74103ec903c8c596e69f775c96e5674f71); judge `sonnet` PASS 91/100, zero blocking; 20 checks on the merge SHA with only the parked D-60 red left] `ci-red-execgo-file-size` — the REQUIRED `test` context was red on `dev`, so every open PR in the repo was blocked, because an unrelated attended checkpoint pushed one file 7 lines over the size gate.**
Measured first-party at iteration 345 before routing: `make check-file-sizes` fails on `cmd/ailang/exec.go` at **807** lines against the 800 ceiling (`make/code-health.mk:166`). Attribution is unambiguous and is NOT a sprint of ours: `8c41d41d4` ("chore: checkpoint existing z.ai provider and benchmark work"), an attended direct-to-dev commit, changed that file by **10 insertions / 1 deletion**, taking it 798 → 807. Fix is a pure move — `spanningEventHandler` and its streaming callbacks byte-for-byte into `cmd/ailang/exec_events.go`, exec.go 807 → **614**, new file **209**, the exec.go diff **193 deletions and 0 additions**, import block untouched. Purity proven with a comparator carrying its own negative control (a one-line-offset region correctly fails to match). **Handed over by `mission-motoko` iteration 38**, which found both of dev's reds and correctly routed them to V1 as the repo-owning mission rather than duplicating the work. **The second red was deliberately not picked**: `launchd drivers (bash 3.2)` is parked on `D-60` by iteration 344 with [#1073](https://github.com/sunholo-data/ailang/pull/1073) open, and it is proven inherited — `20 passed, 7 failed` identical at the PR head and at the base, with zero `tools/launchd/` files in this diff.
**This is the SECOND instance of a class, and the recurrence is the finding, not the fix.** Iteration 332 recorded the same shape: an attended session's direct-to-dev commit (`19d6b03c7`) reddened the REQUIRED `test` context and blocked every PR in the repo, motoko's fix included, and V1 fixed it forward. Both times the commit was a checkpoint rather than a sprint, both times the gate that caught it is one `make` target that runs locally in under two seconds, and both times a mission loop spent an iteration on it. Two instances is below the ≥3-evidence bar for a policy change, so this row records the measurement rather than proposing a guard; a third makes the fix a **pre-push hook or a local `make ci-lite`**, not another iteration.

**[LANDED (M1+M2 of 4) 2026-09-07 (iter-347) — PR [#1090](https://github.com/sunholo-data/ailang/pull/1090) → [`e5a325a20`](https://github.com/sunholo-data/ailang/commit/e5a325a207edbaec0dc5e69e31df5f6519afd9bb); **21 checks on the exact merge SHA, ZERO not-green**, `launchd drivers (bash 3.2)` among them; judge `sonnet` LAND 80/100, both BLOCKING findings reproduced first-party and fixed in-iteration] `m-launchd-notify-subshell-observation` — UNPARKED by human ruling D-60 (option B) and shipped.**
D-60 rejected the test-only scope. The design was revised to fold production bounding in, re-quorumed twice (r1 BLOCKED, r2 `gemini-3-1-pro` PASS with two completeness rejects), and closed under the narrow-refinement carve-out applying both reviewers' verbatim fixes. **Shipped:** the fixture's variable spy becomes a file-backed one on a UNIFIED `MC_TRACE_FILE` medium with PATH-executable `ailang`/`gh` stubs — required in any case, because `_mc_bounded` runs `( exec "$@" )` and **`exec` bypasses shell functions** (measured: `f() { echo FUNCTION-CALLED; }; ( exec f )` → `exec: f: not found`, direct call prints `FUNCTION-CALLED`) — and the three D-60 paths (`:154` drain send, `:184` direct send, `:198` GH notice) each run through `_mc_bounded` at `NOTIFY_TIMEOUT` (env `MISSION_NOTIFY_TIMEOUT`, default 30s). `bash tools/launchd/test_driver_notify.sh`: **20 passed/7 failed → 38 passed/0 failed**, rc=0.
**Bounding would have silently deleted a diagnostic, and that is fixed too:** a cut-off command produces no output, so `MC_BOUNDED_OUT` is empty and the failure WARNING degraded to `FAILED … after 3 attempts:` with nothing after it — the blindness `_mc_notify`'s own comment exists to prevent. Synthesis is now gated on an empty tail with any non-zero rc, so `mktemp` failure (`rc=125`) is covered too.
**NOT delivered — M3 (aggregate drain budget `DRAIN_BUDGET`) and M4 (mutation sweep):** the executor hit its 30-minute cap mid-M3 (`wall_timeout`, rc=13). The partial failed its own two new tests and was discarded, preserved at `~/.ailang/state/mission-v1-iter347-m3-partial/`, and queued below. The plan's own Budget section named M1+M2 as the cut set.

**[CLOSED ON ARRIVAL 2026-09-07 (iter-347) — the defect was REAL at this iteration's base and was fixed on `dev`, by someone else, WHILE the iteration ran] `m-launchd-heartbeat-arms-red-behind-the-notify-red` — filed and closed in the same iteration; the CLASS is the keeper, the row is not.**
**What was measured, and it was true:** at base `81abc956d`, `make test-launchd-drivers` halts at its first failing suite, so while `test_driver_notify.sh` was red **nothing behind it ran**. With that red cleared the target reached three further suites — two passing (**84/0**, **17/0**, never executed on `dev` before) and `test_mission_heartbeat.sh` failing **9 arms**, identical at base and head by paired control (symptoms: `test_mission_heartbeat.sh: line 12: [: : integer expression expected` and `slot-verdict.sh: line 24: MC_PAUSED: unbound variable`).
**Why it is closed:** [`063df9917`](https://github.com/sunholo-data/ailang/commit/063df9917) ("test(launchd): supply pause state to heartbeat fixture"), inside attended PR [#1082](https://github.com/sunholo-data/ailang/pull/1082) merged at **14:15Z**, repaired exactly those arms. Verified first-party on `dev` afterwards: `test_mission_heartbeat.sh` alone → **25 arms ran, rc=0**, and the whole `make test-launchd-drivers` target → **rc=0**. Nothing is owed.
**KEEP THE CLASS, IT IS GENERAL AND IT COST THIS ITERATION A WRONG RECORD.** A red that fails EARLY in a long ordered `make` target SUSPENDS every gate behind it, so **the check set reports one failure where there may be several, and any such report is a LOWER BOUND**. Two consequences the loop should carry: (a) after clearing any red in an ordered target, re-run the WHOLE target rather than the suite you fixed — the next red is the one you have never seen; (b) a measurement of "what else is broken" taken at a stale base has a shelf life, because the thing you are measuring is precisely what other people are also fixing.

**[LANDED 2026-09-07 (iter-348) — PR [#1102](https://github.com/sunholo-data/ailang/pull/1102) → [`81fb19b67`](https://github.com/sunholo-data/ailang/commit/81fb19b67bbc306f3a518b672a7782fc1cd48345); **20 checks on the merge SHA, ZERO not-green**; judge `sonnet` FAIL 35 (r1) → FAIL 61 (r2) → **PASS 86** (r3). The measurement CORRECTED this row: the slowest Windows package is `cmd/ailang` (228.7s = 76% of the old ceiling), not the coordinator, and the failing run was a 1.80x whole-runner slowdown, so the old 300s sat INSIDE the noise band (172.5 x 1.80 = 310.5s). Shipped a PROVISIONAL derived 416s with its arithmetic in the workflow, and a WARN-ONLY `tools/ci/headroom` whose one non-zero exit is a runtime anti-vacuity guard. Reducing `internal/coordinator` itself is out of scope and queued.] `m-coordinator-windows-package-timeout-headroom` — `internal/coordinator` blew the whole-package `-timeout 300s` on Windows while normally finishing in ~100 s, and its baseline has grown 40% in four commits.**
Measured at `72f9cfeca` (`test-windows`, `FAIL github.com/sunholo-data/ailang/internal/coordinator 300.520s`). The panic names the budget, not a hang: *"panic: test timed out after 5m0s / running tests: `TestFinalize_SupersededIsRecordedNotRetried` (2s)"* — the test on the stack had been running **2 seconds**, so the package exhausted its binary-wide budget cumulatively rather than wedging on one arm. **The diff cannot be the cause:** `72f9cfeca` changes four markdown files and nothing else (`gate-3b-ci-green.md` plus three mission docs).
**Trend, measured across four consecutive commits, and this is the part worth acting on** — same job, same package, all `ok`: `81abc956d` **88.7 s** → `8e3927950` **100.8 s** → `98730db02` **99.4 s** → `e5a325a20` **124.4 s**. So the steady state is ~100–125 s against a 300 s ceiling, and today's run was a ~2.5× outlier on top of a baseline that grew **+40%** across the window. `8e3927950` is the `#1082` mission-runtime merge, which added durable attempts, leases and dispatch to this package — including the very test on today's stack.
**This is NOT `m-message-watcher-windows-wallclock-flake`** (landed iter-318): that was a single assertion on an absolute wall-clock bound; this is the package's total budget. Do not "fix" it by raising `-timeout` — rule 3n's intermittent-kill clause: making a flaky ceiling pass more often is the same defect with a longer mean time to discovery. Measure where the ~100 s goes first (`go test -v` per-test timings on the Windows runner; the package opens real SQLite stores), then either parallelise, shrink the fixtures, or split the package. `test-windows` is not one of the four required contexts, so it blocks nothing — which is exactly why it will be allowed to sit red.
**Divergence control: MEASURED, and it says OUTLIER.** `gh run rerun --failed` was refused (*"This workflow is already running"*), but this row's own PR supplies a better control than a re-run would have: a tree differing from `72f9cfeca` by **one markdown file** ran the same job to **`success`** with `internal/coordinator` at **86.470 s** — the fastest of the six readings, on the commit immediately after the 300 s failure. Outcome moved with only the environment varying, so the variable is the environment, exactly as iteration 318 established for the earlier Windows flake. Full series: `81abc956d` 88.7 · `8e3927950` 100.8 · `98730db02` 99.4 · `e5a325a20` 124.4 · `72f9cfeca` **TIMEOUT >300** · this PR **86.5**.
**So the failure is an outlier, and the row survives anyway** — because a package whose honest spread is 86–124 s can reach 300 s at all means the headroom is a fact about the runner, not about the code, and no amount of green will tell you when the next one lands. The actionable half is the trend and the ceiling, not this incident: measure where the ~100 s goes, and decide whether 300 s is the right budget for a package that opens real SQLite stores on the slowest runner in the matrix.

**[RESOLVED BY ATTENDED ACTION 2026-09-08 (D-61 ruled (A) and EXECUTED the same session: all four `~/.config/ailang/mission-*.env` had the SHA pin removed after `merge-base --is-ancestor` confirmed it 100 commits stale; recorded by iter-353, which verified first-party that every env file now carries only the `origin/dev` default comment and that this fire logged `driver pin drift: 15` — the instrument reads correctly again once the ref is a branch). The residual instrument gap — `PIN_DRIFT` reads `0` forever under any SHA pin that is an ancestor of the clone — is re-queued below as `m-pin-drift-blind-under-sha-pin`.] [iter-349, first-party; zero prior mentions of `AILANG_DRIVER_REF` in the charter OR the whole log, so this mechanism has never been in the mission's memory] `m-fleet-sha-pin-freezes-every-driver-fix` — all four missions are pinned to a fixed driver SHA by attended deployment, the pin has not moved in 43 commits, and `PIN_DRIFT` reads `0` by construction because it measures the clone against the pin rather than against `origin/dev`.**
Measured at iteration 349. `~/.config/ailang/mission-{v1,docs,world,motoko}.env` all carry `export AILANG_DRIVER_REF=48c4a6e4975632e1ac3c1452ebbb55e2de52c80f` with `AILANG_DRIVER_PIN_DIR=~/.ailang-driver-pin/<mission>-ollama-gauge-48c4a6e49`, stamped *"Attended Codex quota deployment 20260907T151903"*. `origin/dev` is `b12ee3370`, **43 commits ahead** of that SHA. **Three of those 43 commits change `tools/launchd/`** — `e5a325a20` (iteration 347's D-60 notification bounding, i.e. **M1/M2 of the very sprint whose M3 landed this iteration**), `063df9917` and `2a38f0948` — so the driver executing on the rig right now does not contain them, and the M3 this iteration merged will not run either until the pin moves. Four more change `.claude/skills/mission-control/`. **The instrument cannot report it:** `pin-root.sh`'s contract defines `PIN_DRIFT` as *"how many commits the source clone is behind the ref"*, and under a SHA pin that is an ancestor of the clone, the answer is `0` forever — the driver logged `driver pin drift: 0 below warning threshold 25; notice re-armed` on this very fire while 43 commits of drift existed. This is the self-refuting-staleness-report shape the Repo Profile already names for `MISSION_WORKDIR`, arriving through `AILANG_DRIVER_REF` instead. **Not a one-off:** the immediately preceding backup (`mission-v1.env.pre-ollama-quota-20260907T151903`) shows the previous deployment pinned to `8e5899aff`, so the pattern is pin-forward-per-deployment and never return to `origin/dev`. **Blocked on D-61** — the loop may not repoint the fleet's env files unilaterally.

**Secondary, same measurement, and it is the reason this iteration nearly followed stale rules:** `SKILL.md` resolves through `~/.claude/skills/mission-control` (a symlink into the MAIN checkout, byte-identical to `origin/dev` here), but the twelve `resources/*.md` files it tells you to read are named by RELATIVE path and therefore resolve against the **pin worktree**. Measured: `resources/gate-3-route.md` is 75,448 B in the pin against 78,390 B running, and `resources/gate-3b-ci-green.md` 30,155 B against 33,494 B — while `SKILL.md` itself `cmp`s clean, so the Gate-1 staleness check greens. Ten of twelve matched, which is what makes it quiet. See the Gate-5 skill edit this iteration landed.

**[LANDED 2026-09-08 (iter-351) — PR [#1111](https://github.com/sunholo-data/ailang/pull/1111) → [`5f95a3814`](https://github.com/sunholo-data/ailang/commit/5f95a3814399527c081b0b6b2109f3bc614fc0a7); design authored by iter-350 before it died mid-flight; independent judge `sonnet` round 1 **FAIL** on a real blocking finding (production retry backoff made uncancellable), round 2 **PASS 97/100** zero blocking. Four timer-bound tests **8.13 s → 0.23 s**, summed package time **12.65 s → 4.65 s**, wall **16 s → 6 s**, measured by the controller outside the executor sandbox. **No `t.Parallel()` was added** — the title is a misnomer the design corrected: the win is not sleeping, not concurrency.] [iter-348, from the design's own out-of-scope section, with the evidence] `m-coordinator-test-parallelism` — `internal/coordinator` runs 732 top-level tests with **zero** `t.Parallel()`, and it opens real SQLite stores and spawns processes on the slowest runner in the matrix.**
Measured at `1b8e7eab1`: `ok internal/coordinator 13.072s` locally with the sum of per-test durations at 12.35 s, i.e. effectively serial; `grep -rho 't\.Parallel()' internal/coordinator/*_test.go | wc -l` = **0** against a known-positive control of **17** repo-wide under `internal/`. The package holds 68 `t.TempDir()`, 5 `os.MkdirTemp`, 9 store-open call sites and 13 `exec.Command`, and four timer-bound tests account for 8.14 s of the local 13.16 s. On Windows the same package runs 88–124 s in steady state. **This is explicitly NOT the package that blows next** — `cmd/ailang` is — so it is queued on its own merits and not as a fix for the timeout: iteration 348 measured that speeding the coordinator up would not have prevented the failing run. Start with the four timer-bound tests, which are constant across platforms.

**[LANDED 2026-09-08 (iter-352 — an ORPHANED slot: it ran designer/planner/executor/judge, merged PR [#1113](https://github.com/sunholo-data/ailang/pull/1113) → [`45f02deb3`](https://github.com/sunholo-data/ailang/commit/45f02deb3) at 11:33Z with `test`/`lint`/`build`/`docs-gate` all green and a banked round-1 judge `sonnet` PASS 85/100, closed the judge's two surviving mutants in `25d317650`, and then left zero charter rows, zero log entries and no slot verdict — credited and recorded by iter-353, which verified the merge SHA's required contexts and the banked evaluation first-party; the macOS leg on that merge SHA failed on the flake iter-353 then fixed. Whether a round-2 verdict was obtained after `25d317650` is UNKNOWN: no artifact exists. Docs moved to `implemented/v0_35_3/`.] [iter-351, surfaced by SonarCloud's new-code coverage gate on PR #1111 with a firing negative control; PRE-EXISTING at HEAD, the sprint merely added six new lines to an already-untested function] `m-daemon-task-exec-run-untested` — the coordinator daemon's task-execution path has NO unit test at all.**
Measured 2026-09-08: of the 33 new lines in PR #1111, **7** were uncovered and **6 of those 6** sit in `internal/coordinator/daemon_tasks_exec_run.go` — the `ExecuteOptions` construction at `:238-241` and the `ExecuteWithRetry` call at `:335`, i.e. the only production caller of the retry path this sprint made injectable. They are uncovered because nothing exercises the function, not because the sprint skipped a test: the same lines were uncovered before, they simply were not *new* before. Note what this means for the sprint's own FIX 2 (rebase the production caller on `DefaultExecuteOptions()` and override five fields, quorum reviewer `oc-glm-5-2`'s verbatim objection): the rebase is verified only by code reading and by the judge's contract analysis — **no test executes it**. The seventh uncovered line, `defaultWait`'s elapsed arm, WAS closed in-flight (`TestDefaultWaitBothBranches`, two compiling mutants RED). Deliberately not bundled: covering a daemon path needs a fixture and a decision about how much of the daemon to stand up, which is a design question, not a line to squeeze in behind a coverage gate.

**[LANDED 2026-09-14 (iter-354) — PR [#1166](https://github.com/sunholo-data/ailang/pull/1166) → [`266cf23a2`](https://github.com/sunholo-data/ailang/commit/266cf23a2ea6c7247243ed9a687226639897b9e4); 22 checks on the PR head, ZERO not-green; independent judge `sonnet` r1 **PASS 96/100** (zero blocking; its own diff-anchored mutant found the notice prose unasserted → row added) / r2 **PASS 100/100**; doc + plan + 22-mutant audit + both evaluations in `implemented/v0_38_6/`. Shipped: `PIN_AGE` = `rev-list --count <target>..<origin/dev SHA captured once after the gate refresh>`, transported as `AILANG_DRIVER_AGE`/`AILANG_DRIVER_AGE_BASE_SHA`, own state file + threshold 25 + dedupe-until-doubling + `pin-age` notice, `PIN_AGE_SUPPORTED` handshake for old target helpers; pin suite 81 → 117, notify suite 50 → 82; two test rows repaired from the drill (MUT-R survived with prev=170 — dedupe hid a deleted status guard; MUT-F killed by the AC-D fixture-rot guard, not the named assertion). Also this iteration: the REQUIRED `test` context was red on `dev` from `7423434b4` (a concurrent direct-to-dev commit added `pi-or-minimax-m3` at 32000 without a `ceilingLimited` entry) — fixed forward first as [#1165](https://github.com/sunholo-data/ailang/pull/1165) → `fd44be9ee`.] [iter-353, residue of the RESOLVED `m-fleet-sha-pin-freezes-every-driver-fix` row; measured at iter-349 and re-verified at iter-353] `m-pin-drift-blind-under-sha-pin` — `tools/launchd/lib/pin-root.sh` defines `PIN_DRIFT` as "commits the source clone is behind the ref" (`git rev-list --count HEAD..$ref`, line 219), so under any `AILANG_DRIVER_REF` that is a SHA already contained in the clone the reading is `0` by construction, and a frozen deployment pin is invisible to the one instrument that exists to report it.**
Measured: with the 2026-09-07 SHA pin in place every fire logged `driver pin drift: 0 below warning threshold 25` while `origin/dev` was 43 (then 100) commits ahead of the pinned SHA; with the pin returned to `origin/dev` (attended, D-61) this fire logs `drift: 15` for a clone 15 behind — the instrument is sound for a branch ref and blind for a commit ref. Fix shape (small, `tools/launchd/`): when the ref resolves to a commit rather than a remote branch, ALSO compute and log `rev-list --count <ref>..origin/dev` as `PIN_AGE` and warn on it, so a pinned deployment reports how far behind the default it has drifted; test in `tools/launchd/test_*.sh` with a branch-ref control (`PIN_AGE` absent) and a SHA-ref arm (`PIN_AGE` = the known count). D-61 scopes the loop's pin authority as a bridge that retires with the shell driver, so this is an INSTRUMENT row, not a pin-management one.

**[NEXT] [world-DEMAND — cross-mission proposal from `mission-world` iter-174 (row 73), issue [#1160](https://github.com/sunholo-data/ailang/issues/1160), ghost-disciplined REAL on V1 at iter-354] `m-gate0-self-crash-notice-read` — Gate 0's directive read is blind BY CONSTRUCTION to the driver's own `⚠️ Mission iteration **FAILED to complete** (rc=…)` notices, because the author allowlist (correct, must not widen) drops every comment the loop's own account posts — so a slot killed mid-flight leaves a notice on the bookkeeping issue that no gate ever reads.**
Verified on V1 (2026-09-14): `#1072` carries one self-authored `FAILED to complete (rc=143 — timeout or crash)` comment at `2026-09-07T23:20:56Z` — the slot the verdicts log records as `KILLED_at=gate-3b rc=143` — and `scripts/mission_directives.sh --issue 1072 --since 2026-09-07T23:20:56Z` reports `0 directive(s) … of 16 comments`, correctly; iteration 349 recovered that orphan only through Gate 2's worktree/PR traces. World's instrument (`scripts/gate0_self_notices.sh`, landed `50103fd`, ailang-world#137) returned rc=1 on its first live run for a notice invisible to six days of fires. **Ask, as filed:** (1) a SECOND, no-authority Gate-0 read in the shared skill — self-authored comments on the current AND `-prev` issue matched by the notice's anchored prefix since the watermark — whose only consequence is to trigger Gate 2's died-mid-flight traces (a)–(c) and a log credit (it can unpark nothing, pick nothing); (2) driver: keep the notice PREFIX stable and machine-anchored (`mission-control.sh:1992`; the tail drifted 200 → 284 chars between 09-02 and 09-07); (3) driver: reword *"The queue is untouched; the next interval will retry"*, which reads as "nothing happened" precisely when the dead fire had merged its PR seconds earlier. Demand evidence satisfied by construction (a real downstream consumer with a live repro). Skill half = a Gate-5 skill edit; driver half = a small `tools/launchd/` sprint with `test_driver_notify.sh` arms. Triage verdict acknowledged on ailang-world#138 and #1160 (left open as the tracking issue).

**[NEXT] [iter-354, first-party; pre-existing at base `fd44be9ee`, NOT from this iteration's sprint] `m-codex-quota-admission-test-reads-live-ledger` — `tools/launchd/test_codex_quota_admission.sh` reads the rig's LIVE `~/.ailang/state/quota-ledger.json` (mtime 2026-09-14 13:01, written by the concurrent attended quota-rationing session), so `make test-launchd-drivers` is red on the rig whenever the live ollama bucket is blocked while the identical CI job is green.**
Measured: `FAIL role routing: 1 pi:openrouter/moonshotai/kimi-k3 pi:openrouter/deepseek/deepseek-v4-flash-0731` (the chain walked past its first link) on the sprint tree AND on base `tools/launchd` in a clean worktree at `fd44be9ee`; `test_mission_routing.sh` 83/0 in the same env; CI `launchd drivers (bash 3.2)` green on the same SHA. Fix shape (small): the admission arms must run under a synthetic `STATE_DIR`/`HOME` like every other launchd suite (`mktemp -d`), with a positive-control arm that seeds a blocked-bucket ledger fixture and asserts the walk.

**[PROCESS FINDING — iter-354; third instance, so the ≥3-evidence bar for a routing-policy change is MET and the Gate-5 skill edit spent on it] `note-pi-executor-30-min-cap-vs-milestone-size` — the pi executor lane's 30-minute wall clock is smaller than a plan-sized milestone, three times now: iter-347 M3 (`wall_timeout`, rc=13, partial discarded), iter-354 M1 (`wall_timeout` at 1810 s with T1–T11 landed and every gate green outside the sandbox; the drill never ran) and iter-354 M2 on the OpenRouter fallback link (`wall_timeout` at 1810 s, 31 arms written unrun, 41 tool executions — the OpenRouter route was ~4x slower per tool call than ollama-cloud). Both fallback links were spent on the same milestone size, and the end-of-chain `opus` executor spawn is DENIED by the spawn-pin hook while the role carries a provider pin, so the controller repaired the unrun arms itself (10 assertion needles measured against the observed traces; production untouched). Skill edit landed this iteration: the cap is a per-run parameter (`--max-seconds`), default 1800, raised to 3600 for a milestone the plan sizes above ~150 LOC — still bounded, never `0`. Separately observed and NOT fixed: `scripts/mission_pi_run.sh` does not pass the two `-e` sandbox extensions the recipe calls mandatory, so pi executors have run unfenced since the runner was adopted (1 first-party reading; the post-hoc main-checkout `git status` was clean).**

**[NEXT] [iter-353 weekly sweep, rotation week of 2026-09-14] `m-weekly-sweep-orphans-2026-09-14` — the Monday sweep enumerated **85** open issues (`gh` count control 85 = 85; positive control `#1111` fires at 4 in the charter; a fresh negative control fired) and found **7 with zero mentions** across charter, log, log-archive, status-archive and dashboard. Batched as ONE row per the Gate-0 sweep rule; a sweep never outranks existing picks by itself.**
Per-issue: `#1136` rig-lock yield (design doc `m-rig-lock-yield` landed 2026-09-13 as an as-built retro — likely close-with-verdict), `#1134` IFC cross-module labels (doc `m-ifc-cross-module-labels` landed 09-13), `#1132` Daneel's IFC Phase 2 asks (referenced by the same doc), `#1131` `ailang docs` cannot find the stdlib outside the repo (doc `m-docs-stdlib-resolution` landed 09-13), `#1130` `std/cognition` Msg effect has no CLI handler (doc `m-cli-msg-handler` landed 09-13), `#1096` `mission_pi_run.sh` reports `empty_worktree` for a COMMITTING executor (a sibling of the parked `m-pi-runner-worktree-assertion-vacuous-on-revision` / D-58 — the runner's worktree-diff verdict is blind in BOTH directions: an executor that commits leaves no diff, one that revises an untracked file leaves the previous run's), and `#1089` (the docs mission's own weekly bookkeeping thread — not an orphan, listed for completeness). Triage-lite owed: ghost-discipline each repro at HEAD → verdict comment → queue-or-close; the five daneel issues that already carry landed design docs need only the comment linking the doc and a queue position, not a new design.

**[NEXT] [iter-351, round-3 judge, derived from the DIFF rather than from any plan's list; PRE-EXISTING scope, surfaced by removing a vacuous assertion] `m-approval-poll-production-defaults-unexercised` — the store-backed approval checkpoint's two production defaults have never been exercised TOGETHER through the real constructor.**
Measured 2026-09-08. `NewStoreBackedApprovalCheckpoint` has **zero** production callers (design doc V16), so the pair `2 * time.Second` + `defaultPollTick` is only ever assembled by tests — and every test uses one or the other: the edge tests and the injection guard pass a recorder `tick`, while `TestDefaultPollTickFires` calls `defaultPollTick` standalone. Nothing runs `RequestApproval` end-to-end on the real ticker at the real interval. The judge also measured that `TestDefaultPollTickFires` does **not** pin the `interval` argument: a mutant ignoring the parameter and hardcoding 50 ms survives, because it still delivers inside the 500 ms liveness bound. Low severity while there are no production callers — dormant, not live — and deliberately filed rather than fixed, because the fix is a new end-to-end test on a just-landed green path. Distinct from `m-ratelimit-window-default-unpinned`: that one is a default with no tripwire, this one is a default with no end-to-end exercise.

**[NEXT] [iter-351, measured by the independent judge in round 2 and disclosed by the design doc itself as O2c; PRE-EXISTING at HEAD, not introduced by the sprint that surfaced it] `m-ratelimit-window-default-unpinned` — the rate-limit reset window in `checkRateLimit` is a hardcoded `time.Second` with **no tripwire anywhere in the suite**.**
Measured 2026-09-08 by the round-2 judge in a disposable scratch copy: mutating `event_handler.go`'s `if now.Sub(h.lastEventTime) >= time.Second` to `500 * time.Millisecond` leaves the **entire** `internal/coordinator` package green — including `TestCoordinatorEventHandler_RateLimitReset`, `TestCoordinatorEventHandler_RateLimitWindowInjected` and the new `TestProductionTimerDefaultsPreserved`. The clock-injection guard cannot catch it by construction: it asserts that an INJECTED clock is honoured, which stays true when the window constant moves, and it advances the clock by 1100 ms, which resets a window of 1 s and a window of 500 ms alike. `TestProductionTimerDefaultsPreserved` pins `RetryBaseDelay` and `maxEventsPerSec` (both proven RED against compiling mutants) but the window is not a constructor field, so there is nothing for it to read. Deliberately NOT fixed in-flight — a fresh un-reviewed assertion added to a just-passed sprint is the change this loop keeps closing — and the iter-351 test's docstring was narrowed instead, so no reader can mistake it for covered. Likely shape: shorten-direction mutation coverage via a clock advance derived from the window itself, or lift the window to a named constant the defaults test can read.

**[NEXT] [iter-348; quorum reviewer `gpt6-astra`'s objection, accepted verbatim rather than argued down] `m-headroom-blocking-threshold-calibration` — the shipped `-timeout 416s` rests on a RIGHT-CENSORED measurement, so `tools/ci/headroom` ships WARN-ONLY and no blocking threshold can be justified yet.**
The 300.520 s reading that drove the whole derivation is the package being **cut off at the ceiling**, not its completion time — so the 1.34x term is a lower bound and 310.5 s is an extrapolation, not an observed runtime. Neither establishes that 416 s provides headroom, nor that transient scheduling noise cannot cross a blocking boundary. What would: repeated `test-windows` runs at a FIXED commit under a documented longer diagnostic `-timeout`, recording runner resources, package coverage, cache status, completion times and any remaining timeouts, then an explicit acceptable false-positive rate. That is N CI runs, which is why it is its own row rather than a step in iteration 348's sprint. Until it exists the budget stays labelled provisional and the instrument stays WARN-ONLY — a FAIL tier on a runner with 1.80x measured variance would be a new flake source introduced by the fix for a flake.

**[NEXT] [iter-348, round-3 judge, derived from the DIFF rather than from the plan's list] `m-headroom-residual-mutations` — four mutations of `tools/ci/headroom` survive its whole suite, and one of them is a compound deletion that produces a silently-green CI job.**
The judge's mutation drill anchored to the diff (rule 3n) rather than to the plan's named rows. Survivors: deleting the M3 build step AND its invocation **together** leaves the job green with the instrument never running and zero test coverage of that state; the `cannot-read-log` exit-1 path is untested; and `budget="Inf"` / `budget="NaN"` pass the `budget <= 0` validation (`ParseFloat` accepts both). Near-zero real-world risk on the last two, but the first is the same shape as the defect that cost this sprint two rounds — a state in which the instrument reports nothing and everything reads green. Also outstanding from the same report: the sprint plan's M2 acceptance commands still describe a fixture that cannot clear the package-count floor.

**[NEXT — iter-347 Gate-1-class read on my own merge SHA; a standing RED on the `dev` BRANCH that no workflow name can surface, because SonarCloud is a GitHub App and not an Actions workflow] `m-sonar-dev-branch-security-rating-c-on-new-code` — the `dev` branch quality gate has been failing on **C Security Rating on New Code** since `8e3927950`, and it is NOT from this iteration.**
Measured by walking the check back over consecutive commits, which is the negative control this class needs: `81abc956d` **success**, `5debf4e10` **success**, then `8e3927950` **failure**, `3ed49a6d1` **failure**, `4e9466b28` **failure**, `98730db02` **failure**, and iteration 347's own merge `e5a325a20` **failure** — so the transition is at `8e3927950` (the `#1082` mission-runtime merge), four commits before anything this iteration pushed. The paired control that makes this unambiguous: the same check is **`success`** on this sprint's PR head `1c7783800`, because PR analysis and BRANCH analysis use different new-code periods, so a PR can be green on exactly the code whose branch gate is red.
Failing condition, verbatim: *"C Security Rating on New Code (required ≥ A)"* —
[dashboard](https://sonarcloud.io/dashboard?id=sunholo-data_ailang&branch=dev). Not required for merge, so it does not block; that is precisely why it accumulates. Gate 1's own rule is the reason this row exists at all: the per-workflow health loop is a hand-maintained allowlist and is **blind by construction** to a GitHub App, and the last time this went unnoticed it ran red for six consecutive commits. Start with the `sonarcloud-triage` skill and the security hotspots the gate names; do not mark anything Safe without reading it.

**[PROCESS FINDING — iter-347, no code owed, recorded because it is the second collision of this shape on this repo] `note-attended-landing-collided-with-a-live-mission-item` — an attended session landed a competing, narrower fix for the item this iteration was mid-flight on, under a ruling that had already rejected the narrower scope.**
At 14:15Z, while iteration 347 was between its executor and its judge, PR #1082 landed [`2a38f0948`](https://github.com/sunholo-data/ailang/commit/2a38f0948) — a test-only repair of the same seven notify assertions, with the honest note *"Production driver is unchanged … does not claim completion of the broader retry/drain/environment coverage."* But **D-60 had already ruled option B and rejected test-only**, and the two are not composable: the attended fix keeps `ailang()`/`gh()` as in-shell FUNCTION stubs, which `_mc_bounded` cannot reach at all, so the iteration's fixture work SUPERSEDES rather than duplicates it. Reconciled on rebase by keeping the superset and carrying the attended note forward verbatim into the design doc.
Nothing here is a fault: the attended session was clearing a red it had inherited on its own merge, and it could not see a mission mid-flight. Gate 1 scopes a RED to the owning mission and Gate 2's died-mid-flight traces look for orphaned PRs — **neither sees a HUMAN working the same file right now**, and the CLAIM message this loop sends on the controlplane is not something an attended session reads. The cheap mitigation, if it recurs a third time: re-read `origin/dev` for the picked item's own files immediately before Gate 3b, not only at Gate 1.

**[LANDED 2026-09-08 (iter-349) — M3 only; PR [#1107](https://github.com/sunholo-data/ailang/pull/1107) → [`b5513ccdf`](https://github.com/sunholo-data/ailang/commit/b5513ccdfb7b9e014c197e9021d27be5272ebbe1); judge `sonnet` PASS 88/100, zero blocking, 21 checks on the PR head with zero not-green. **M4 is still owed** — the sprint's 11-row M1-M3 mutation sweep was never executed (attempt 1's executor hit `wall_timeout` partway through it); the 4 M3-anchored rows were run by the judge, the M1/M2 rows are not. This row stays open for M4.] [iter-347; the unexecuted M3/M4 of the sprint above] `m-launchd-drain-aggregate-budget` — `_mc_drain_notices` bounds each row and not the whole drain, in the one phase of the fire with no deadline behind it but the 6-hour `HARD_TIMEOUT`.**
Raised by quorum reviewer `gpt6-astra` and CONFIRMED first-party at iteration 347: `_mc_drain_notices` is called exactly once, at `mission-control.sh:972`, immediately after the `# --- DRIVER PIN DECISION END ---` marker — i.e. in the driver's PREFLIGHT, before the controller session exists. The only deadline covering that phase is `HARD_TIMEOUT="${MISSION_TIMEOUT:-21600}"` (`:795`, 6h, whole slot); the stall watchdog cannot rescue it, because `:289` logs *"stall watchdog has NO progress instrument … early kill DISABLED"* in exactly that situation, and a drain making slow progress is not "no progress" anyway. And the spool is unbounded by construction: `notice-spool` has exactly two references (`:147` read, `:195` append) — no size cap, no row cap, no rotation. So an N-row spool of hanging sends costs `N × (bound + overhead)` inside the phase whose only backstop kills the entire fire.
Design section already written and quorum-reviewed (`planned/v0_35_2/m-launchd-notify-subshell-observation.md`, Half 1 step 6 + row G4); sprint plan milestones M3/M4 already written (`…-sprint-plan.md`). The executor's partial M3 — production side substantially complete, its own two new tests FAILING at `37 passed, 2 failed` — is preserved at `~/.ailang/state/mission-v1-iter347-m3-partial/` with its suite output. **Verify, do not adopt.**

**[LANDED 2026-09-14 (iter-353) — PR [#1161](https://github.com/sunholo-data/ailang/pull/1161) → [`3faaf47fa`](https://github.com/sunholo-data/ailang/commit/3faaf47fa06549c94eac95432a6933ee11df4cf6); 22 checks on the PR head, zero not-green; judge `sonnet` r1 PASS 88 / r2 PASS 98, zero blocking; doc + plan + audit + both evaluations in `implemented/v0_38_6/`. **The mechanism was the CAPTURE HELPER, not the assertions**: `r.Close()` before `<-done` discarded whatever the copier had not drained; fixed with a named-return `defer` teardown (writer → hook → bounded wait → reader → copy-error panic) pinned by four gated-reader tests and a six-mutant drill. **AC6 is OWNED BY THE NEXT ITERATIONS**: zero `Build macos-latest` jobs failing on this test across the next 25 `dev` runs (~1.5% false-clean probability at the measured ~8%/execution base rate); a single green is not evidence. The row's "cached 0s ago" timing hypothesis and "structural counters" remedy were both refuted by measurement.] [iter-346 owns this: it is a red my own landing put on `dev`] `m-debugcacheforms-flaky-on-macos-ci` — `TestPipelineModulePhases_DebugCacheFormsAndCounters` fails intermittently on the macOS CI runner, and 28 local runs cannot reproduce it.**
Measured first-party after landing [#1071](https://github.com/sunholo-data/ailang/pull/1071). The failure is `pipeline_module_phases_test.go:268`, *"invalid summary counters missing"*, on `Build macos-latest` at [`5debf4e10`](https://github.com/sunholo-data/ailang/commit/5debf4e101b6ef11d6e8d271f5489374fcefcdab) — a commit whose entire diff is **4 added lines of markdown**, so the content cannot be the cause. **Negative control, and it is what establishes intermittency rather than breakage:** the same check reads `success` on all three preceding commits — `5c64d3a1a`, `202358fd3` (the merge that introduced the test) and `1fd1c7a70` — so identical test code passed on macOS three times and then failed. **Local repro attempts, all rc=0, none reproducing:** the single test `-count=25`; the full `internal/pipeline` package `-count=3`; and `./internal/pipeline ./internal/loader -p 4`. So it is specific to the CI macOS runner, not to the code path or to package-level parallelism.
**This is the THIRD platform this one test has failed on, and that is the finding.** Iteration 342 already skipped it on Windows (`runtime.GOOS == "windows"`) for a *different* symptom of the same underlying fragility: it asserts on `[CACHE]` diagnostic **string forms** and summary counters, and the captured output in this macOS failure shows the assertion tripping on cache-summary lines that are timing-dependent (`cached 0s ago`) and, on Windows, on a drive-letter cache-path bug. A characterization test whose oracle is a formatted human-readable diagnostic is fragile by construction; the answer is almost certainly to assert on the counters structurally rather than on the rendered summary, NOT a second `t.Skip` — the sprint has now bought platform coverage down from three to one, and a third skip would leave the behaviour untested everywhere.
**Not blocking dev:** `Build macos-latest` is not one of the four required contexts (`test`, `lint`, `build`, `docs-gate`), all of which are green at dev HEAD. It is a red that should not be allowed to become ambient — iteration 346's own Gate 1 spent real effort proving the *other* standing red was legitimately parked, and two standing reds is how the third one gets missed.

**[NEXT] [WORLD-DEMAND — `mission-world` iter-166 filed [#1075](https://github.com/sunholo-data/ailang/issues/1075); triaged and CONFIRMED first-party by V1 iter-346, which owns `tools/launchd/`] `m-pin-root-redirects-de-forked-mission` — `pin-root.sh` re-execs every mission into a worktree of the DRIVER's own clone, which is correct for three missions and puts the fourth in the wrong repository.**
Confirmed at HEAD, not transcribed: [`tools/launchd/lib/pin-root.sh:267`](tools/launchd/lib/pin-root.sh) does `MISSION_WORKDIR="$wt"` **unconditionally**, `$wt` is derived at line 164 from `$src`, and `$src` at line 106 is `$(dirname "$0")/../..` — the driver's own root. For v1/docs/motoko, which are separate *clones of the same repo*, that is right. For World, whose work repo is a *different repository*, it silently relocates the mission. World measured its own fire: `MISSION_WORKDIR` = `~/.ailang-driver-pin/world`, origin `sunholo-data/ailang`, `ls design_docs/world-mission.md` → No such file, with the driver logging `PIN_STATUS=pinned` and drift 0 throughout. **The failure is invisible by construction**: every Gate-1 health instrument answers correctly *about the wrong repository*, so the gate reads green. `mission-control.sh`'s own comment at ~894 asserts the opposite — *"MISSION_WORKDIR keeps `$REPO` pointing at the mission's work repo across the re-exec"* — and must be corrected with the fix. `tools/launchd/*` is frozen core for World (`D-WORLD-DRIVER-1`), so World mitigated rig-locally (`AILANG_DRIVER_PIN=0`, reversible, backup taken) and handed the guard to us; it will re-enable the pin once this lands. **The issue carries the full spec**, the origin-URL-vs-`git-common-dir` measurement (all three clones have different common dirs, so common-dir would break motoko), and AC-F1..AC-F12 with named RED mutations — including World's own independent evaluator proving **AC-F8 is vacuous as written**, which is a gift and should be fixed before the sprint plan, not after. Note the demand-evidence gate is satisfied by construction: a real downstream consumer is currently running with our safety mechanism switched off.
**Also arrived with it, and NOT actioned this iteration:** World proposes a one-line Gate-1 assertion — before recording any health verdict, assert `test -f "$REPO/$MISSION_DOC"` and that `git -C "$REPO" config --get remote.origin.url` names `$MISSION_REPO`. V1 iteration 346 **ran that check by hand and it passed**, which is corroboration, not adoption. It is a sound proposal and it is mission-independent; it is not in the skill because this iteration's single Gate-5 skill edit was already spent (the judge-instrument rule). It should be the next iteration's edit if nothing outranks it.

**[NEXT] [iter-346, measured by the independent judge's six-mutation drill and REPRODUCED first-party by the controller for #5; PRE-EXISTING coverage gaps at the shipped code, not defects the sprint introduced] `m-cachesrc-phase-ordering-untested` — three mutations of the newly-extracted compile-cache phases survive both packages' entire suites.**
Measured at `202358fd3` on the rebased tree, each mutation restored byte-clean and sha256-verified: **(1)** removing the "verified-but-nil-payload falls through to a fresh compile" guard in `serveFromCache` survives — that state is never exercised; **(5)** reordering `run()` so `registerResolver()` runs *before* `compileAllAndFinalize()`, making the resolver iterate an empty `compiledUnits` map, survives `go test ./internal/pipeline ./internal/loader -count=1` rc=0 **including both packages' `ModeEval` tests** (controller-reproduced: mutation applied, `go build` rc=0, tests rc=0, restored byte-clean); **(6)** flipping the nil-`SourceContent` `cacheable` flag from `false` to `true` survives, because the existing assertions are all satisfied via the empty cache key naturally missing lookup. None of the three says the shipped code is wrong — the shipped order and flags are correct — they say **the suite cannot see a future regression at three named spots**, and #5 in particular means cross-module resolver-registration timing is unobservable to anything in `internal/pipeline` or `internal/loader`. Cheap: one forced `(nil, entry, true)` cache-load test, one ordering assertion, one direct assertion on the `cacheable` return.

**[NEXT] [iter-346, first-party; a metric-attribution trap that has now cost a controller AND its independent judge in the same iteration] `m-sonar-vs-gocognit-instrument-mismatch` — `go:S3776` and `gocognit` disagree on every cognitive-complexity target measured, and nothing in the loop says which instrument a written-down number came from.**
Measured across all five of this sprint's Sonar targets at base `d794cac25`: SonarCloud 29/19/24/16/103 against gocognit 47/21/38/20/106 — **every one differs**, and the live API confirms `from 103` for key `AaBzgnbFBD7wArG_Hhqs`. This iteration flagged the doc's `103` as a defect, the independent judge scored an evidence-integrity ding for it, and **both were wrong**: the doc cites Sonar with its issue key and the exact reproducing `curl`. The design doc's acceptance criterion is a Sonar re-analysis, but every local gate this loop actually runs is `gocognit`, so a green local reading is evidence for a *different* metric than the criterion it is quoted against. Two cheap fixes, neither of which needs a design pass: **(a)** require every complexity figure written into a doc, plan or log to name its instrument inline (`103 [sonar]`, `106 [gocognit]`); **(b)** make the local complexity gate print its instrument and version in the same line as the number. The general form is rule 3c aimed at a metric rather than a service, and the sharper lesson is about the *judge*: an independent agent re-running the controller's wrong instrument produces agreement, not corroboration.

**[NEXT] [iter-346, first-party; the index has been silently stale for five iterations and Gate 2 mandates grepping it before every pick] `m-mission-index-stale-since-340` — `design_docs/v1-mission-index.md` stops at iteration 340, so the file Gate 2 calls "the cheapest way to find out whether something has already been tried" is blind to 341–346.**
Measured: the index's newest row is `340`; iterations 341–346 are absent. It is regenerated wholesale by `ailang mission rotate-log` and explicitly must never be hand-appended ("an append-only index drifts the moment an entry is edited, and an index that answers confidently and wrongly is worse than none") — so the staleness is a *missed rotation*, not a missing append, and it is a direct consequence of iterations 341–344 dying before Gate 4. Note the compounding shape: the four iterations whose work was hardest to find are exactly the four the lookup instrument cannot see. Fix is to run the rotation; the durable question is whether the index regeneration belongs at Gate 4 unconditionally rather than only on a rotation week.

**[NEXT] [iter-345, measured by the independent judge's mutation drill; PRE-EXISTING at HEAD, not introduced by the split] `m-exec-event-handler-untested` — `spanningEventHandler` has NO behavioural test coverage, so `go test ./cmd/ailang/...` cannot see any change to how `ailang exec` emits spans, NDJSON or chat-history rows.**
Four independent mutations of `cmd/ailang/exec_events.go` — an OTEL attribute-key rename, a dropped `eventStore` chat-history write, an inverted `h.turnSpan != nil` check that would nil-deref at runtime on the next `OnText` after a turn ends, and a removed `h.turnSpan.End()` that leaks a span — **all four survived** `go build`, `go vet` and `go test ./cmd/ailang/...` green, each restored byte-clean and proven by SHA-256 against the committed blob. This is a statement about the tests, not about iteration 345's move: it was equally true before the split, which is why it did not block a pure move. The consequence worth acting on is that this handler is on the path of every agentic `ailang exec` run — the executor lane the fleet itself uses — and its observability output is currently unguarded by anything. Scope when picked: table-driven tests over the handler's eight callbacks against a recording `eventStore` and an in-memory span exporter; the four mutations above are the ready-made non-vacuity controls, since a suite that does not kill all four has not earned its green.
...
**[PARKED — needs-human-review, iteration339, independent round3 hard fail] `m-pi-runner-shell-suite-coverage`** — candidate branch `sprint/v1-iter339-pi-runner-shell-suite-coverage` is preserved at `2eb17d026dee62649297a19d50f2943612a20438`; it was neither pushed nor merged. The final tree independently passes focused19/19 and wiring37/37, including W10 startup refusals, with43 recorded identities and zero survivors. Round3 nevertheless returned FAIL70/100 because mandatory full `make test` hit unchanged `internal/smt/TestSolve_HardTimeout_FakeSolverIgnoringT` startup race #602; isolated exact attribution passed, but the evaluator skill has no baseline/flakiness waiver. The judge also measured one residual private temp directory when pre-fork mkfifo creation is injected to fail, with no process launch. Earlier rounds FAIL83 and FAIL88 found real ordinary-exit and startup-orphan defects, now repaired. **Resume only after human disposition at the three-round cap:** authorize a fresh scoped iteration that first closes temp cleanup and separately establishes a green mandatory aggregate, or decline/archive the candidate; do not land on controller-local green or silently add a fourth evaluator round. This item remains separate from D-58.

**[LANDED 2026-09-07 (iter-340) — PR [#1069](https://github.com/sunholo-data/ailang/pull/1069) → [`aeeafc880`](https://github.com/sunholo-data/ailang/commit/aeeafc880dec8bb30215620332d938e96904aaf0); OpenRouter MiniMax PASS99, all13 mutations killed] [iter-336; first recorded session-protocol instance] `m-pi-evaluator-session-handshake`** — `scripts/mission_pi_run.sh` now supplies canonical GCP/project messaging values at the Pi child boundary while preserving `AILANG_STORAGE`; Gate 3 delivers one tested, source-bound same-session read/list/summarize/ack/judge preamble. Focused12/12, aggregate Pi extensions, full test/lint, Windows, platform builds and docs are green. The configured Ollama MiniMax primary prepended disallowed exports despite the exact instruction, so its report was correctly rejected as transport failure; the OpenRouter twin obeyed the bare bounded command before acknowledgement and supplied the accepted independent verdict. Full evidence: `docs/sprint-retros/iter340-pi-evaluator-session-handshake-evaluation-openrouter.md`.

**[LANDED 2026-09-07 (iter-346, verify-and-land of iterations 341-343) — PR [#1071](https://github.com/sunholo-data/ailang/pull/1071) → [`202358fd3`](https://github.com/sunholo-data/ailang/commit/202358fd3d4af042b369432a1dd8d7b01c9e5e76); judge `sonnet` LAND 91/100, zero blocking; `runModuleWithCacheDependencies` gocognit 106 → 0] [iter-331, measured first-party on PR [#1053](https://github.com/sunholo-data/ailang/pull/1053); NEW and MINE, not inherited] `m-cachesrc-cognitive-complexity` — the SonarCloud new-code maintainability gate is red on the M2 diff, and it will stay red for M3 and M4.**
...
**[NEXT] [iter-349 Gate-0 triage of public feedback `fb_cc1cb8fb23e1ead1` (`mcp-public`); CONFIRMED first-party at `origin/dev`, and the report UNDER-states it] `m-std-ai-no-output-token-bound` — no `std/ai` step entry point accepts an output-token bound, so an AILANG program can read the output-token count it spent but can never cap or raise the budget it is allowed.**
Measured at `origin/dev` on `std/ai.ail` (375 lines): FOUR exported step entry points — `step` (:166), `stepWithCache` (:184), `stepWithStream` (:276) and `stepWithStreamRecorded` (:292) — and `max_tokens`/`maxTokens`/`max_output` appear **zero** times anywhere in the module. `output_tokens` appears only on the RESULT side (`StepResult` :129, `StreamChunk.Usage` :235). Controls fired: `StepResult` = 12 hits (known-present), an invented symbol = 0 (known-absent). The reporter named three entry points and cited lines :203/:225/:330, i.e. it measured an older copy and missed `stepWithStreamRecorded` — the gap is one entry point WIDER than reported. **This is public feedback, so it enters the queue on this first-party evidence and not on the strength of the request** (Gate 0 rule 4). Not prioritised above anything.

**[NEXT] [iter-349, first-party while triaging an `eval-suite` inbox notification; the fifth consumer the `m-eval-standard-mode-input-files-gap` verification table did not enumerate] `m-eval-suite-notification-blind-to-validity` — the eval-suite completion notification counts a `Validity`-invalid row as a FAILURE, so a benchmark that was never measured lands in the controlplane success rate as a benchmark that failed.**
`cmd/ailang/eval_suite_finalize.go` computes `ranCount = successCount + failCount` (:137) and `success_rate = successCount/ranCount` (:164) with **zero** references to `Validity` or `IsValid()` in that file — control: `internal/eval_analysis/validity_filter.go` reads 4 hits for the same pattern, so the grep fires. The `m-eval-standard-mode-input-files-gap` design chose `MarkInvalid(ReasonModeIncompatible)` precisely because `LoadResults` → `FilterValidResults` excludes invalid rows from *"EVERY aggregate"*; its own verification table enumerates eval-elo, confidence-gating, curation-cycle and the dashboard/export path. The controlplane notification is a separate path that never loads through `eval_analysis` at all. **Scope, stated honestly:** in the normal scheduled path component 2 already excludes the benchmark so no job is created, and this is reachable only through a direct `--benchmarks` invocation — which is exactly the case the M2 dispatch-time guard exists for, so the guard's own reporting is the part still wrong. **Ruled out while filing this:** *"tonight's `Eval Suite partial: 0/2 passed` message is this defect firing"* — **REFUTED by timestamp**: that run finished `2026-09-07T19:59:50Z` and #1104 merged at `20:36:26Z`, so the 0/2 predates the gate entirely and is the pre-existing `input_files` bug the gate was written to close.

**[NEXT] [iter-349, first-party, found while verifying the drain this iteration LANDED; the loudest alert this fleet has, silent on V1 for over 24h] `m-v1-lane-degraded-notices-never-deliver` — every V1 lane-degradation notice since 2026-09-07T00:37Z is stuck in the spool and re-fails on every fire, while the byte-identical notice from a sibling mission delivers.**
Measured at iteration 349. `~/.ailang/state/mission-v1-notice-spool.tsv` holds **3** rows, all titled `Mission v1: executor/planner lane degraded`, stamped `00:37:21Z`, `05:27:59Z` and `12:35:33Z` on 2026-09-07. The driver has logged `notice spool: N notice(s) still undeliverable — kept for the next fire` on **six consecutive fires** (08:56, 11:23, 14:34, 19:50, 00:02, 01:32), so `_mc_notify`'s three-try retry AND the spool drain have both failed every time. **Controls, both firing and both discriminating.** Positive: V1's own controller-session sends land normally — 11 `mission-v1` messages delivered to `controlplane` on 2026-09-07 alone. Cross-mission negative control (rule 3l — the fleet is the control group): `mission-docs` delivered a **driver-sent** notice of the same kind, `Mission docs: executor/planner lane degraded`, at `2026-09-07T19:42:27Z`. So this is neither "the CLI is broken" nor "the message plane is down" nor "driver-sent notices cannot deliver" — it is specific to V1's driver path. Cause NOT established and deliberately not chased here (standing rule 1); candidates to discriminate first are `MSG_FROM`, the per-row body size and the `env`-prefixed `_mc_bounded` child's PATH under launchd. **Why it outranks its size:** the spool's whole design intent is *"an outage costs a delay, not the record"*, and the record has now been undelivered for over a day while the driver reports the condition as a routine one-line INFO. The `m-launchd-drain-aggregate-budget` fix landed this iteration bounds the drain's TIME and is orthogonal to this — with 3 rows failing in ~17s total the aggregate budget is nowhere near engaged.

**[NEXT] [iter-349, the round-1 judge's non-blocking finding #1, reproduced by the controller independently via code reading BEFORE the judge reported it — two routes, not one instrument twice] `m-drain-deferred-counter-overcounts` — `_mc_drain_notices` reports a deferred-row count that is too high whenever a malformed spool row precedes the deferral point, and the malformed row is silently and permanently dropped.**
`deferred=$(( total - sent - kept ))` (`tools/launchd/mission-control.sh:161`, landed this iteration): `total` is `wc -l` of the spool, while a row with an empty title hits the pre-existing `[ -z "${title:-}" ] && continue` — which advances `lineno` but increments neither `sent` nor `kept`, so it inflates `deferred` and is never re-appended. Demonstrated by the judge with a standalone extraction of `_mc_drain_notices`/`_mc_bounded`: a 4-row spool containing one malformed line (`MISSION_NOTIFY_TIMEOUT=2 MISSION_DRAIN_BUDGET=9 STUB_HANG_AILANG=1`) logs `deferred 2 row(s)` while **3** rows land back in the spool. The well-formed cases are correct — budget-exhausted-at-row-1 preserves all 3 rows and reports 3. **Why it is worth a row despite low likelihood:** the drop is pre-existing behaviour, but the *counter* is new, and a diagnostic that exists specifically for when the notice plane has already failed is the worst possible place for a number that lies. Scope it to the counter plus a decision about whether a malformed row should be dropped at all, with a test that kills the mutation. **Deliberately NOT fixed in-flight at iteration 349:** a one-line edit to a just-merged green production path, with no new test and no second review, is the un-reviewed change this loop keeps closing.

**[NEXT] [iter-345 weekly external-issue sweep, Gate 0 rule 5; batched, NOT prioritised above anything] `sweep-2026-09-07-orphan-issues` — 11 open issues have ZERO mentions across the charter, the log, the STATUS archive and the dashboard, so nothing in this mission's memory knows they exist.**
Sweep verdict, published per-issue rather than as a summary: **14 orphans of 100 enumerated**, enumeration proven COMPLETE against a paginated `gh api repos/.../issues?state=open` count of **100** (rule (c) — the list, not just the pattern), known-tracked positive control `#1046` reading **3** in the charter, and a fresh negative control literal firing at **0**. Three of the 14 are mission bookkeeping THREADS, not work items, and are correctly absent: `#1072` (V1, this week), `#1078` (motoko), `#979` (docs). The remaining **11** are real and unheld:
`#1075` pin-root.sh redirects a DE-FORKED mission out of its own repo · `#1043` mission_pi_run.sh wires neither containment extension · `#1042` resolve-role-spawn.sh return-value defect · `#981` Gate-0 directive watermark advanced by fires that did not triage · `#978` driver notice episode-gating · `#683` `make fmt-check-ail` enumerates a directory that does not exist · `#678` eml attachments: RFC 2047 filenames · `#654` verify-examples' manifest gate prints a total it did not measure · `#651` design-quorum's zero-signal guard is vacuous · `#636` `ailang publish --dry-run` truncates all three digests · `#633` registry-validator vendor-namespace authorization.
**Note the shape of the set, because it is the finding.** Eight of the eleven are defects in the mission loop's OWN machinery — the pin, the pi runner, the spawn resolver, the watermark, the notice gating, the quorum guard. They are exactly the class this loop is best placed to fix and worst placed to notice, because they arrive as issues rather than as reds and nothing routes them into the charter. Triage-lite each when picked (ghost-discipline the repro at HEAD → verdict comment → queue-or-close); a sweep row never outranks an existing pick on its own, and only a confirmed soundness or regression finding inside it may.
