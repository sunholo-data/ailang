...
- **[NEW iter-105] m-nightly-flake-guard** (NEW-DOC needed; P2, ~0.5–1d) — the nightly eval regression detector has **no variance/flakiness guard**: it compares this run against the single previous run (N=2 trials each) and files a GitHub issue on any pass→fail flip, so a benchmark that is merely *bimodal on a local model* generates recurring false alarms. Evidence: `json_parse` on `opencode-qwen3-5-35b-a3b-mxfp8` has now produced **four** nightly issues (#286, #292, #480, #485), all closed as noise; its banked history is 1/2 → 2/2 → **0/2** → 2/2 → **0/4** (07-23…07-27), i.e. the 07-25 and 07-27 alarms are the SAME flip and 07-25 self-recovered with zero action. Each false alarm costs a Gate-0 triage slot and, left open, reads to external viewers as an unresolved regression (the #417 problem). Fix direction: require a **sustained** multi-night drop (or a per-benchmark flake-rate/CI-style confidence gate) before filing, and label a single-night flip as `suspected-flake` rather than `regression`. Also fold in: compare like-for-like CONDITIONS — #485 compared a run against yesterday's `_rag_on` dir while today produced both `_rag_on` and `_rag_off`.
