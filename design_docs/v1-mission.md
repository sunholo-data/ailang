...
**[NEXT] [iter-298 Gate-0 inbox triage, characterised but NOT root-caused] m-coordinator-child-env-opencode-retry-storm**
...
&mdash; **So the remaining suspect is the environment the coordinator hands its CHILD process, not the daemon's own.** That is un-root-caused and is deliberately recorded as such rather than guessed.
...
&mdash; Needs a repro at HEAD before routing; the retry loop should also be bounded regardless of the root cause, since an unbounded retry against a permanent condition is its own defect.
