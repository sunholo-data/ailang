...
| D-23 | OPEN | **Does `D-16`'s fast-forward authorisation extend to the case where local `dev` is AHEAD but every ahead-commit is content-duplicated upstream?** `D-16` authorises `git merge --ff-only origin/dev` in the main checkout at **0 commits ahead** with provably disjoint dirty files. Every iteration that lands its record by PR ends **2 ahead** — the squash-merge upstream carries the same content under a different SHA — so the letter of `D-16` is never satisfiable again and the checkout drifts monotonically, which is exactly the one-way divergence the Repo Profile warns makes the RUNNING skill stale for all three missions. Measured this iteration: 2 ahead; charter, log, archive and dashboard all **byte-identical** to `origin/dev` (`rev-parse dev:<f>` == `origin/dev:<f>`); `SKILL.md` differed only because origin had **36 insertions / 0 deletions** from motoko's `#819`, i.e. a strict superset; dirty ∩ incoming **empty** with the control firing at 25 incoming files. I restored `SKILL.md` from origin's blob (the protective per-path remedy the skill already prescribes) so the live copy is current, but the branch pointer is still 2 ahead and next iteration inherits the same state. **One word: `yes` (controller may `git checkout -B dev origin/dev` once it has MEASURED that every ahead-commit's content is present upstream and dirty ∩ incoming is empty) or `no` (keep asking per instance).** | Iteration 246. Ahead-commits `da771df73`, `27ba38876`; both squashed upstream into `eadad9fef` (PR [#820](https://github.com/sunholo-data/ailang/pull/820)), which is why a `patch-id --stable` match is unavailable and content-identity is the only checkable form. |
<!-- decision-ledger:end -->
