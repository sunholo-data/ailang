...
**0a. [PARKED — needs-human-review on `D-35` (module boundary) — P1, cross-mission blocker, `D-33`] `#764` — a protocol-only serveapi module**
...
times, `absent_reviewers` EMPTY both times**, four objections, all four answered). Parked on
...
open. Not parked on design direction: no reviewer has disputed that a protocol-only package should
exist, across four reviews. Not `PARKED-ON-LANE`: nothing here unblocks on a clock. **Resume
predicate:** `D-35` answered → route straight to `sprint-planner` (the doc is planner-ready; the
only text that changes under (b) is the packaging milestone).
