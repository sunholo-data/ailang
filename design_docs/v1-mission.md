...
- **(G4) gemini REPO-MOUNT upgrade** — **[PARKED needs-human-review iter 51 — Mark greenlit clone-over-egress
  (#399 2026-07-18); Phase-2 DECOMPOSED + 2-round quorum-hardened, BLOCKED on 2 convergent objections
  (typed `RequiresEgress`/`CapNetworkEgress` gate vs `Metadata`; bounded-execution/timeout-reuse). Doc PARK-NOTE
  is decision-ready — one designer pass applies both fixes then it's plan-ready. Log 56.]** iter-45 refuted the `repository`/`inline`
