...
| D-51 | OPEN | **Ratify or replace the charter's countable finish-line unit.** Iteration 309 added one because the comms contract requires every report to state distance to the finish line, and the charter had none; it is explicitly marked PROVISIONAL pending your ruling. The unit is **open queue rows** — measured at iteration 311: **68** `[NEXT]` + **1** `[PARKED]`, distance = that reaching zero. **The problem is that it is anti-correlated with good work.** Iteration 311 landed a milestone, fixed a real encoding defect and corrected two earlier iterations' framing, and moved the number by **0** — while an iteration that files five triage rows moves it *backwards*. Options: **(a) RATIFY as-is** — cheap, derivable, and every report keeps saying "provisional"; **(b) REPLACE with milestone burn-down** — count open milestones across `[IN-SPRINT]`+`[NEXT]` design docs rather than rows, which is what iterations actually move (recommended: it is still derivable, and it would have read "1 of 5" today instead of "0"); **(c) REPLACE with an outcome unit you name** — e.g. eval pass-rate on a fixed set, cost-per-verified-success, or a dated release milestone; **(d) DROP the unit** and let reports say "no finish line", accepting that the Progress row becomes narrative. **Loop's recommendation: (b).** **Default if unanswered:** the loop keeps quoting (a) marked provisional and re-asks nothing — this row simply stays OPEN, which is the status quo and costs nothing but leaves every Progress line misleading. | Charter Goal block (added iteration 309, `## Goal — the countable unit (PROVISIONAL, ...)`), which itself says "Mark to ratify or replace" and "defining the true finish line is a judgment only Mark can make". Measured at iteration 311 with `grep -cE '^\*\*\[<TAG>' design_docs/v1-mission.md`. This row exists because that ask was written as charter prose and never as a ledger row, so `mission_decisions.sh --open` returned zero and no report ever surfaced it. |
| D-52 | OPEN | **Is the ~40% reaped-slot rate worth an iteration to diagnose?** Of iterations 296–310, **six** (299, 300, 302, 303, 306, 307) produced no record at all — `git log -S "ITERATION <n>"` shows **0** charter commits for each, so they are dying mid-flight, not losing their record to a rotation bug (that half was audited clean this iteration: 270 archive stamps, every other iteration accounted for). The loop can see the frequency and cannot diagnose the cause: a slot that dies exits `rc=0`, so neither watchdog fires and the next iteration inherits a silently skipped number. Recovery costs a whole later iteration each time (308 spent itself recovering 306 and 307). Options: **(a) SPEND ONE ITERATION** on it — instrument the driver to write a heartbeat artifact per gate so a dead slot names the gate it died in (recommended; it is the one thing that would turn six anecdotes into a mechanism); **(b) ACCEPT the rate** as the cost of unattended operation and keep recovering opportunistically via the Gate-2 died-mid-flight traces, which do work; **(c) REDUCE THE FIRE RATE** so fewer slots overlap, trading throughput for completion. **Loop's recommendation: (a)** — the traces recover work but never explain it, and the rate has not fallen. **Default if unanswered:** (b), i.e. nothing changes; the loop keeps reporting the rate each iteration and keeps recovering by hand. | Measured iteration 311, first-party: charter+archive membership for iterations 296–310, with `git log -S` distinguishing "never written" from "written then removed". Controls fired (ITERATION 310 present in charter = 1; ITERATION 295 present in archive = 1). Prior instances the loop recorded but never aggregated: iterations 306 and 307 (recovered by 308), iteration 302 (recovered by 305), and the standing-rule-7 background-reaper class documented in the skill. |
<!-- decision-ledger:end -->

## Goal — the countable unit (PROVISIONAL, added iteration 309; **Mark to ratify or replace**)

The 2026-08-31 comms contract requires every report to state **distance to the charter's finish
line in the charter's own countable unit**. This charter had no such unit, and the skill makes
that omission a standing Gate-5 process-fix trigger, so iteration 309 added one rather than
writing "no finish line" a second time.

**Unit: open queue rows.** Measured at iteration 309: **53** rows tagged `[NEXT]`,
**2** `[PARKED]`, **29** `[LANDED]`. Distance to finish = the `[NEXT]` + `[PARKED]`
count reaching zero. Every iteration reports the delta it moved.

**This is a placeholder chosen because it is DERIVABLE, not because it is right.** Counting queue
rows measures backlog burn-down, and the mission's real goal is a self-specializing harness whose
success is not obviously a row count — a single row can be worth fifty, and the loop files new rows
as fast as it closes them, so this number can legitimately go UP during good work. Defining the
true finish line is a judgment only Mark can make. Until he rules, reports quote this unit and say
it is provisional. If he prefers a different unit (benchmark pass-rate, cost-per-verified-success,
extension count, a dated milestone), it replaces this block wholesale.

## STATUS 2026-09-01 — ITERATION 313: **The judge refuted the executor's reason for a deviation my own directive had authorised.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == `origin/dev` verified `cmp`-identical (355,904 B both sides) against the **RESOLVED** symlink target (`readlink -f` → main checkout, inode `60291442`; the pin's own copy is `61105181` — a different file, so the relative-path form would have compared the wrong one). Canonical cloud inbox with the invalid-store control FIRING (`unknown message store mode`, rc=1); **20 unread** triaged, none directive-class — the `pkg-sunholo-ailang-parse` traffic is another repo's package feedback and `mission-world`'s two are a sibling's approval request plus a skill proposal iteration 311 already answered. **0 directives** on #972 AND on #852 (both watermarks read, `2026-08-31T20:35:15Z`). Ledger **52 rows, 2 OPEN** (D-51, D-52 — both awaiting Mark, neither blocking this pick). No rotation owed: today is **Tuesday**, #972 was created after Monday's 07:00 local boundary. dev HEAD `b926c6a84` **checks=13, zero failures, five still pending** — in flight, so neither green nor red, and not a pick. Main checkout is **8 behind origin/dev with 4 dirty files** — the standing one-way divergence; route-around applied (record written in a worktree branched from `origin/dev`), reconcile still a human decision. **Pick = the IN-SPRINT row's Sprint 1 M3**, continuing the sprint rather than the queue head. Confirmed unlanded first-party (`BuildModuleIface` **0**, `PublishLimits` **0**, `iface_subprocess.go` ABSENT; same-scope controls `InterfaceHash` **18** and `setProcessGroup` **5**; fresh negative literal **0**; all roots `test -d`-asserted) with M1 and M2 both present. Died-mid-flight sweep: 3 open PRs on the fleet account, every one attributed via `git worktree list` — #997 docs-mission's, #971 and #945 V1's own carried drafts — no orphan. **I APPLIED M2's LESSON BEFORE THE DEFECT, NOT AFTER.** M2 died on an unimplementable import direction, so before routing I measured this one: `internal/iface` does not reach `internal/pkg` (**0** of **104** transitive deps, control **13** internal imports), so `internal/pkg → internal/iface` is safe. It was. **AND MY OWN BASELINE INSTRUMENT WAS BROKEN, CAUGHT BY ITS OWN CONTROL.** Baselining the gate list (the false-green #4 rule aimed at my own directive), I asserted `--- PASS:` lines and read **0** for every gate — including the known-present control `TestInterfaceHash`, which cannot be absent. `--- PASS:` is printed **only under `-v`**; without it a genuinely passing test yields rc=0 and zero PASS lines, indistinguishable from `[no tests to run]`. Re-measured with `-v`: control **5** PASS lines rc=0, non-matching regex rc=0 / **0** PASS lines, and all six M3 gates RED at base. Had the control not fired I would have shipped a gate list that could not distinguish a passing suite from an empty one. Executor `codex:gpt-5.6-sol` (probe rc=0), sandboxed worktree, no git writes, rc=0, 375 LOC across two files; it correctly labelled the whole-package gate `UNINFORMATIVE UNDER SANDBOX` (`httptest` could not bind `[::1]:0`) — green when I re-ran it outside. Containment clean: main checkout held exactly its 4 pre-existing dirty files at start and end. **My own diff-anchored drill (rule 3n) found two hunks the executor's report did not name**: `cmd.Dir`, `setProcessGroup` and the `PublishLimits` defaults are each SOLE killers, while the `ctx.Err()` early return and the `filepath.Abs` normalization survive neutering with the whole package green. **THE JUDGE (sonnet, its own worktree) RETURNED PASS 79/100 WITH TWO BLOCKING FINDINGS, AND THE FIRST ONE IS MINE.** (1) The executor deviated from acceptance criterion M3-A5 — comparing the wrapper against the same `internal-dump-iface` subcommand it invokes, i.e. the mechanism against itself — citing an import cycle. The cycle is real for an **internal** test (`package pkg` importing `internal/pipeline` → rc=1 `import cycle not allowed in test`) and **false** for an **external** one (`package pkg_test` → rc=0, named PASS): Go links both into one test binary, so the existing `TestMain` override still applies. Both arms measured first-party, the control is what makes it a finding. **This was a CONTROLLER error, not executor non-compliance** — my directive explicitly authorised the fallback and never named the external-test-package route, so the executor complied exactly. (2) `resolveIfaceBinary` had **no coverage on EITHER branch**, not merely the fallback the executor disclosed: replacing its entire body with an always-failing stub left all six named arms PASSing (**9 PASS 0 FAIL**), and that function decides which binary gets exec'd. Plus a new non-blocking finding: the export-limit `>` → `>=` mutant reddened **nothing**, because only the over-limit case was exercised. **All three reproduced first-party before I acted, and all three fixes are PROVEN rather than asserted** — each mutant asserted LANDED (sha256 delta) and BUILDS (rc=0), red set read by test NAME, source restored from a **copy** and confirmed byte-identical: whole resolver body neutered went *nothing red* → **SOLE killer** `TestResolveIfaceBinary_FallsBackToPath`; export limit `>=` went *nothing red* → **SOLE killer** `TestBuildModuleIface_ExportLimitBoundary`; the inverted `isAilangBinaryPath` predicate reds three arms; `cmd.Dir` remains sole-killed by `MatchesInProcess`. **One over-strict assertion of my own, caught by running it rather than by reasoning:** my new canonical test first asserted `got.Module` EQUALITY, which pins the fixture's LOCATION, not the mechanism — inside `t.TempDir()` the compiler auto-relaxes MOD010 and reports the temp-qualified path. Pinned as a suffix instead. Controller re-ran every gate OUTSIDE the sandbox: `internal/pkg` **256 PASS 0 FAIL**, scoped build/vet/boundaries/file-sizes/changelog all rc=0, gofmt 0 files, `internal/pipeline`/`internal/iface`/`cmd/ailang` all rc=0. **`go build ./...` is rc=1 at base** (`cmd/wasm`) and was deliberately kept out of the gate list. Also corrected the sprint plan's own D2 reasoning in the record: its conclusion (return `*iface.InterfaceJSON`, not `*iface.Iface`) is right, its stated premise — "no deserializer exists" — is **false**, `internal/types/json.go:723` defines `UnmarshalScheme`; the conclusion survives because that function consumes the types package's Scheme JSON, which the canonical interface JSON does not carry. And doc **AC14** (`TestBuildModuleIface_ModulePathResolution`) was absent from the plan's M3 list — the doc wins on scope, so it shipped. Metered **$0.00** of the $5 ceiling; designer and planner not spawned (doc and plan both existed). Three residuals declared rather than papered over: `ctx.Err()` and `filepath.Abs` have no killer, and `PublishLimits.Overall` has no consumer until publish orchestration lands. → PR [#1000](https://github.com/sunholo-data/ailang/pull/1000)

## STATUS 2026-09-01 — ITERATION 312: **The sprint plan's M2 was unimplementable, and the judge then caught a regression I had shipped in my own fix.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == `origin/dev` verified against the **RESOLVED** symlink target (`readlink -f` → main checkout, inode `60291442`; the pin's own copy is `61105181`, a different file — the relative-path form would have compared the wrong one). Canonical cloud inbox with the invalid-store control FIRING (`unknown message store mode`, rc=1) and the `store: gcp (Firestore, project ailang-multivac)` header present; 20 unread triaged, **0 directives** on #972 (watermark `2026-08-31T20:35:15Z`); package-feedback traffic is `sunholo-data/ailang-parse`'s and already triaged there (issue #40) — not V1's repo. Ledger **52 rows, 2 OPEN** (D-51, D-52 — both filed by iteration 311, both still awaiting Mark). No rotation owed: #972 created `05:56:11Z` = **07:56 CEST**, AFTER Monday's 07:00 local boundary; 8 comments. dev **16 checks**, sole non-green **SonarCloud**, and it is **INHERITED** — `failure` on all five of `origin/dev~0..~4`, `absent` at `~5` — two of those commits docs-only, so it is not this iteration's doing; it is NOT a required context (required = `test`,`lint`,`build`,`docs-gate`), failing on *72.4% coverage on new code* and *B security rating on new code*. Named, not picked. **Pick = the IN-SPRINT row's Sprint 1 M2**, continuing iteration 311's sprint rather than the queue head. Confirmed unlanded first-party (`BuildCanonicalJSON` **0**, `internal-dump-iface` **0**, same-scope control `outputInterface` **2**, both roots `test -d`-asserted, M1's `HashProjection`/`SignatureSet` both present). Died-mid-flight sweep: 3 open PRs on the fleet account, all attributed — #997 is docs-mission's (`sprint/iter3-docs-5`), #945 is V1's deliberate iteration-296 draft, #971 is V1's iteration-308 draft already carried as a queue row (charter **1** / log **2** mentions) — no orphan. **THE REFUTATION.** The sprint plan (opus planner, iteration 310) and the design doc both specify `iface.BuildCanonicalJSON` in `internal/iface`. It is **unimplementable**: `internal/pipeline` imports `internal/iface` in **7** non-test production files, so the reverse import is a cycle. The first executor run (`codex:gpt-5.6-sol`, probe rc=0) **refused the milestone and said why** — 0 files changed, rc=0 — which is the directive's "STOP and say so" path working, and I treated it as a claim rather than a verdict: measured two-arm in a throwaway worktree, `go build ./internal/iface/` is **rc=1 `import cycle not allowed`** with the import and **rc=0** without, worktree restored to 0 dirty. This survived **two quorum rounds × three external reviewers** (both rounds `blocked`, 3/3 reject, `absent_reviewers` `[]` read via the corrected `.synthesis.` path), the opus planner's re-scope, and iterations 310 and 311. **The corrected home was derived, not chosen:** `internal/pipeline` already imports `iface`, already owns `RunWithContext` (`pipeline.go:154`) and `Config` (`DryLink` + `PackageDir` both present, 29 fields); `cmd/ailang` already imports both; and it need **not** be importable by `internal/pkg`, because `internal/pipeline` transitively imports `internal/pkg` — which is exactly why the plan's M3 is a subprocess wrapper. M3 is unaffected. I also refuted a round-2 quorum objection in passing: gemini blocked on "M2 runs `pipeline.Run` in-process inside `registry-validator`" — measured **0** `pipeline.Run` and **0** `internal/iface` imports there (same-scope control **73** `^func `), it shells out at `validate.go:76/:95/:116`, and M2 touches the `ailang iface` CLI only. **THE REGRESSION I SHIPPED AND THE JUDGE CAUGHT.** Run 2 delivered M2 clean on every gate I had baselined, and I found — reading the diff, not from any gate — that `outputInterface` now passed `packageDir="."`. `packageSearchDir` lets an explicit packageDir **win outright**, defeating the entry-file-directory anchor added for ailang#671. Two arms on the identical fixture from the repo root: pre-refactor binary **rc=0 / 235 bytes**, post-refactor **rc=1 / 0 bytes**. The shipped golden test could not see it for **two independent reasons** — it calls `BuildCanonicalJSON` directly rather than through `outputInterface` (rule 3k: it verifies the arithmetic, never the artifact), and its fixture has no intra-package imports. Fixed with a named `outputInterfacePackageDir = ""` constant the test consumes, and the mutant `= "."` builds, reds **only** the new arm, and leaves the golden passing. **Then the judge (sonnet, its own worktree) FAILED round 1 at 66/100** with three blocking findings, all reproduced first-party before I acted: the whole `internal-dump-iface` dispatch arm was deletable with **zero** test failures; `iface --compact` and the trailing newline were both unpinned; and my own commit message asserted byte-identity generally on **one** measured input. Round 2 added four tests that drive a **real binary** via the existing `buildAilang`/`runAilangBin` helpers from the repo root, and all three surviving mutants now RED — each asserted LANDED (sha256) and BUILDS (rc=0) first, and each read for WHICH test failed. **Round 2 scored 95/100 PASS with all three findings independently confirmed CLOSED**, including an adversarial sweep of the byte-identity contract across absolute paths, `.ail`-suffixed module paths, missing files and a real type error — zero disagreements. Controller re-ran every gate OUTSIDE the codex sandbox (mandatory, the diff touches `cmd/*`): scoped build/vet rc=0, boundaries rc=0, file-sizes rc=0, gofmt 0 files, `internal/pipeline` **819 PASS 0 FAIL**, `internal/embed` 68, `internal/apiserver` 356, hidden-vs-public byte-identity diff empty. **`go build ./...` is rc=1 at base** (`cmd/wasm`) and was kept out of the gate list — the false-green #4 rule aimed at my own directive. Containment clean throughout: main checkout held exactly its 4 pre-existing dirty files at start and end. Deviation adjudicated by measurement, not by the executor's word: the `ctx.Err()` early return in `RunWithContext` is STRICT — all 10 call sites pass `context.Background()` or a live request ctx, **none nil**. Metered **$0.00** of the $5 ceiling; designer and planner not spawned (doc and plan both existed). Two honest residuals filed rather than papered over: the compact test pins shape not content, and `Config{DryLink:false}` has no killer (pre-existing, moved verbatim, but M3 will call the library directly).

## STATUS 2026-08-31 — ITERATION 311: **A judge found a collision in the encoding I prescribed, and the fix is the milestone's only interesting line.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == origin verified against the **RESOLVED** symlink target (inode `60291442`, distinct from the pin's own `60316994`). Canonical cloud inbox with the invalid-store control FIRING (`unknown message store mode`, rc=1); 20 unread enumerated, **0 directives** on #972 or #852 (both watermarks read, rotation-week catch applied); ledger **50 rows, 0 OPEN**, read AFTER the origin fetch and cross-checked against `origin/dev`. No rotation owed &mdash; #972 was created `05:56:11Z` = **07:56 CEST**, AFTER this Monday's 07:00 local boundary. dev **16 checks green** at `3ad55d53b`; the sole non-green is SonarCloud, and it is **inherited** (same `failure` on parent `bd17d9643`), so not this iteration's pick. **Pick = the IN-SPRINT row's Sprint 1 M1**, not the queue head: iteration 310 landed both the design and an opus sprint plan, and M1 is a pure function &mdash; the fastest feedback on the one decision the sprint rests on. Confirmed unlanded first-party (`HashProjection` **0**, `SignatureSet` **0**, same-scope control `func .*ToNormalizedJSON` = **1**). **I baselined the whole gate list myself before routing** (the false-green #4 rule aimed at the controller's own directive): all four new gates measured RED, `TestXModAlias` GREEN with exactly 2 PASS lines, build/vet/boundaries rc=0 &mdash; and the load-bearing one, `go test -run <non-matching-regex>` exits **0** with **0** PASS lines, so every gate asserts `--- PASS: <Name>` AND rc=0. Executor `codex:gpt-5.6-sol` (probe rc=0), sandboxed worktree, no git writes, rc=0 with a complete diff; containment clean (main checkout carried exactly the 3 pre-existing dirty files it had at Gate 2). **Controller re-ran every gate independently and added three diff-anchored mutants the plan's table does not name** (rule 3n): the funcs sort and the effects sort are each SOLE-killed by `TestHashProjection_Deterministic`; the defensive `Params` copy has **NO KILLER** &mdash; reported and declared in the code rather than given a test that could only assert an implementation detail. **The evaluator is what made this iteration worth more than its diff.** `sonnet`, its own worktree, **PASS 92/100, zero blocking** &mdash; and its first non-blocking finding was real: the `module:kind:name:signature` encoding **I** prescribed is **non-injective**. Reproduced first-party: `{Type:"A:B", Effects:[]}` and `{Type:"A", Effects:["B:"]}` both render `mod:func:run:A:B:`. M5 diffs these sets to decide ADDED vs REMOVED vs RETYPED and M7 will feed them from untrusted uploads, so a collision is a wrong release classification. Fixed in-milestone (the function is new, unwired and reversible) with `escapeSigField`, plus `TestSignatureSet_EncodingIsInjective` pinning the judge's own pair &mdash; and the guard is PROVEN, not asserted: the pre-fix encoding restored as a mutant LANDED (sha256 delta), BUILT (rc=0) and reds the named arm on the exact assertion, blast radius **2** (`SortedAndStable` also reds because its golden now carries the escaped comma &mdash; the mutant's phenotype, explained, so the `-skip` rc=0 inverse is correctly **not** applied here). Two more judge findings acted on: **anti-vacuity floors** added to the two arms whose fixtures could silently disable them (an alphabetised `Ctors` fixture made a real aliasing bug invisible &mdash; measured), both floors confirmed to FIRE; and my own `Params` comment was **overstated** and corrected (the guarantee is structural, since `HashProjection` returns only `[]byte`). Judge findings deliberately NOT acted on, with reasons: alias-body retype invisibility is design doc **D6**'s named, accepted scope limit, and a CHANGELOG entry belongs with the sprint's user-visible landing, not with unwired dead code (`make check-changelog` rc=0). **Gate-5 skill fix, bar met at 3 instances and this one CORRECTS the previous two:** iterations 309 and 310 both recorded that `absent_reviewers` "does not exist". It does &mdash; at `.synthesis.absent_reviewers`, present in **22 of 22** artifacts (control `has("synthesis")` = 22), written by `quorum.go:51`; and the verdicts live at `.reviewers[].result.verdict`, not `.reviewers[].verdict` (**0 of 22**). So the writer was never at fault, no code fix is owed, and the skill's documented `jq` path returned `null` &mdash; the vacuous pass the rule exists to prevent, reached by obeying the rule. **Bookkeeping:** PR #968 (CONFLICTING since 08-30) closed as superseded after rescuing the one genuinely orphaned file it still owned &mdash; the 57,990&nbsp;B retry-storm design doc, absent from origin with **0** charter/log mentions while its own sprint plan had landed. **STATUS rotation audited and HEALTHY** (270 archive stamps; the 6 missing iterations 299/300/302/303/306/307 are reaped slots, `git log -S` = 0 charter commits each &mdash; not rotation loss). metered=**$0.00**.
...
**[IN-SPRINT &mdash; iter-313: Sprint 1 **M1+M2+M3 LANDED**; M4&ndash;M5 remain (~2d)] `m-registry-interface-hash-blind-to-signatures` &mdash; a BREAKING API change is classified `patch` to every downstream consumer.**
