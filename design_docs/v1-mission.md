...
Deliverable now: fix by class, cheapest first (4 and 3 are pure additions to the list enumerator),
re-running the per-file scan after each so the residue is measured rather than predicted.
