...
- **m-check-strict-fallbacks** (1d) **[PARKED needs-human-review, iter 41]** — pick-time quorum did
  not converge in its one bounded round. Doc HARDENED (Premise Verification: corrected `internal/check/`
  → `internal/pipeline/` per the `warn_split_args.go` precedent). Residual = ONE open design decision
  the sprint's planner must resolve first: the pass needs pre-elaboration typed-AST info (`FuncDecl` +
  `Result[_,_]` return type) but `warn_split_args` only offers a post-elaboration Core hook — option (a)
  verify/add a pre-elab hook, or (b) thread return-type info into a Core pass. Re-quorum (now 2-reviewer,
  quorum tool fixed this iter) once resolved, then route. See log entry 44.
