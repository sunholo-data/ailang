...
## STATUS 2026-07-28 — ITERATION 113: **`m-nightly-flake-guard` LANDED END-TO-END (PR #504, squash `038d9322d`, dev CI green) — the nightly detector now has a variance guard, and its non-vacuity is PROVEN rather than asserted.** The detector compared tonight against the SINGLE previous run (N=2 trials) and filed an issue on any pass→fail flip; the real cost was **45 `[nightly-eval]` issues across 24 distinct benchmarks, all closed** — ~11× the 4 the design doc claimed, with **two more (#499, #500) firing on 07-28 while the sprint ran**. Planner **opus** → executor **`codex:gpt-5.6-sol`** → evaluator **sonnet PASS 87/100 r1, zero blocking**. Shipped: classifier extracted to `tools/nightly_classify.py` (behavior-preserving, byte-identical over all five surviving nights) · history off `/tmp` onto a JSONL with atomic write + **ownership-checked lock** (PID + random token, `os.kill(pid,0)` liveness gating any steal) · trailing-window solidity (W=5, MIN 2 nights/4 trials) with `suspected-flake`/`insufficient-history` labels where only `REGRESSION` reaches `--type bug`. **Replay over the REAL corpus: `filed=5 → guarded=2, suppressed=4`** — and it still barks: **#483 `higher_order_functions` (4/4 solid window) pages the SAME night**, `csv_to_json_converter` newly escalates on 07-27, an alarm today's detector never raised. **The planner refuted 6 premises, two load-bearing** — including one of the controller's OWN measurements: `/tmp` **directories outlive their contents**, so 07-22/07-23 are empty shells and only FIVE nights of data exist (I had counted dirs and reported seven), which also caps `--bootstrap` below a full W=5 window; and **`make ci` is run by NO workflow**, so the doc's M4 CI wiring would have been **completely vacuous** — the exact iteration-110 failure mode. The tests therefore attach directly to `ci.yml`'s test job with a companion step that fails under 20 `--- PASS:` lines, and **the Actions job log proves they ran** (40 PASS at log line 31949, incl. `test_Lock_stale_but_alive_holder_is_not_stolen_from`) — closing the one AC the executor had to leave UNINFORMATIVE because it cannot push. **Controller mutation-proved the guard in THREE directions**, each reverted byte-identical (`sha256 a53f1e3a…`): suppress-everything → 3 red incl. the same-night test; suppress-nothing → 6 red; **liveness-blind lock steal → 2 red incl. the stale-but-alive test**, which is quorum round 2's objection genuinely closed (the tests use real `subprocess.Popen` holders, **zero mocks** — a mocked test passes against the naive mtime-only steal, the plan's named #1 way this could ship silently broken). Deviation, approved at plan time: `consec >= K AND not already-regressed` replaces the doc's literal `consec == K`, which loses the escalation FOREVER on a missed night. ⚠ **Landing action for Mark**: run `python3 tools/nightly_classify.py --bootstrap` once before the next 05:00 nightly, else night 1 is loudly DEGRADED and files zero issues. `metered=$0.00`.**
