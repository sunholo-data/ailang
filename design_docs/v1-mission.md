...
- **[NEXT — filed 2026-08-22 (iter-253) from the M4b cohort itself; ~1–2d, P1 for the v1.0 claim]**
  `m-contract-verification-coverage` — **the KPI's denominator is suppressed by Z3 SKIPPING, not by
  models writing wrong programs.** Classifying all 30 M4b runs: **14 partial** (Z3 skipped ≥1
  function), **8 counterexample** (contract genuinely violated), **4 verifier error**, **3 fully
  verified**, **1 did-not-compile**. Across the cohort **53 functions were SKIPPED against 28
  VERIFIED** — nearly 2:1. Because `isVerifiedSuccess` requires `verify_skipped == 0`, a single
  skipped function disqualifies an otherwise-correct run, which is why **14** runs verified
  *something* and only **3** verified *everything*. This is a statement about AILANG's own contract
  encoder on the exact surface the v1.0 "verified AI-orchestration language" claim rests on: the
  cheapest available lever on the headline KPI is raising verification COVERAGE, not model quality.
  **Not yet diagnosed** — the next step is to enumerate WHY functions skip (unsupported sort?
  recursion depth? `--verify-timeout 5s`?) from the banked `verify.results[].status` rows, which the
  cohort has already captured. Related: `#513` (per-call bounds do not bound a whole run).
