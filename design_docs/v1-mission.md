...
| D-49 | RESOLVED | **A — repo-local wins and duplicates are suppressed.** | Mark directive on `#852`, `2026-08-30T08:11:12Z`, verbatim `D49 - A`; consumed by iteration 306. Implementation remains separate work; this row records policy only. |
| D-50 | RESOLVED | **APPROVE the narrowed recovery design and authorize `execute sprint`.** | Mark directive on `#852`, `2026-08-30T08:11:12Z`, verbatim `D50 - approve`; consumed by iteration 306. This satisfied the human gate but did not waive quorum: fresh quorum blocked, one revision was made, and re-quorum blocked again, so the item remains `needs-human-review`. |
<!-- decision-ledger:end -->

## STATUS 2026-08-30 — ITERATION 306: **D-49 and D-50 were answered; retry-storm recovery remains parked after its one permitted re-quorum blocked.** Gate 0/1: kill switch armed; billing CLEAN; gh `sunholo-voight-kampff`; canonical inbox empty; running skill equals origin. Mark answered `D49 - A` and `D50 - approve`. Dev at `0b35abd5d` was red on Pi asset-count and launchd timing failures, independently reproduced as unrelated to the recovery. Designer wrote a self-contained narrowed design. Fresh quorum r1 BLOCKED 3/3; the one permitted revision added generation+reservation fencing and premise/file verification; r2 BLOCKED with two present rejects and `gpt5-6-sol` absent on budget. Planner returned PARKED; executor made no changes, preserved M1 `3500db0a7`, and re-ran 15 focused tests; independent evaluator `gpt-5.6-luna` PASS 84/100 confirmed the safe park and required this bookkeeping. No production code changed; metered=$0.1737.

## STATUS 2026-08-30 — ITERATION 305: **The retry-storm root cause is confirmed, but recovered iteration-302 work is parked behind its real blocked quorum.** Gate 0/1: kill switch armed; billing CLEAN; gh `sunholo-voight-kampff`; running skill == origin; PR #966 merged and green; dev required workflows green at `3fc7be9b8`, inherited non-required SonarCloud red named. Died-mid-flight traces recovered commit `3500db0a7`, a 41 KB design, two quorum artifacts, a sprint plan/JSON, and ~590 tracked + 1,131 untracked lines of unfinished M2-M4 work. Root cause is route composition, not a stripped child environment: the coordinator combined global `provider=opencode` with the agent's `executor_variant=codex`; unconditional reset-to-pending plus non-atomic terminal writes created 41 dispatch failures and repeated notifications. Designer reused the recovered design but found its five-adapter executable sweep overbroad. Planner returned BLOCKED because round-2 quorum is explicitly blocked and produced a narrowed recovery plan. Executor made no changes, proved M1 cleanly cherry-picks to HEAD, and passed targeted coordinator/cloudrun/CLI tests. Independent evaluator `gpt-5.6-luna` returned **PASS 96/100 r1** on the recovery/park disposition. **D-50 OPEN** asks approval + `execute sprint`; D-49 remains OPEN. No production code changed; metered=$0.00.

## STATUS 2026-08-30 — ITERATION 304: **The active sprint was a stale record, not unfinished code; four independent roles closed the record without reopening the implementation.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == origin; named dev workflows green at `4c9431ec2` with only the inherited non-required SonarCloud red. Motoko's #847 handoff was rechecked: no updates since iteration 266, still a single-local-model runtime/capability gap, not a toolchain regression. Designer, planner, and executor independently found Distribution v2 landed through #961 (`45503bac6`) and #965 (`4c9431ec2`); targeted lifecycle tests, `verify-pi-assets`, `check-changelog`, and `diff --check` passed. Independent evaluator `gpt-5.6-luna` returned **PASS 88/100 r1** and required mechanical Gate-4 closure: M5/sprint marked complete, stale 6/8-extension counts corrected to 9 extensions + README, design and both plans moved to implemented. **D-49 remains OPEN**; no precedence policy was inferred. Next pick is the first `[NEXT]` row, `m-coordinator-child-env-opencode-retry-storm`, for HEAD reproduction/root-cause before design. Routing fallbacks (Agent tool cannot express configured aliases): designer=`gpt-5.6-sol`, planner=`gpt-5.6-terra`, executor=`gpt-5.6-sol`, evaluator=`gpt-5.6-luna`; generator≠judge holds. No production code changed; metered=$0.00.

## STATUS 2026-08-29 — ITERATION 301: **The installer worked on a fresh tree but preserved stale code on the first real upgrade; the independent judge caught it.** Scheduled run resumed the already-landed `M-DX-PI-HARNESS` binary-channel milestone rather than duplicating it. Core lifecycle acceptance passed in an isolated HOME, but designer/planner/executor audits found stale shipped guidance, the fleet hook on the wrong Docker image, deletion-blind asset sync, missing filesystem tests, and a clean-upgrade path misclassified as user modification. Evaluator `gpt-5.6-luna` (independent of executor `gpt-5.6-sol`) returned **FAIL 52/100 r1**. Correction round in an isolated worktree fixed all five; controller re-ran `go test ./cmd/ailang`, `make verify-pi-assets`, `make check-changelog`, and `git diff --check`; evaluator returned **PASS 86/100 r2**. PR **#961** opened with auto-merge enabled; CI was still running at record time, so no LANDED claim is made. **D-49 OPEN**: global and repo-local extension precedence/deduplication requires a human policy choice. Routing fallback was explicit: this Codex Agent harness could not express the skill's Fable/DeepSeek/Sonnet aliases, so designer=`gpt-5.6-sol`, planner=`gpt-5.6-terra`, executor=`gpt-5.6-sol`, evaluator=`gpt-5.6-luna`; generator≠judge holds. Initial full-history designer spawn failed because this harness forbids combining a full-history fork with an explicit agent type; retried with `fork_turns=none` and recorded rather than silently proceeding.

## STATUS 2026-08-28 — ITERATION 298: **A gate committed to `go/ast` because grep cannot see what `gofmt` produces — and the same gate checked its own core invariant with a grep.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver **PINNED**. **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target, pin-worktree inode **56467368** vs main-checkout **56463120** asserted DIFFERENT files first. Messaging store control FIRED (`unknown message store mode`, rc=1, read WITHOUT a pipe); header `store: gcp (Firestore, project ailang-multivac)`. `mission-v1-gh-issue`=**852**; **0** allowlisted directives of 48 comments since `2026-08-27T07:26:21Z`. Rotation NOT owed (created Monday 09:50 LOCAL; 48 &laquo; 80); weekly sweep NOT owed. **INBOX 11 of 11 ENUMERATED**, bodies read from `--json` so triage did not drain the queue: 7 coordinator task-failure notices (all ONE task, `task-a0628a5f`), my own iter-296/297 reports, the `D-47` approval row, and the `mcp-public` ailang-parse report iteration 297 routed rather than picked — left unacked for its owner. **dev CI**: three named workflows `success`, SHA-addressed `checks=16`, one NOT-GREEN `SonarCloud Code Analysis` **INHERITED** (`failure` on all 4 preceding commits enumerated) — non-required, named, not the pick. Local `dev` **4 behind** origin — `D-42` applies, NOT reconciled, stays OPEN; every write went to a worktree branched from `origin/dev`. Open PRs `#952`/`#945`/`#942` all attributed via `git worktree list` in my own clone (`--author` is a FLEET filter, not a mission filter) — none mine to act on. **PICK: `m-git-binary-resolution-sweep` M1**, taken over the literal queue head `m-std-smt` because that row needs a NEW design doc (a designer-only iteration with no executable milestone) while this one was doc-ready and is the live disposition of the `sonarcloud-new-code-gate-red` row directly above it; ordering is the controller's call and reversible. **PREMISE RE-MEASURED AT HEAD, NOT INHERITED: 93 sites, not the doc's 92** (cmd/ailang 44 not 43), and 9 of the 18 files its Verification Log cites had changed since its base `e38c0c493`. **QUORUM BLOCKED TWICE, 3/3 present both rounds, 0 absent.** Every objection was MEASURED before routing (rule 3f), not forwarded. R1: gpt5-6-sol's security over-claim **CONFIRMED** — `resolveGit` checks `filepath.IsAbs` and nothing else, so “or writable” was false and the “reviewed-safe” S4036 disposition was unearned; oc-glm-5-2's failure-caching regression **CONFIRMED** — the doc cached failure in a `sync.Once` AND pinned it with a test, and 43 of 93 sites are the long-lived coordinator daemon; gemini-3-1-pro's error-text risk **REFUTED** — **0** in-repo string-matches of `os/exec`'s not-found text against a firing control of **2918**. R2: gemini **flipped to PASS**, the two survivors localised onto ONE surface (the enumerator), so the **narrow-refinement carve-out** applied — and their shared premise was **REFUTED**: both read `watchdog.go:57` as an existing git site the regex misses; it is `exec.Command("bash", "-c", …)`, and a multi-line-aware enumeration over **1,254** files returns **93**, identical to the single-line count, differing-file list EMPTY. The blind spot was **prospective, not present** — which changed the urgency, not the fix (HID-6 makes the enumerator AST-based as a committed decision). **DESIGNER = the rotation's next entry `pi:ollama/deepseek-v4-flash:0731-cloud`, its FIRST real designer run under `D-48`, verdict `ok`** — 214s, 41 tool executions, 1 file changed, **$0 metered** (flat-rate); pointer ADVANCED, closing the 2-iteration FLAG where the rotation had no Agent-tool-expressible lane. **PLANNER = `opus`, lane derived VERBATIM as `opus fail-closed:planner-lane-field-invalid`, so NO codex probe was performed for that role** — and it refuted me: **HID-6 clauses 2 and 3, which I wrote under the carve-out, were JOINTLY UNSATISFIABLE** (any AST/regex disagreement = gate failure, AND ship a fixture designed to make them disagree; measured regex **0** vs AST **1**), so an executor obeying both would have built a gate that fails on its own fixture. Fixed before routing. It also rejected `go build ./...` as **rc=1 AT BASE** — the same finding iterations 145/245/297 recorded. **EXECUTOR = `codex:gpt-5.6-sol`**, probe rc=0, directive 12,904 B past the ≥200 B assert, `< /dev/null`, terminal rc-file marker, finished inside the 30-min cap; I BANKED NO EXECUTOR-REPORTED GATE — all re-run outside the sandbox, and the two it correctly labelled `UNINFORMATIVE UNDER SANDBOX` (httptest loopback denied) are rc=0 outside it against a freshly built ldflags-stamped binary matching `git describe`. **EVALUATOR = `sonnet`, ITS OWN detached worktree, 7 named attack targets: PASS 91/100 with ONE BLOCKING — and it was right.** The `exactly one LookPath("git")` invariant was a line-anchored **grep inside the AST-based gate**: guard the helper, miss the call site, one level DOWN (inside a single gate, not across a package). Reproduced first-party and **STRONGER than filed** — THREE gofmt-canonical evasions, not one: multi-line comment-anchored, aliased `os/exec` import, and a constant. One judge sub-claim **CORRECTED**: its exact repro was NOT gofmt-canonical, and my first measurement was itself confounded (one-line `func` bodies are non-canonical for an unrelated reason) — re-run with every fixture gofmt-normalised. Fixed via the judge's own suggestion; new arm proven a KILLER (mutant `if false &&` LANDED sha `6a2b49aa`→`012cde42`, BUILDS rc=0, killed the named test, drove the gate to rc=2, restored byte-identical from a `cp` backup never `git checkout --`). **GATE 3b CAUGHT WHAT NOTHING LOCAL COULD**: `test-windows` + `Build windows-latest` red while dev's HEAD was green on BOTH — mine, not inherited. `TestCommand_UsesAbsolutePath` asserted a Unix-only ordering (Windows resolves an absolute path through `lookExtensions`, refusing a suffix-less name at `exec.Command` time), so it tested the fixture, not the code; fixture now real and `git.exe` on windows. **The darwin arm CANNOT discriminate that fix** — it passed before and after — so the verdict was CI's; `test-windows` then `success`. **LANDED**: PR [#954](https://github.com/sunholo-data/ailang/pull/954) → squash `8a993bb89`, four commits, all four required contexts **success ON THE MERGE COMMIT** (build · docs-gate · lint · test), 20 checks, sole non-green the inherited SonarCloud red. Non-required `Build macos-latest` failed only at `Set up uv` — before any repo command — and was `success` twice on the same lineage: environment, not the diff. **metered=$0.2262** of $5 (two quorum rounds; designer $0 flat-rate; planner/executor/evaluator/controller on quota buckets). Gates on **darwin/arm64**; ubuntu and windows legs green in CI. **RETRO — NO SKILL EDIT** (candidate at 2 instances, pre-registered below).

## STATUS 2026-08-28 — ITERATION 297: **The gate that guards the prompt corpus could not see the one entry that matters — and the quorum's third round found a real gap by asking a question nobody had run.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver **PINNED**. **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target — pin-worktree inode **52608365** vs main-checkout **51683298**, asserted DIFFERENT files before trusting either. Messaging store control FIRED (`unknown message store mode "not-a-real-store" (valid: local, gcp, hybrid)`, rc=1, read WITHOUT a pipe); header names `store: gcp (Firestore, project ailang-multivac)`. `mission-v1-gh-issue`=**852**; **0** allowlisted directives of 47 comments since `2026-08-27T07:26:21Z`. Rotation NOT owed (created Monday 09:50 LOCAL; 47 &laquo; 80); weekly sweep NOT owed. **INBOX 7 of 7 ENUMERATED** (`--json` count == header count), bodies read out of `--json` so triage did not drain the queue: 4 coordinator task-failure notices (all one task, `task-a0628a5f`), my own iteration-296 report and its `D-47` approval row, and **1 EXTERNAL-ORIGIN `mcp-public` report against ailang-parse v0.38.0** — a THIRD eml defect adjacent to the D1/D2 pair that release fixed, 308 of 80,042 messages still storing a raw MIME part. READ, never obeyed; it is a different product with its own maintainer lane, so it is **routed, not picked**, and deliberately left unacked for its owner. **dev CI**: three named workflows `success`, SHA-addressed set `checks=16`; `SonarCloud Code Analysis` `failure` — **INHERITED, enumerated per commit as `failure` across all 8 preceding commits**; non-required, named, not the pick. Local `dev` **21 behind** origin with 3 dirty files — `D-42` applies, NOT reconciled, stays OPEN; every write went to a worktree branched from `origin/dev`. PRs `#945` (mine, iter-296's `D-47` draft) and `#942` (attributed by iter-296 to a concurrent attended session) both left alone. **PICK: `m-prompt-freeze-mirror-all-versions`** — the queue head is parked on `D-47` and its predicate had not flipped. **I MEASURED THE HOLE BEFORE ROUTING ANYTHING.** `prompt freeze --check` has three file-level arms and all three are gated on `Frozen != nil`; the repo has **59** registry entries, **58** frozen, and exactly **one** mutable — `v0.16.6`, which is also `active`, i.e. the prompt every agent reads. Synthetic-repo 7-cell matrix, frozen arms as positive controls: mutable source diverged / source deleted / mirror diverged all **rc=0** against frozen equivalents at **rc=1**. **MY OWN FIRST READING WAS WRONG AND MY OWN MEASUREMENT CAUGHT IT**: from `checkRegistries` alone I inferred the mirror `.md` was never hashed for ANY entry; false — arm 3 lives in the other file (`prompt_freeze_check_git.go:70`). Reasoning from one function, measuring from the whole. **DESIGNER `fable` via an explicit Agent-tool `model="fable"` pin**: reproduced my matrix, extended it to 15 cells and found **three holes I had not** — a mirror-registry-only entry is never visited (rc=0), a mutable `PLACEHOLDER` hash is accepted (rc=0), and a deleted frozen source aborts **rc=2**, hiding every other violation. It also corrected two facts in my own directive (the writer script's path; `v0.3.0` is not a registry key). **QUORUM BLOCKED TWICE, 3/3 EXTERNAL REVIEWERS PRESENT BOTH ROUNDS, `absent_reviewers` EMPTY.** R1: `gpt5-6-sol` — the `else if` chain suppresses independent violations; `oc-glm-5-2` — the `checked` count is computed beside the per-entry work, so AC6/M-ADD assert a property the variable cannot deliver. Both went back to the designer and both produced design changes, not arguments; `gemini-3-1-pro`'s non-blocking pointer-map concern I **refuted by measurement** (`map[string]*registryEntry` at `prompt_freeze_core.go:29`, the same nil-deref already shipping at `:351`) and handed over as a measurement rather than an objection. **R2 IS THE ITERATION'S BEST RESULT: all three objections were PREMISE objections about ONE surface — the call graph and consumers of the two interfaces under change — so rule 3f applies and I measured them rather than forwarding them.** `oc-glm-5-2` **CONFIRMED a real gap**: `checkRegistries` has 4 callers and `prompt_freeze_test.go` holds two of them (`:125`, `:138`), absent from the doc's Files-to-modify. `gpt5-6-sol` **REFUTED**: the sole consumer of `--check` is `make/code-health.mk:185`, which neither pipes nor parses stdout — nothing requires it empty. `gemini-3-1-pro` **CONFIRMED and strengthened**: `freezeVersion` validates hash-enforceability and bytes-vs-hash before its first write AND writes **no `.md` at all** (probe 0, control 5). Applied under the **narrow-refinement carve-out** (ratified at iterations 251/284) rather than a third Fable run, since the diet's one-doc ceiling — authoring plus one protocol-mandated revision — was reached. **PLANNER `opus`, lane derived VERBATIM as `opus fail-closed:planner-lane-field-missing`, no codex probe performed per Step 1b**: re-baselined all six gates and all eleven AC base rcs first-party, and found what I had not — **AC1 was one assertion from being a gate that cannot fail** (rc=0 at base and after, so the `checked 59 registry entries` stdout line is its only falsifiable half; `--check` writes nothing to stdout today, measured), and **AC8 bundled a can't-fail regression half with a falsifiable one**, split into AC8-REG and AC8-&Delta;. It also caught a doc-INTERNAL divergence: the Milestones section says `AC1-AC8` while the AC table has AC1&ndash;AC10 — stale r1 text; the doc wins and M8/M9's fixtures ARE AC9/AC10. **I CORRECTED THE PLAN BEFORE ROUTING IT**: its T13 put the changelog entry in the root `CHANGELOG.md`, which is an INDEX that `make check-changelog` rejects — the gate iterations 152 and 293 already paid for. Redirected to `changelogs/v0.32-current.md` and added `check-changelog` as G7, baselined rc=0. **MY DIRECTIVE'S GATE LIST WAS BASELINED IN THE LANE FIRST**: all seven rc=0 on the pristine worktree, and the negative control `go build ./...` is **rc=1 AT BASE** (`cmd/wasm` has no native `main`) — disqualified rather than shipped. **EXECUTOR `codex:gpt-5.6-sol`** (probe rc=0; directive 7,828 B past the &ge;200 B assert; `< /dev/null`; terminal rc-file marker; finished inside the 30-min cap): delivered the milestone plus ten mutants. **Its deviation was self-reported and I adjudicated it by measurement in BOTH arms** — it added `errors.Is(readErr, fs.ErrNotExist) { continue }` to the merge-base loop so a deleted frozen source stops aborting; a frozen source that is MODIFIED rather than deleted still trips `immutability violation in prompts/v0.3.21.md bytes`, so it is **strict, not a weakening**, and T7's invariant held (`git diff` touches no `baseEntry.Frozen` line). **I BANKED NO EXECUTOR-REPORTED GATE**: nine gates re-run outside the sandbox, all rc=0, and ten AC probes re-run first-party against a post-change ldflags-stamped binary — every criterion that was rc=0 or rc=2 at base is now rc=1 with its named string, the count observable moved **59 &rarr; 60**, restored control rc=0. **EVALUATOR `sonnet`, ITS OWN WORKTREE, SEVEN NAMED ATTACK TARGETS: PASS 96/100, ZERO BLOCKING — AND ITS SECOND NON-BLOCKING FINDING WAS REAL AND I FIXED IT.** It attacked my deviation adjudication and could not break it, probed a third compound state I had not tried (invalid hash + missing mirror &rarr; both fire), neutered four tests' own PRECONDITIONS and watched each die, and independently reclassified the executor's M1 from Criterion A to B. Then it walked the diff **hunk by hunk** and found the one hunk with no killer: reverting the `fmt.Printf("checked %d registry entries")` to `_ = checked` left the entire `cmd/ailang` suite **rc=0**. That is rule 3n exactly — the mutation set was chosen by thinking about the DEFECT, so it covered what the milestone FIXES and not what it SHIPS — and it lands on AC1's only falsifiable half. Reproduced first-party (mutant LANDED sha `1b6afdf7`&rarr;`89e1efdb`, BUILDS rc=0, suite rc=0), fixed with an arm that reads the command's actual stdout, proven RED under the still-applied mutant, proven **SOLE KILLER** by `-skip` inverse arm rc=0, production file restored byte-identical from a `cp` backup. **LANDED**: PR [#949](https://github.com/sunholo-data/ailang/pull/949) &rarr; squash `5e5c77dee`, four commits, all four required contexts green **on the merge commit** (build &middot; docs-gate &middot; lint &middot; test) with 19 of 20 checks `success` — the one non-green is the inherited SonarCloud red enumerated at Gate 1. This also CLOSES `m-prompt-freeze-mutable-mirror-unchecked`, measured two-arm on one tree: pre-change binary **rc=0**, post-change **rc=1**. **Routing**: controller `opus` (session) &middot; designer `fable` via an Agent-tool pin &middot; planner `opus` via the Agent tool, lane verbatim &middot; executor `codex:gpt-5.6-sol` via the cross-provider recipe &middot; evaluator `sonnet` via the Agent tool in its OWN worktree. generator&ne;judge holds on both axes: codex wrote the milestone, opus wrote the stdout arm, sonnet judged and refuted neither adjudication but found what both had missed. **metered=$0.2358** of $5 (two quorum rounds, $0.0910 + $0.1448; no managed_agents, no pi lane). Gates on **darwin/arm64**; ubuntu and windows legs green in CI. **FLAGGED: the designer rotation pointer did NOT advance** — last-used is `claude:claude-fable-5`, so the next entry was `pi:ollama/kimi-k3:cloud`, which the Agent tool cannot express, and this run's operator directive was Agent-tool spawns; I re-used the claude entry and left the pointer unchanged rather than writing a value I never used. That is **instance 2** after iteration 294 — the rotation has one Agent-tool-expressible authoring lane, and a routing-policy fix needs a human. **RETRO — NO SKILL EDIT**, candidate now at TWO instances and pre-registered: *the quorum's value is highest when its objection is a PREMISE nobody has run, and rule 3f says measure it — but nothing tells you that when EVERY surviving objection is a premise about ONE surface, that is the carve-out's canonical case rather than a park.* Instance 1 is iteration 284's r2; instance 2 is this r3, where measuring three objections took four minutes and one of them found a real gap that would otherwise have broken the build. The tell: a quorum round's objections all begin "the doc does not verify…" and you are drafting a revision directive.

## STATUS 2026-08-28 — ITERATION 296: **The queue head's residue was implemented, and the judge proved the delivered mechanism cannot do what its own design doc specifies — so the honest outcome is a PARK, not a merge.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver **PINNED**. **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target — pin-worktree inode **52608365** vs main-checkout **51683298**, asserted DIFFERENT files before trusting either. `mission-v1-gh-issue`=**852**; **0** allowlisted directives of 45 comments since `2026-08-27T07:26:21Z`. Rotation NOT owed (created Monday 09:50 LOCAL; 45 « 80). Weekly sweep NOT owed. Messaging store header names `store: gcp (Firestore, project ailang-multivac)`; bodies read out of `--json` so triage did not drain the unread queue. **dev CI**: three named workflows `success`, SHA-addressed set `checks=16`, `SonarCloud` `failure` — **INHERITED, enumerated per commit as `failure` at `d5305fa79`/`28014b1df`/`4d8705699`/`e38c0c493`/`caea1f9e1` and ABSENT at `c3d9ae0d9`; non-required, named, not the pick.** Main checkout **behind** origin (`0f9abeee1` vs `d5305fa79`) with 3 dirty files incl. Mark's uncommitted sprint-JSON reset — `D-42` applies, NOT reconciled; every write went to a worktree branched from `origin/dev`. **PR #942 left ALONE**: `--author` is a FLEET filter, and it has **0** mentions in charter or log against a control of **3**/**4** for `#939`, so it is a concurrent attended session's, not this mission's. **PICK: queue head `m-openrouter-session-chain-registration`.** Designer and planner **NOT SPAWNED** — the routing table's conditions were not met (design doc AND sprint plan both exist; the task is plan line 41's unchecked checkbox), so the Fable diet is **UNSPENT** and the rotation pointer unchanged. Neither omission was a spawn failure. **PREMISE CHAIN VERIFIED FIRST-PARTY BEFORE ROUTING**: `ai_agent.go:289` sets `Correlation.SessionID = chainID`; `openrouter/correlation.go:70` emits it as the wire `session_id`; `otlp_receiver.go:468` resolves via `LookupChainBySessionID`. Standard mode had **0** `UpsertSessionWithCorrelation` calls (same-file control `SetCorrelation` **1**; agent-mode known positive **1**). **MY GATE LIST WAS BASELINED IN THE LANE BEFORE THE DIRECTIVE WENT OUT** — all six rc=0 on the pristine worktree, and the negative control `go build ./...` **rc=1 at base**, which is exactly the gate I would otherwise have shipped broken (iterations 145/245's finding, bought a third time and caught this time). **MY FIRST ldflags BUILD SILENTLY FAILED**: I used `github.com/sunholo/ailang`; the module is `github.com/sunholo-data/ailang`, and Go does not error on an unmatched `-X`, so `--version` read `dev` while `git describe` read `v0.34.0-118-gd5305fa79`. Corrected and re-asserted equal. **4 test files shell out to a bare `ailang` from PATH**, so both arms were run side by side (stale PATH rc=0, fresh PATH rc=0 — identical, so staleness was not distorting them, which is a measurement rather than an assumption). **EXECUTOR `codex:gpt-5.6-sol`** (probe rc=0; directive 7,863 B past the ≥200 B assert): delivered the registration plus a resolver test, and correctly labelled its two red gates `UNINFORMATIVE UNDER SANDBOX` — both were `workspace-write` loopback bind denials and **rc=0 outside**, where I re-ran them. I banked no executor-reported gate. **I FOUND A GAP IN ITS TEST AND THEN THE JUDGE FOUND THE SAME GAP IN MY FIX.** Its test calls the helper directly, so deleting the CALL SITE left it — and the whole `cmd/ailang` suite — **rc=0**: M1's own defect one layer down. I added an AST wiring arm; two mutants then discriminated cleanly (helper neutered → resolver 1 / wiring 0; call site deleted → resolver 0 / wiring 1), each a sole killer, both asserted LANDED by sha256 and BUILDS rc=0 first, both restored byte-identical. **EVALUATOR `sonnet`, ITS OWN WORKTREE, SEVEN NAMED ATTACK TARGETS: FAIL 58/100, TWO BLOCKING — AND I REPRODUCED BOTH.** **Blocking 1:** one `evalChain` is created per **`eval-suite` invocation** covering every benchmark (`eval_suite.go:500`), `sessions.session_id` is `TEXT PRIMARY KEY` (`migrate.go:184`), and the upsert is `stage_id = COALESCE(excluded.stage_id, sessions.stage_id)` with `excluded.stage_id` non-NULL every time (`store_sessions.go:88`) — so each benchmark **overwrites** the one row and the receiver hands that single `stage_id` to every span resolving by chain. Bare `eval-benchmark` builds no chain at all (`createEvalChain` in `eval.go` = **0**, control in `eval_suite.go` = **1**), so the only reachable caller is the multi-stage one. That trades *unresolved* traces for *misattributed* ones — a silent data-integrity regression, which Principle 2 forbids. **Blocking 2, against MY OWN test:** wrapping the call in `if false { … }` leaves the wiring arm **rc=0** and the full suite **rc=0** (mutant LANDED + BUILDS rc=0, restored byte-identical). `ast.Inspect` walks unreachable nodes, so it pins SYNTACTIC PRESENCE, not reachability — and its own doc comment claimed the stronger property. Its known-positive control does fire correctly, which is precisely what made the hole invisible: **a firing control certifies the walk, never the semantics.** **ROOT CAUSE IS THE DESIGN DOC, NOT THE EXECUTOR**: `m-mission-loop-unified-telemetry.md:146` specifies a row *'bound to the stage'*, keyed by a value `internal/ai/correlation.go:19` documents as *'the chain ID, the finest grain already persisted per run'*. A stage-bound row on a chain-grained PRIMARY KEY is not constructible. That contradiction survived the doc's quorum, its sprint, iteration 295's 15/15 acceptance re-verification, and my own routing. **DISPOSITION: PARKED, NOT MERGED.** PR [#945](https://github.com/sunholo-data/ailang/pull/945) → `d86399f0a` is converted to **DRAFT** with the findings posted (comment count asserted 0→1); CI was MERGEABLE with 5 runs created, CodeQL and Build-and-Release `success`, CI and Docs still in flight when the park was taken — so **no LANDED tag is claimed**. Filed as **`D-47`**: register **chain-only** (correct, no misattribution, strictly better than today's zero resolution, small) vs thread a **per-request unique correlation id** onto the wire (full fix, changes eval-path wire bytes). Both are design calls on a live eval path with a banked KPI, so neither is mine to invent at 02:40 headless. The changelog's *'and stage'* claim is walked back in the same comment. **Routing**: controller `opus` (session) · executor `codex:gpt-5.6-sol` via the cross-provider recipe · evaluator `sonnet` via the Agent tool in its OWN worktree · designer/planner not spawned (conditions unmet, recorded not silent). generator≠judge holds on BOTH axes: codex wrote the registration, opus wrote the wiring arm, sonnet judged and refuted both. **metered=$0.00** of $5. Gates on **darwin/arm64**; ubuntu and windows legs unrun locally. **RETRO — NO SKILL EDIT**, candidate pre-registered at TWO instances: *this file's mutation rules all ask whether a guard FIRES, and an AST/source-shaped guard can fire perfectly while proving only that a token is PRESENT — so `if false {}`, a never-true condition, or a dead branch defeats it with the control still green.* Instance 1 is my wiring arm here; instance 2 is rule 3j's `check_protocol_closure`-class source greps. The nearest existing rule, 3i, asks *'which write does this read?'* — the missing question is **'could this assertion hold while the code never runs?'** The tell: your guard's observable is a fact about the SOURCE rather than about an EXECUTION.
...
**[PARKED — needs-human-review, `D-47`; iter-296 implemented it and the evaluator proved the mechanism misattributes] m-openrouter-session-chain-registration**
...
**[NEEDS-HUMAN-REVIEW] [iter-306 D-50 approved; single permitted re-quorum still BLOCKED] m-coordinator-child-env-opencode-retry-storm**
...
&mdash; Canonical recovery design and plan live under `design_docs/planned/v0_35_0/`. D-50 already supplies approval and `execute sprint`, but quorum r1 and the one permitted re-quorum both BLOCKED. Resume only after a new human directive reopens amendment/review, then fresh quorum PASS; preserve M1 commit `3500db0a7`, implement OpenCode-only pinning plus M3/M4 fresh, and do not adopt the parked broad M2-M4 diff wholesale.
