...
- **[NEXT]** m-cost-per-success-kpi (dashboard KPI flip to cost-per-verified-success + v1.0 measured baseline, 1–2d) — **DESIGN DOC created iter-103** ([planned/v1_0_0/m-cost-per-success-kpi.md](planned/v1_0_0/m-cost-per-success-kpi.md)) via `codex:gpt-5.6-sol` rotation designer; **quorum-cleared** (gemini-3-1-pro + controller, 2 rounds — R1 silent-fallback objection → designer-revised; R2 `QueryEvalResults` boundary objection = phantom [it's an Observatory-local `Store` method, not an eval-package call] → NARROW-REFINEMENT CARVE-OUT [ratified iter-98] applied as a §3 clarifying edit). Ready to route to **sprint-planner (opus)** next iteration. Open-for-Mark (do NOT block M1–M3): verified-success definition ratification, v1.0 cohort manifest, headline placement.
