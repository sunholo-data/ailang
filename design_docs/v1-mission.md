...
- **m-mapE-queryall-retention (`#610`) — retaining `mapE`-over-query-rows values costs ~49× vs a range loop** ([cli-DEMAND]; P2 memory; **repro infra gates the design**: duckdb CLI + `sunholo/duckdb@0.1.1` + `-memprofile` heap profile on the reporter's retain/discard pair — no doc until the mechanism is named). Direction CONFIRMED at HEAD without duckdb (iter-191 synthetic, stable ×2 runs): identical retained data (300 fresh 768-int vectors) costs ~187 MB produced under `mapE` over 300 std/json-parsed rows vs ~77 MB under `mapE` over a range (~2.4×); per-row field access adds nothing (268 vs 270–279 MB); parse-only control 92 MB. So core `mapE`+`std/json` CANNOT explain 49× — the magnitude lives in the `queryAll` path or its interaction; the reporter's own controls already rule out `join`, float-list retention, and embed results per se. Sub-findings banked for the eventual doc: `std/list.ail:235` builds `mapE` via non-tail `[y] ++ mapE(f, rest)` (candidate for the residual ~2.4×), and `queryAll`'s ~1.2 GB-for-10-rows is duckdb's own buffer pool → disposition as documentation (`SET memory_limit`), not an AILANG cost.
