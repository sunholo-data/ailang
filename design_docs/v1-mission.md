...
**[NEXT — TOP OF QUEUE; iter-347 first-party, a RED on `dev` and the only thing now keeping `launchd drivers (bash 3.2)` from green] `m-launchd-heartbeat-arms-red-behind-the-notify-red` — `tools/launchd/test_mission_heartbeat.sh` fails 9 arms, and it had been INVISIBLE because an earlier suite in the same ordered target failed first.**
`make test-launchd-drivers` runs its suites in sequence and halts at the first failure, so for as long as `test_driver_notify.sh` was red **nothing behind it ran**. Clearing that red at iteration 347 let the target reach three further suites: two pass (**84/0** and **17/0** — in CI, having never executed on `dev` before) and `test_mission_heartbeat.sh` reports **`FAIL: 9 heartbeat arms failed`**. Attribution is a paired control, not an inference: run ALONE at base `1825fd8c7` and at head `58af47580` it gives the **identical nine named arms** — `empty-stamps => DIED-PRE-GATE-0`, `rc=0 last=gate-5 => REAPED at=gate-5`, `sigkill mid-gate-1 => REAPED at=gate-1`, `rc=0 last=abort => ABORTED`, `rc=0 last=complete => COMPLETED from consolidated state dir`, `rc=143 last=gate-3 => KILLED`, `rc=1 last=gate-2 => CRASHED`, `250 appends leave exactly 200 lines`, `attempt 2 pre-gate-0 => DIED-PRE-GATE-0 attempt=2`. Two visible symptoms to start from: `test_mission_heartbeat.sh: line 12: [: : integer expression expected` and `slot-verdict.sh: line 24: MC_PAUSED: unbound variable`.
**The class is the finding, and it generalises past this suite:** a red that fails EARLY in a long ordered `make` target SUSPENDS every gate behind it, so the check set reports one failure where there may be several — and each one you fix reveals the next. Anything that reports a single red from an ordered target is reporting a lower bound.
