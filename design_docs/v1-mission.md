...
**Left un-re-run deliberately:** `gh run rerun --failed` was refused with *"This workflow is already running"* because the run's `test` job was still in flight, and iteration 347's slot ended before it cleared. The divergence control (re-run on the byte-identical tree) is therefore **UNMEASURED**, not negative — run it first, since it is the one command that separates an outlier from a regression.
