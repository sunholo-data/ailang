...
| D-45 | OPEN | **Should the OpenRouter session→chain registration bind CHAIN-ONLY, or chain+stage?** One word: `chain` or `stage`. Context: `M-MISSION-LOOP-UNIFIED-TELEMETRY` M1's read side is live and tested, its WRITE side was never built, so every Broadcast trace is still unjoinable. The call site `AIAgent.SetCorrelation` (`internal/eval_harness/ai_agent.go`) receives **only** `chainID` and already sends it as the OpenRouter `session_id`; the design doc scopes the sessions row as bound to `chain_id` **and** `stage_id`. `chain` = ~40 LOC, strictly better than today, joins a trace to its iteration but not to the specific stage. `stage` = thread the stage id through the eval dispatch path, a wider change on a live path. | Measured iteration 295, first-party: `UpsertSessionWithCorrelation` exists on all four backends and nothing calls it with a chain id (`internal/ai/openrouter/correlation.go` registration grep rc=1/hits=0, same-file control `session_id` rc=0 at 2). Live 434 MB observatory: `sessions_keyed_by_a_chain_id` = **0** against controls **19262** and **18947**. Prod, 72 h: 159 `LLM Generation` spans, 97 with `session.id`, **0** resolving to a chain. The controller declined to choose because it is a design call on a live eval path (Standing rule 2). Queue row: `m-openrouter-session-chain-registration`. |
...
`internal/ai/openrouter/correlation.go` &mdash; the design doc's own planned file for this &mdash; has **0** registration
hits (`grep -nE 'Upsert|Register|observatory|sessions'` rc=1) against a same-file control `session_id` rc=0 at **2**.
...
&mdash; **It carries a design decision and must NOT be implemented before that is answered** (`D-45`): the call site
receives **only** `chainID` &mdash; there is no `stage_id` in scope &mdash; while the design doc scopes the row as bound to
`chain_id` **and** `stage_id`. Chain-level-only linkage is strictly better than today and is ~40 LOC; stage-level
needs the stage id threaded through the eval harness's dispatch path, which is a wider change. Iteration 295
deliberately did not choose: inventing it would be the controller manufacturing a design call on a live eval path.
