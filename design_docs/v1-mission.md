...
**[NEXT — 3 of 15 dispositioned; 12 remain] m-sweep-orphans-2026-08-17 — the zero-mention issues from
...
- **Mission/loop infra** — ~~`#727`~~ **[LANDED 2026-08-18 (iter-219) — PR `#765` → squash
  ...
  `#687` (`⚠ Binary may be stale` is an mtime heuristic over CWD-relative dirs, so it fires on a
  content-identical binary in every fresh worktree — i.e. it mis-fires on this loop's own sprint
  worktrees).
...
reports. Do NOT batch-close: each needs its own measurement, and a CI-enforced guard where the
