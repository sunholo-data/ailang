...
**[NEXT] [iter-333, measured first-party on the windows runner; PRE-EXISTING, surfaced by M4 rather than caused by it] `m-cache-sanitize-module-id-windows-colon` — the compile artifact cache publishes NOTHING on Windows, because the drive-letter colon survives into the artifact directory name.**
