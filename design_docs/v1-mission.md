...
**[NEXT] m-sweep-orphans-2026-08-17 — the 14 remaining zero-mention issues from iteration 216's
weekly external-issue sweep.** ONE batched row per the Gate-0 sweep rule (a sweep never outranks
...
- **Mission/loop infra** — `#727` (`govulncheck-filter`: two `decide()` refusal branches untested
  at any level — a rule-3j shape, and the `/govulncheck-filter` gitignore landed alongside `#696`
  is adjacent but does NOT close it), `#708` (`design-quorum` records no per-reviewer token usage,
  making Gate-3's chain-telemetry token mandate unsatisfiable — note this is the same defect the
  skill's own iter-190 measurement recorded as "two OpenRouter quorum stages at ZERO tokens"),
...
Per-issue triage-lite is OWED and NOT yet done for these 14 — ghost-discipline each repro at HEAD
before routing, exactly as `#696` turned out to be already-fixed. Two are strong candidates to be
ghosts or already-fixed on the same pattern (`#687`'s heuristic and `#708`'s token plumbing both
sit in code this loop has since edited). Sequence them by lane: mission-infra first (cheapest to
