...
- **Effect sprints** (decomposed): **m-effect-replay-contracts (2/4, 3d) [LANDED PARTIAL iter-99 →
  ...
  **m-effect-replay-subsumption [DECIDED by Mark 2026-07-27 (attended) → ROUTABLE [NEXT]: YES —
  an explicitly declared mode SUBSUMES the bare/os effect requirement; implement as the narrow
  relaxation on the `SubsumeEffectRows` validate path ONLY (function-value mode distinctness
  unchanged). Was: NEW GATING clause-4, PARKED needs-human-review iter-99 — a Mark/type-system
  decision]**: `SubsumeEffectRows` treats effect modes as INVARIANT (`internal/types/effects.go:625`,
  guarded by `TestSubsumeEffectRows_InvariantOnParams` which Phase-5 routeable→fixed depends on), so a
  `!{Rand[mode=seeded]}` fn CANNOT call the bare-`!{Rand}` (os) `std/rand` wrappers. **SHARED gate for the
  whole effect-mode-dispatch line** — the parent doc's `Clock[mode=pinned] = now()` examples have the
  identical structure, so sprints 3/4 (clock/net/fs) hit it too. **Q for Mark: does an explicit declared
  mode subsume an os/bare required effect?** (parent-doc Examples 1/2 imply yes; the `SubsumeEffectRows`
  validate path is separate from the `effectParamsCompatible` unification path → a narrow relaxation is
  feasible without weakening function-value mode distinctness). Resolving it unblocks seeded/crypto here
  AND sprints 3/4. ·
