...
**N = 12** — 9 existing design docs + 3 NEW-DOC units. Inside the attended session's honest range
...
**[PARKED — needs-human-review, ledger `D-55`; iter-328. Quorum BLOCKED after the one revision and the one re-quorum, so this parks rather than forcing through (Standing rule 2). Evaluator `sonnet` PASS 92/100 independently agreed PARK was correct.] `m-compile-cache-unverified-artifacts`** — the compile cache verifies the manifest and never the artifacts it actually executes.
Doc landed at `design_docs/planned/v0_35_2/m-compile-cache-unverified-artifacts.md` with both quorum artifacts; defect reported at [#1046](https://github.com/sunholo-data/ailang/issues/1046). CONFIRMED first-party at HEAD and reproduced end-to-end: source says `99`, the manifest `cache_key` is the CORRECT key for the `99` source, the blobs are from the `42` compile, and `ailang run` prints **42** with no diagnostic. Negative control — ordinary edit-and-re-run invalidation works (42 → 99). Mechanism: `LoadArtifacts(moduleID)` (`cache_store.go:185`) receives no key and verifies nothing (`grep -n CacheKey cache_store.go` = exactly 2 lines, neither in the artifact path; control `modDir` = 11), while `pipeline_module.go:369` writes the manifest entry unconditionally and `:377` discards the artifact-write error. **Resume predicate: `D-55` is answered.** If ruled (a), this goes straight to the planner with no further design work — the doc is otherwise complete at 4 milestones / 4 days with 48 verification rows.
...
**[NEXT] [iter-328, first-party; the doc explicitly and deliberately leaves this in place] `m-cache-sanitize-module-id-collision`** — `sanitizeModuleID` maps `/` and `\` to `__`, so module IDs `a/b` and `a__b` both resolve to the artifact directory `a__b`.
...
**[NEXT] [iter-327, first-party; the two instruments the loop routes on disagree by construction on any doc-less pick] `m-resolver-hook-disagree-on-docless-pick` — `resolve-role-spawn.sh planner` with no design-doc argument returns an alias the spawn-pin hook will always deny.**
