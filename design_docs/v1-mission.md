...
**[NEXT] [iter-334, first-party; the runner's OWN load-bearing assertion, and it failed silently in the direction it exists to prevent] `m-pi-runner-worktree-assertion-vacuous-on-revision` — `mission_pi_run.sh` reports verdict `ok` for a run that wrote nothing, whenever the target file was already dirty or untracked before the run.**
`scripts/mission_pi_run.sh`'s header states the case plainly: `stopReason` is evadable in both directions and fired on **0 of 4** real failures, so *"the load-bearing assertion is the worktree diff"*. That assertion is `worktree_changed_files`, derived from `git status --porcelain` — a snapshot of the tree's state, **not a measurement of what THIS run did**. On a first authoring run the two coincide. On a REVISION of a file the previous run created, they do not: the file is still untracked, the count is still ≥1, and a run that read nine files and wrote zero bytes returns `{"verdict":"ok","worktree_changed_files":1}`. Measured first-party at iteration 334: the deepseek revision pass returned `ok` at 234s with 9 tool executions, 4 `agent_start`/`agent_end` and 3 `auto_retry_start`, and the design doc's mtime (`04:42:02`, the controller's own earlier `sed`) **predated the run**. Caught only because the controller had taken a `shasum` before the next lane's run for an unrelated reason. This is the stale-artifact class (standing rule 7's amendment) aimed at the runner that exists to detect exactly this. **Fix:** hash the worktree's tracked+untracked content before and after and compare, or capture `git status --porcelain` as a BASELINE at launch and diff the two snapshots, so the verdict reports the delta rather than the level. Cheap, and it makes `empty_worktree` (rc 10) mean what its name says. **Resume predicate:** re-read the verdict JSON's fields against a deliberately no-op run on a dirty tree — today it returns `ok`.

**[IN-SPRINT — ABSORBED 2026-09-06 (iter-334) into `m-cache-module-id-encoding`, together with `m-cache-sanitize-module-id-collision` below; DESIGNED + PLANNED, 0 of 4 milestones executed.** Design `design_docs/planned/v0_36_0/m-cache-module-id-encoding.md` (quorum 2 rounds, 3/3 reject each, all objections answered — round 2 under the narrow-refinement carve-out), plan `…-sprint-plan.md` (`codex:gpt-5.6-sol`, 4 milestones / 1.35 days), judge `sonnet` **PASS 85/100 zero blocking**. Its own resume predicate below is DISCHARGED: the sanitizer's escaping is designed once, covering the colon, the collision and the reserved-device-name class, with a per-platform table test. **Resume predicate: execute M1** (the pure `encodeModuleDirName` + its unit table; 0.35 d). Note the plan flags **M3 and M4 as having no non-vacuous mutation of their own diff** — read its non-vacuity ledger before executing either.] [iter-333, measured first-party on the windows runner; PRE-EXISTING, surfaced by M4 rather than caused by it] `m-cache-sanitize-module-id-windows-colon` — the compile artifact cache publishes NOTHING on Windows, because the drive-letter colon survives into the artifact directory name.**
`sanitizeModuleID` (`internal/pipeline/cache_store.go:162`) replaces only `/` and `\\` with `__`. On Windows a module ID begins `C:/Users/...`, so the sanitized directory is `C:__Users__runneradmin__...` — a path component containing a colon, which Windows forbids. Every publication therefore fails, and the diagnostic M1 added says so verbatim: `CACHE_WRITE_FAILED module=C:/Users/... stage=publication path=...\\compile\\modules\\C:__Users__runneradmin__... : ARTIFACT_INVALID`. **The whole compile artifact cache is non-functional on that platform.** PRE-EXISTING: `sanitizeModuleID` was introduced by `d96de92f5` (M-INCREMENTAL-TYPECHECK M2), long before this sprint, and `internal/pipeline` is untouched by iteration 333's PR (`git diff --stat 353082b1c..944b3a5ef -- internal/pipeline` empty; control, `apiserver` = 2 files). M1 only made the failure audible. It went unseen because every existing Windows test TOLERATES a cache miss; M4's two MCP fixtures are the first that require a PUBLISHED artifact, which is why they are the first to redden. **Not absorbed into M4 deliberately** (iteration 257's rule — a pre-existing defect a reviewer surfaces is a queue row, not a revision): adding `:` to the replacer changes artifact-directory identity on EVERY platform and interacts with the already-filed `m-cache-sanitize-module-id-collision` row (`a/b` and `a__b` both map to `a__b`), so the two want one design, not two one-liners. Meanwhile `requireCompileArtifactCache` in `cmd/ailang/serve_api_mcp_surface_test.go` skips both fixtures on Windows — conditioned on the OBSERVED empty manifest AND `runtime.GOOS`, so on any platform where publication works an empty manifest is still a hard failure. **Resume predicate:** design the sanitizer's escaping once, covering the colon, the collision, and any other reserved character, with a table test per platform.

**[NEXT] [iter-332; 2 instances, both first-party, one recorded by the PREVIOUS iteration] `m-gate1-shared-clone-ref-drift` — every mission on this rig shares one clone's `.git`, so `origin/dev` advances when a SIBLING fetches, and Gate 1's sync verdict silently expires.**
...
**[ABSORBED 2026-09-06 (iter-334) into `m-cache-module-id-encoding` — the chosen `m-<slug>-<16hex>` encoding makes the mapping injective, so this row is designed away rather than fixed separately. Do not pick it; it lands with M2.] [iter-328, first-party; the doc explicitly and deliberately leaves this in place] `m-cache-sanitize-module-id-collision`** — `sanitizeModuleID` maps `/` and `\` to `__`, so module IDs `a/b` and `a__b` both resolve to the artifact directory `a__b`.
