...
- **(G4) gemini REPO-MOUNT upgrade** — **[LANDED (code) iter 52 — Mark "apply both fixes, ship it" (#399
  2026-07-18T08:54:45Z); Phase-2 clone-over-egress IMPLEMENTED + MERGED to dev `80cbd9612`, CI green. Both
  approved fixes applied (typed `RequiresEgress`/`CapNetworkEgress` gate + shared `ValidateTaskCapabilities`;
  bounded-execution/timeout-reuse) + an iter-52 re-quorum shallow-fetch-by-SHA fix; opus executor, sonnet
  evaluator 91/100 PASS. **ONE premise pending Mark:** the live-gated E2E (`AILANG_LIVE_MANAGED_AGENTS_MOUNT=1`)
  confirming provider `git fetch --depth 1 <sha>` support SKIPs without ADC → needs a Mark ADC+Vertex run;
  doc stays in `planned/` until then, then move to `implemented/v0_30_0/`. **gemini can now join the designer
  rotation once that live E2E confirms** (repo-mount usable). Log 57.]** iter-45 refuted the `repository`/`inline`
...
- **Prompt teaching** (batchable, ~0.5d each): ~~m-prompt-option-none-idiom~~ **[SUPERSEDED
  ...
  designer authored+revised; quorum (gpt5-6-sol+gemini-3-1-pro+controller) bounded round
  author→reject→revise→re-quorum→reject; Part A (module diagnostics) + Part B (ghost-close) UNANIMOUSLY
