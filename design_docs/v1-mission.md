...
## STATUS 2026-08-31 — ITERATION 310: **A sibling agent's bug report is real, and the design to fix it was blocked twice by three reviewers who were right both times.** Gate 0/1: kill switch armed; billing tripwire CLEAN; gh `sunholo-voight-kampff`; running skill == origin verified against the **RESOLVED** symlink target (inode `60291442`, distinct from the pin's own `60316994`, so the check was not the self-refuting form). Canonical cloud inbox with the invalid-store control FIRING (`unknown message store mode`, rc=1); 24 unread, 0 directives on #972 or #852; ledger **50 rows, 0 OPEN**; weekly sweep already done this week by iteration 308. dev checks=14 at `bd17d9643`, only `test` in flight. **Gate 2 refuted the queue head.** `m-coordinator-config-route-preflight` prescribes a `config check` that *“calls the same `ResolveExecutionRoute` the daemon does”* — and `ExecutionRoute` is **0** occurrences in `internal/`+`cmd/` **non-test** at HEAD (**1** including tests &mdash; a comment in `internal/dispatch/cloudrun/accept_set_pin_test.go:13`; scope qualifier restored after the iteration-310 evaluator caught the headline dropping it) (control `func .*Dispatch` = **9** in `internal/` alone, **10** across `internal/`+`cmd/` &mdash; the count travels with its scope; scopes asserted with `test -d`). It exists only on the unmerged evidence branch `mission/iter309-route-authority-parity` @ `8c8c29864`, so half the row is unroutable and its stated purpose — gating the route freeze — gates a sprint iteration 309 PARKED. Re-scoped, not silently skipped. **Pick = `m-registry-interface-hash-blind-to-signatures`**, ghost-disciplined REAL at HEAD: `InterfaceHash` (`internal/pkg/hasher.go:73-100`) folds only name/edition/ailang/sorted module **paths**/sorted effects (grep `Signature|Type|Func|Arity|Param` = **0**, same-file control `Fprintf` = **6**), so adding, **removing** or **retyping** an export leaves it byte-identical and `classifyChange` (`pkg_events.go:162-170`) labels the release `patch`. That function is a 2-valued classifier whose `ContentHash` arm is **dead code** — both remaining branches `return "A"` — so there is no additive class at all. Designer = rotation entry **`pi:ollama/deepseek-v4-flash:0731-cloud`** (flat-rate, $0), three runs all verdict `ok`: 211s/47 tools, 443s/69 tools, 137s/28 tools, containment clean every time (exactly 1 changed file; main checkout untouched). **Quorum BLOCKED twice, 3/3 external present both rounds** (no N−1 degrade). Round 1's objections spread across three surfaces; **round 2's localised onto ONE** — compiling untrusted package source in-process — which is the decomposition signal. All premise objections were MEASURED, not forwarded: (a) my own round-1 fact **V9 was REFUTED** — `runPrePublishSmoke` is a **subprocess** (`publish_validator.go:88` `exec.CommandContext`, binary from `os.Executable()`), so no interface data crosses back, and the designer had correctly labelled it UNVERIFIED rather than laundering it; (b) the registry validator is a **live HTTP server** (`main.go:70` `ListenAndServe`) that already isolates compilation in a subprocess (`validate.go:76/95/116`), so the design's in-process path was the deviation, not the reviewer's objection; (c) the doc's publisher/validator equivalence claim was **REFUTED** — `pipeline.Run` = **0** and `DryLink` = **0** across all 11 files of `cmd/registry-validator/` (control `^func ` in `main.go` = **7**). Resolved under the **narrow-refinement carve-out** (all three fixes reviewer-authored and concrete; none disputed the design DIRECTION), applying them verbatim — one hidden `internal-dump-iface` subcommand invoked as a subprocess by both binaries, which also dissolves D4. **A pre-existing defect surfaced and deliberately NOT absorbed:** those three validator sites use `exec.Command`, not `exec.CommandContext`, so the server compiles untrusted uploads with no timeout today — filed as its own queue row per the defect-the-doc-did-not-introduce rule. **Two stale parks found:** both queue rows tagged PARKED cite decisions the authoritative ledger marks **RESOLVED**; `D-47` was answered by Mark on 2026-08-28 naming the consequence verbatim (*“UN-PARKED … take the ~2 LOC chain-only change off PR #945”*) and five iterations walked past it. Quorum artifacts are `.gitignore:82`-ignored with **0** tracked on origin/dev (control 1473 under `design_docs/`), so the decision-bearing record went into the tracked doc rather than `git add -f`. Evaluator `sonnet` **87/100 PASS, zero blocking**; its two numeric findings (a headline count quoted without its scope, and a broken `git check-ignore` control) were reproduced and CORRECTED in-iteration. Planner (opus) returned **PROCEED WITH RE-SCOPE, 9d not 4**, refuting **three blocking defects both quorum rounds missed** &mdash; chiefly that 16 of 19 acceptance gates are green at base and can never go red, and that D5's table would stall the cascade for every package. Design + plan LANDED; sprint plan routed to an **opus** planner (`derive-planner-lane.sh` → `opus fail-closed:planner-lane-field-missing`, used verbatim). metered=**$0.1664**.
...
**[IN-SPRINT &mdash; iter-312: Sprint 1 **M1+M2 LANDED**; M3&ndash;M5 remain (~3d)] `m-registry-interface-hash-blind-to-signatures` &mdash; a BREAKING API change is classified `patch` to every downstream consumer.**
