  ...
  **RE-TRIAGE BATCH 1 — root + v0_29_0 (iteration 85, 2026-07-23; 52 docs, controller-lane, 4
  ...
  - **GATING-candidate → queued (clause 3/4; VERIFY-FIRST at pick — survey-sourced, inherit ghost
    ...
    **CORE proven bit-exact SplitMix64 at HEAD**, revised+re-quorumed; sole block = `split` scope,
    Path X defer / Path Y 2-word `Gen` — Mark decides, then unpark) · **[GATING clause-3, PARKED
    iter-95 needs-human-review]** `m-budget-scoping-bug` (effect `@limit` per-function attribution
    vs shared BudgetContext — bug-claim; **live-repro CONFIRMED REAL at HEAD iter-95**, design
    DIRECTION ratified via quorum [hierarchical bubbling frames], re-quorum blocks only on 2 narrow
    reviewer-verbatim refinements → unblock via the QUORUM narrow-refinement carve-out once Mark
    ratifies its first use, then straight to sprint-planner). **[GATING clause-4]** `m-agent-step-cancellation` (`std/ai.step` graceful
...
- **Effect sprints** (decomposed): **m-effect-replay-contracts (2/4, 3d) [PARKED needs-human-review
  iter-96 — QUORUM round-2 blocked on premise-verification + a real seeded-source program-level
  seeding design gap (`modal_rand.ail`'s `mode=seeded` fn seeds via `rand_seed`, which is `mode=os`);
  the round-1 rand_seed/os byte-identical contradiction was RESOLVED by 1 designer-revision. Mark: pick
  a seeding path (a mode-aware `rand_seed` / b `AILANG_SEED`-only + rewrite the example / c rethink)
  + optionally ratify the narrow-refinement carve-out to auto-fold the premise rows]** ·
  m-effect-clock-net-fs-modes
  (3/4, 3d — BLOCKED behind sprint-2) · m-effect-scope-params (4/4, 2.5d — release-gate re-score candidate)
...
- **m-mission-cost-chains** ([planned/v0_30_0](planned/v0_30_0/m-mission-cost-chains.md), 2026-07-18;
  **P1½ — the clause-5 KPI's data substrate**, Mark: "keep an eye on these budgets… that should
  all appear in ailang chains CLI") — **[PARKED needs-human-review iter-97 (2026-07-24)]**.
