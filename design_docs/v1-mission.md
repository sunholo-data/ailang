...
## STATUS 2026-08-27 — ITERATION 293: **An external report was right that the served teaching prompt is mislabelled, wrong about why, and the real defect is a writer that has been propagating it for 15 files.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver **PINNED** (`~/.ailang-driver-pin/v1`). **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target (main checkout inode 51683298; the pin worktree's copy is 52608365, a DIFFERENT file). Local dev == origin/dev exactly. Namespaced `mission-v1-gh-issue` = **852**; **0** allowlisted directives of 42 comments since `2026-08-27T07:26:21Z`. Rotation NOT owed (created Monday 09:50 LOCAL, after the 07:00 boundary; 42 « 80). Messaging store control FIRED (`unknown message store mode`, rc=1, captured WITHOUT a pipe after the first attempt read `$?` through `head` — this file's own trap, sprung on its own control) and the header names `store: gcp (ailang-multivac)`. Weekly sweep NOT owed (iteration 292 ran it this rotation). Nightly-eval `#847` open but already tracked (charter 1, log 3) and triaged. **PATH BINARY WAS 39 COMMITS ADRIFT** (`v0.34.0-75-gfb6084f4b-dirty` vs repo `-114`); built to a scratch dir WITH ldflags and prepended to PATH rather than `make quick-install`, so concurrent agents were undisturbed — `--version` == `git describe`. **GATE-1 FOUND dev RED AND IT WAS NOT CODE.** `Build and Release` FAILURE on HEAD `caea1f9e1`, the 7 preceding dev runs all `success`, so NOT inherited. Failing step read rather than the check name: step #5 `Get dependencies`, `go: github.com/emirpasic/gods/v2@v2.0.0-alpha: verifying go.mod: reading https://sum.golang.org/tile/8/0/x084/628: stream error; INTERNAL_ERROR; received from peer` — a transient checksum-DB failure. The other three jobs were fail-fast `cancelled`. Disposition per the outage rule: **re-run, not revert, not fix-forward**. **The re-run went `completed success` on the byte-identical tree**, which is the negative control that settles it. dev now green on all **4** required contexts (build · docs-gate · lint · test). `SonarCloud` `failure` is NEW at `caea1f9e1` (4 preceding analysed commits green) on **63.3% coverage-on-new-code** (needs 80%) and **B security rating**; non-required, named, filed — and NOT attributed to `#939`, because `inNewCodePeriod` spans **2404** issues / **203** hotspots, far wider than one merge (rule 3b(ix): the scope travels with the count). **INBOX: 4 substantive external-origin reports, all ghost-disciplined at HEAD.** CONFIRMED REAL: `charAt` panics out of bounds (rc=1) while the in-bounds control returns rc=0 — and its reporter's second-order point is the sharper one, that a panicking accessor inside a fold makes exit-code-based negative tests vacuous. **CORRECTED, and this is the iteration's pick:** the report `ailang prompt serves v0.16.2 on a v0.34.0 binary` is **WRONG ON MECHANISM**. Served bytes sha `43d4ebd6…ea61` are byte-identical to BOTH `v0.16.6.md` mirrors and the registry `active` is `v0.16.6` — the loader was always correct. What is broken is that the served file's own line 1 reads `# AILANG v0.16.2`, so the artifact misidentifies itself; and `--version 0.34.0` failing is a namespace confusion (prompt versions ≠ binary versions), not a bug. Measuring rather than transcribing decomposed one report into one real defect, one non-defect, and one separate content gap. **IT IS SYSTEMIC: 15 of 54** `cmd/ailang/prompts/*.md` have header ≠ filename (39 match, so mismatch is not the convention), and the pattern names the mechanism — `v0.16.3/4/5/6` ALL claim `v0.16.2`. **PLANNER (`opus`, lane derived VERBATIM `opus fail-closed:planner-lane-field-missing`) REFUTED ONE OF MY FACTS AND WIDENED THE HOLE.** My directive claimed a stale registry hash breaks mirror/registry agreement; it does not — that compare is registry-vs-registry, so leaving BOTH hashes stale returns rc=0 (proven, `/tmp/fz_e`). The hash update is mandatory for a different reason: `prompt_loader.go` hash-verifies MUTABLE versions, caught by `make test`, never by the freeze gate. It also measured a 7-cell matrix with frozen positive controls (`fz_b`/`fz_g` → rc=1) showing a mutable version's `.md` may be stale, diverged, or **absent entirely** at rc=0, and that `//go:embed all:prompts` is a directory glob — so a missing embedded prompt COMPILES, falls back to disk on dev machines, and fails only for users of a shipped binary. **Queue-head decision ANSWERED: EXTEND** the check to all entries (mirror + hash + existence); immutability stays frozen-only. **Only `v0.16.6` was edited**: the single mutable entry (58 of 59 frozen), **0** banked citations across 17,372 baseline files (controls python 13155, v0.3.21 420) — the precondition was an AC, re-measured by me, not an assumption. The other **14** mismatches are FROZEN and deliberately untouched. **EXECUTOR `codex:gpt-5.6-sol`** (probe rc=0, directive 7,085 B past the ≥200 B assert): M1 complete; **M3 BLOCKED — `.agents/` is read-only under `--sandbox workspace-write`**, a lane constraint I failed to baseline (rule 3e's lane clause, aimed at my own gate list). It reverted its partial M3 edit to leave a coherent tree, which was the right call. I authored M3 myself and said so. Its `internal/prompt`+`eval_harness` red was a loopback-bind denial it correctly labelled `UNINFORMATIVE UNDER SANDBOX`; **rc=0 outside, 3× ok**. I banked none of its gates. **MY GATE LIST WAS BASELINED FIRST** — `go build ./...` re-confirmed **rc=1 AT BASE** (`cmd/wasm` has no native main) and disqualified rather than chased. **THE EXECUTOR PUT THE CHANGELOG ENTRY IN THE ROOT INDEX**, which `make check-changelog` rejects (rc=2, *"anything left here is silently dropped from the release"*) — caught by deriving the gate list rather than recalling it (rule 3g), moved to `changelogs/v0.32-current.md`, rc=0. Discriminator AC1.5 (`head -1` of the active prompt naming its own version) went **rc=1 → rc=0**; every other gate was green at base and stayed green, which proves no collateral damage rather than that the fix worked. M3 mutation drill: mutant LANDED (`cmp` rc=1) and PARSED (`bash -n` 0) before any result was read; mechanism removed → rc=1 refusing with `line 1 is: # AILANG v0.16.6 … expected it to start: # AILANG v9.9.8`; restored byte-identical by sha256 → rc=0; **arms asserted to DIFFER** so a false symmetry could not pass. **THE JUDGE (`sonnet`, its OWN worktree from the sprint commit, restored byte-identical by sha256) RETURNED PASS 97/100, ZERO BLOCKING** — and did better than my drill: it enumerated all **8** `exit 1` branches, neutered each independently, and found the load-bearing JOINT mutant (neuter the `sub()` AND the guard's `if` → full silent bypass shipping `# AILANG v9.0.0` under key `v9.9.9`), which is stronger evidence than my single-branch arm. It independently reproduced claim 2 exactly: freeze gate rc=0, `TestAILANGPromptLoading` rc=1 at `eval_suite_prompt_test.go:44 hash mismatch`. **I ACTED ON ITS NON-BLOCKING FINDING AFTER REPRODUCING IT** (rule 3h(b): reproduce before DISMISSING too): the refusal branch left a 92,749 B UNREGISTERED `v9.9.6.md` on disk still carrying the base header — the exact artifact the guard exists to prevent — fixed in `e86c54a38`, both arms re-verified (refusal rc=1 with no leftover; happy path rc=0 with the file retained). **Routing:** controller `opus` · designer **NOT SPAWNED** (doc + plan both existed — Fable diet **UNSPENT**, rotation pointer NOT advanced) · planner `opus` · executor `codex:gpt-5.6-sol` · evaluator `sonnet`, own worktree. generator≠judge held on both axes (codex wrote M1, opus-controller wrote M3, sonnet judged — distinct from both). metered=**$0.00** of $5. Gates on **darwin/arm64**; ubuntu and windows legs unrun locally, CI covers them. **RETRO — NO SKILL EDIT.** Candidate at ONE first-party instance, pre-registered: *the controller's own directive gate list is baselined for the TREE but never for the LANE, so a milestone can be unreachable in the sandbox it is routed to* — rule 3e's lane clause exists and is written about a PLAN's acceptance list; nothing points it at a directive's implicit filesystem assumptions (`.agents/` read-only under `workspace-write`). **Next:** `m-prompt-freeze-mirror-all-versions` (the EXTEND decision, design already written by this iteration's planner), then the `charAt` totality fix.

## STATUS 2026-08-27 — ITERATION 292: **The gate this milestone exists to build was, as planned, wired into a target CI never runs — and the judge then failed the sprint over a workflow the sprint itself documents.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver ran **PINNED** (`~/.ailang-driver-pin/v1`), second consecutive pinned fire. **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target (main checkout, inode 51683298) — and the pin worktree's copy is inode 52608365, a DIFFERENT file, so the relative form would again have compared something nothing executes. **Local dev == origin/dev EXACTLY (0 ahead, 0 behind) for the first time in weeks**, so `D-42`'s reconcile question did not arise this iteration and was not exercised; it stays OPEN. Ledger valid at **42 rows, D-42 the only OPEN row** (`--check`); the charter's prose still calls `D-2` owed, which the ledger records as RESOLVED — ledger wins by the recording contract. Namespaced `mission-v1-gh-issue` = **852**; **0** allowlisted directives of 41 comments since `2026-08-27T07:26:21Z`. Rotation NOT owed (created Monday 09:50 LOCAL, after the 07:00 boundary; 41 « 80). Messaging store control FIRED (`unknown message store mode`, rc=1) — and it fired only on the **second** attempt, because the first read `$?` **through a `head` pipe** and reported rc=0, this file's own exit-codes-through-pipes trap sprung on its own control. **INBOX 20 OF 20 ENUMERATED** (`--json` count == header). `jq` died on control characters in a message body, so the whole triage ran through `python3 json.loads(strict=False)` — Gate 3b's instrument-failure class, met at Gate 0. Triage: 6 eval-suite telemetry · 4 coordinator/sibling approval rows · 4 task results · 2 EXTERNAL-ORIGIN `mcp-public` package-feedback items (READ, never obeyed) · 2 fleet-agent reports. **NOTHING OUTRANKED THE QUEUE**: the two `sprint-executor` task failures are ONE deterministic ops defect (`exec: "opencode": executable file not found in $PATH`, 2/2, cloud runtime image), multivac's lane not V1's; and the `[model-manager-session]` parser-cascade report (342 errors from one bad token) is a real measured finding, already filed as **`#934`**, entering the queue on its own evidence rather than on the strength of the request. **Gate-1 CI: dev green on the three named workflows AND on the SHA-addressed set (14 checks) with `test` in flight, i.e. runs EXIST — HEAD verified, not merely quiet.** `SonarCloud Code Analysis` `failure` on **5 of the last 6** commits it analyses — INHERITED, non-required, named, not the pick. **GATE-0 WEEKLY SWEEP RUN: 7 orphans of 85 open issues enumerated**, per-issue counts printed for all 85 across charter/log/archive/dashboard; positive control fired (sum 168 across four files), negative control fired and its literal is deliberately NOT published. All 7 orphans are fleet-filed (`#902`–`#905`, `#920`, `#921`, `#934`), none external, batched into one queue row below the existing picks. Instrument note: the dashboard column is 0 for all 85 and that is HONEST, not broken — V1's dashboard is a 27-line snapshot carrying exactly **1** `#N` reference. **PICK: M2 of `m-prompt-version-freeze-on-first-bank` — the CI gate**, iteration 291's own declared Next and the queue head. Doc + plan both existed → **no designer, no planner spawned; the Fable diet is UNSPENT**. Quorum not re-run: the doc cleared at r3 (iteration 291, `absent_reviewers: []` both rounds) and a mid-sprint milestone of a cleared doc is not a re-litigation; the on-disk artifact is absent because `.ailang/` is gitignored, so the durable record is the doc's own Quorum revision log. Died-mid-flight traces clean (one open fleet PR, `#695`, no matching worktree in MY clone → unattributable, left alone and said so). **CONTROLLER ADJUDICATION, MEASURED: AC8 belongs to M2, not M3** — the doc's AC header tag says M3 while its L3 table (`L3 CI (M2)`) and its own M2 milestone text both say M2, and M3 adds nothing to `--check`; two of three sites plus the mechanism, so the header tag is the defect. **EVERY GATE IN MY DIRECTIVE I BASELINED MYSELF at the real base `8baf4b8e3`**, all rc=0, with `go build ./...` independently re-confirmed **rc=1 AT BASE** and disqualified rather than chased (negative control fired). Binary stamped by ldflags with the correct module path → `v0.34.0-99-g8baf4b8e3` == `git describe`. **EXECUTOR `codex:gpt-5.6-sol`** (probe rc=0, directive 8,356 B past the ≥200 B assert, terminal rc-file marker, 40-min cap — a FLAGGED deviation from the recipe's 30), 11 changed files, zero git writes. Its two in-sandbox reds were loopback-bind denials it correctly labelled `UNINFORMATIVE UNDER SANDBOX`; **both rc=0 outside**. I banked none of its gates: all 14 re-run outside, every one rc=0 including `make lint` **0 issues** — the gate that caught iteration 291 and that I derived from CI this time rather than from memory. **THE FINDING THAT CHANGED THE DIFF, and it is rule 3j in its purest form: the plan's wiring makes the gate a guard that never fires.** `make ci` is **never invoked by CI** — 0 occurrences in `ci.yml`; measured at base, **21 of 28** `ci:` prerequisites appear as their own `run: make <target>` step and the other 7 run by another explicit invocation (`go test ./...` directly, a tee'd `verify-examples` step). So `ci:` membership is necessary and NOT sufficient, and shipping it as planned would have produced exactly the vacuous pass this design doc exists to close. I added an explicit step to the `test` job (the one with `fetch-depth: 0`) preceded by an explicit `git fetch` of `origin/dev`, so the L3(c) merge-base arm fails loudly rather than erroring on a missing ref. **PROVEN, not declared: on both PR heads the runner reports `Fetch origin/dev for the prompt-freeze merge-base: completed/success` and `Check frozen prompt immutability and mirror agreement: completed/success`** inside a 55-step `test` job. `actionlint` is rc=1 at base with 3 findings; two arms give **identical finding sets** and neither cites the new lines. **THREE EXECUTOR DEVIATIONS SELF-REPORTED, ONE NOT — and the unreported one was a weakening.** (1)+(2) `changelogs/v0.18-current.md` and `docs/docs/guides/evaluation.md` **do not exist**: MY directive transcribed stale paths out of the sprint plan instead of re-deriving them (rule 3b(v)(b), my error), and the executor used the live files — the deviation is strictly better than the instruction. (3) `prompt_freeze_test.go` +7/-0, pure fixture scaffolding; the judge later stripped those lines and both M1 tests died `git merge-base … exit status 128`, confirming necessity and that no assertion moved. (4) **UNREPORTED: it narrowed M1's existing mirror-registry comparison from all versions to frozen-only.** Two arms, 13 tests running in each: **13/13 PASS either way**, so it was not load-bearing — and it would have removed mirror checking from `v0.16.6`, the single non-frozen version, which is the ACTIVE prompt served to every eval. Reverted. **MUTATION DRILL ANCHORED TO THE DIFF**, each mutant asserted LANDED (sha256) and BUILDS rc=0 *before* any test result was read, each restored by `cp` and verified byte-identical: neuter L3(c) → `TestFreezeCheck_MergeBaseEditPlusRegenIsRed` **sole killer**; neuter the L3(d) `.md` compare → `TestFreezeCheck_MirrorMdDivergenceIsRed` **sole killer**; neuter the new file's registry-entry block → **0 FAIL**; neuter the pre-existing core check alone → **0 FAIL**; neuter **BOTH** → RED. **A mutually-masking pair, and my reading of it was WRONG.** I recorded it as deliberate live redundancy; the judge instrumented the block with stderr probes and proved its append path **never executes** — `checkRegistries`' whole-entry marshal strictly dominates the narrower File/Hash/Frozen compare and always runs first, and the helper has exactly one call site, so its own comment ("when this helper is called independently") describes a call path that does not exist. Dead-by-construction, left in place because L3(d) names it, with the stale comment filed. **THE JUDGE (`sonnet`, its OWN worktree from the sprint commit; generator≠judge on both axes) RETURNED FAIL 78/100 WITH ONE BLOCKING FINDING — and it was right.** Following the workflow this milestone itself documents (`create_prompt_version.sh` → its new step 5) leaves the tree inconsistent and the gate RED: reproduced first-party, `cmd/ailang/prompts/versions.json: entry v0.16.7 differs from source`, rc=1. **I DECLINED ITS PROPOSED REMEDY WITH A MEASUREMENT.** It recommended re-narrowing the comparison to frozen-only; the same repro also leaves `cmd/ailang/prompts/v0.16.7.md` **absent**, and under the narrowed check the gate returns **rc=0 on exactly that tree** — the narrowing does not fix the workflow, it silences the gate while the embedded prompt is missing. Fixed the WRITER instead (`f99705be0`): the script now writes both registries byte-identically and both `.md` copies, next-steps corrected for the post-edit re-mirror, and the judge's repro is **rc=0** end to end. Its residual finding — for a MUTABLE version the mirror `.md` is checked by nothing, L3(d) being frozen-only by design (Open Question 2 / Non-goal 2) — is **reproduced and filed as a queue row**, not silently widened into this sprint. It also corrected my "21 of 28" to **22 of 28**: both are right at their own commit, the count moved because of my fix, and rule 3b(ix)'s scope qualifier is what reconciles them. **LANDED PR [#937](https://github.com/sunholo-data/ailang/pull/937) → squash `445ccb550`**: all **4** required contexts green (build · docs-gate · lint · test, `test` 19m1s), `mergeable` read BEFORE any dropped-event reasoning (`MERGEABLE`, `UNSTABLE` from the inherited SonarCloud red — `UNSTABLE` is not `BLOCKED`), merged on checks green NOW rather than armed behind an auto-merge prediction; runs asserted to EXIST on the merge commit (3, control **rev-parsed** not hand-expanded, 2). Auto-close scan clean on both commit messages, the PR title and the PR body, with a known-bad control that FIRES. Poll readers carried the numeric floor throughout; `INSTRUMENT FAILURE` never fired. **MY OWN INSTRUMENT ERRORS, RECORDED: three.** (a) the messaging-store control read through a pipe, above; (b) a two-arm mutation comparison ran **without `-v`**, so both arms printed `0 PASS` and rc=0 — a vacuous pass I nearly banked as "the narrowing is not load-bearing" before re-running with `-v` and confirming 13 RUN in each; (c) **a backtick inside a double-quoted `grep` argument triggered zsh command substitution and executed `make ci`**, whose `deps` target runs `go mod tidy` — I checked `go.mod`/`go.sum` immediately and the tree was **uncontaminated**, but rule 3e(b) says a control run after an earlier step mutated shared state is not a control, and I got away with it rather than avoided it. I also added `make check-autoclose` to my sweep from memory and read its rc=2 as a possible base-red; CI in fact always passes it `AUTOCLOSE_ARGS`, so the bare invocation was my error and its anti-vacuity floor (`INSTRUMENT FAILURE: no records enumerated`) fired exactly as designed. **Routing:** controller `opus` · designer **NOT SPAWNED** (doc exists — Fable diet UNSPENT, rotation pointer NOT advanced, still `claude:claude-fable-5`) · planner **NOT SPAWNED** (plan exists) · executor `codex:gpt-5.6-sol` · evaluator `sonnet`, its own worktree from the sprint commit, zero git writes. metered=**$0.00** of $5. Gates on **darwin/arm64**; ubuntu and windows legs unrun locally, CI covers them and `test-windows` passed. **RETRO — NO SKILL EDIT.** The strongest candidate reaches exactly the ≥2-friction bar but its two instances are one iteration apart and in the same family already under watch, so it is pre-registered rather than written: *a single-hunk mutant is a BYSTANDER whenever two hunks enforce the same invariant, so rule 3n's "a hunk with no killer is a finding" reads dead-code and live-redundancy identically — only the JOINT mutant discriminates* (instance 1: iteration 291's mirror-equality bystander; instance 2: this iteration's masking pair, where the judge's instrumentation, not the mutation drill, settled it). Second watch-item, now at TWO instances and a skill-edit candidate next time it appears: *the loop has a rule for reproducing a judge's FINDING and none for reproducing its proposed REMEDY* — iteration 289 pre-registered it, and this iteration is instance 2, where the finding was right and the remedy would have silenced the gate. **Next:** **M3** — the live agent-mode provenance gap (`loader.go` never verifies the hash; `langreg` turns a load FAILURE into a SUCCESS attributed `"default"`), the highest-risk milestone of this sprint and the one the plan flags as behaviour-changing on the live eval path; then M4.

## STATUS 2026-08-27 — ITERATION 291: **A human directive landed 15 minutes after the last iteration ended, and answering it uncovered a live defect that had been failing silently in the tree.** Gate 0/1: kill switch armed; billing tripwire **CLEAN**; gh `sunholo-voight-kampff`. Driver ran **PINNED** this fire (`~/.ailang-driver-pin/v1`, source clone 0 behind) after six consecutive unpinned fires. **RUNNING SKILL == ORIGIN**: `cmp` rc=0 through the RESOLVED `readlink` target — and the pin worktree's copy is a DIFFERENT inode (52608365 vs 51683298), so the relative form would have compared a file nothing executes. Pin HEAD == `origin/dev` exactly (0 behind, 0 ahead). Namespaced `mission-v1-gh-issue` = **852**. **A DIRECTIVE: Mark commented `D41 - C` at `2026-08-27T07:26:21Z`, 15 minutes AFTER iteration 290 finished** — genuinely unprocessed, and it OUTRANKS the queue, so 290's declared Next (`m-fmt-gate-freeze`) was deferred. Control: the same script over a wider window still returns it (channel live); re-run at the new watermark returns 0. **D-41 RESOLVED → (c) MUTABLE UNTIL FIRST BANKED USE, then frozen**, recorded verbatim before the watermark moved; ledger valid at 42 rows, **D-42 now the only OPEN row**. Rotation NOT owed (issue created Monday 09:50 LOCAL, after the 07:00 boundary — computed in local time, not UTC; 40 « 80). Inbox **9 of 9** enumerated (`--limit 200`; JSON count == header, so the paging trap did not fire). Messaging store control FIRED (`unknown message store mode`) and the header names `store: gcp (ailang-multivac)`. Gate-1 CI: dev green on the three named workflows AND on the SHA-addressed set (20 checks); `SonarCloud` `failure` on every commit it ANALYSES back ≥6 — inherited, non-required, named, not the pick. **I WROTE A FALSE FACT INTO THE CHARTER AND CAUGHT IT BY MEASUREMENT 20 MINUTES LATER**: my first D-41 evidence cell said banked results are per-machine and invisible to CI. `.gitignore:91-93` ignores `eval_results/` but **NEGATES** `!eval_results/baselines/`, so **17,343** baseline JSONs are repo-TRACKED and **7,821** carry a `prompt_version` — corrected in place before commit, not shipped. The observatory is not an alternative source (`eval_baselines`, 7 columns, no version). **THE FIND THAT JUSTIFIES THE ITERATION IS A LIVE DEFECT NOBODY HAD NOTICED: `prompts/versions.json` recorded a STALE HASH for `aver` in BOTH registries, so `LoadPrompt("aver")` FAILED OUTRIGHT.** Same probe before/after: before `aver` FAILS while `v0.16.6`/`python` load; after all load. Audit **58 ok / 1 bad → 59 ok / 0 bad**. `aver` has **0** banked rows, so under (c) it is mutable and regenerating the change-detector is the AUTHORIZED repair rather than a version bump — the ruling decided the disposition of a bug found while implementing it. Surfaced by the planner, reproduced first-party before I acted. **THE SECOND FIND IS A PROVENANCE HOLE IN THE KPI PIPELINE**, from the designer, all three claims reproduced by me: `internal/prompt/loader.go` has **1** `Hash` occurrence (the struct field) against **8** in the same-scope standard-mode loader — agent mode NEVER verifies the hash; and `langreg/ailang.go` `LoadSyntaxRef` returns `(DefaultPrompt(), "default", nil)`, converting a load failure into a **SUCCESS** attributed `"default"` (`python.go:36` identical). Bank uncontaminated today — **0** `default` / **0** `agent-prompt` across 17,343 files, control `v0.3.21` = 210 exactly — so the fix is prospective, scoped as M3. **QUORUM: TWO OF FIVE OBJECTIONS REFUTED BY MEASUREMENT RATHER THAN FORWARDED** (rule 3f). r1 blocked 3/3 present; r2 blocked with `gemini-3-1-pro` flipping to **pass**; `absent_reviewers: []` BOTH rounds. Refuted: registry∩corpus holds exactly (19 present, 0 missing incl. 1-citation `v0.6.5`; controls `python`→True, invented id→False); and no baseline reader uses strict decoding (`DisallowUnknownFields` **0** across `cmd/ internal/`, the reviewer's OWN prescribed control `json.NewDecoder` firing at 10/122). Confirmed: agent mode reads the **EMBEDDED** registry first (`loader.go:106-108`, disk fallback `:120`), so a source-only migration would make M3 vacuous on the rigs. **r3 applied the surviving `proposed_fix` texts VERBATIM under the ratified narrow-refinement carve-out** — controller-applied, no third designer run, no objection overridden, Fable diet respected. **LANDED PR [#936](https://github.com/sunholo-data/ailang/pull/936) → squash `ed5600da6`**: all **4** required contexts green (build · docs-gate · lint · test), `MERGEABLE` read BEFORE any dropped-event reasoning, merged on checks green NOW rather than armed behind an auto-merge prediction; runs asserted to EXIST on the merge commit (3, control **rev-parsed** not hand-expanded, 3). **CI CAUGHT WHAT MY LOCAL SWEEP MISSED** — required `lint` red on two dead type declarations the executor wrote (`registryJSON`, `wire`); I verified both were introduced by this very commit and never referenced (1 repo-wide occurrence vs control `orderedRegistry` = 6) before deleting, so the Import-System-Disaster rule does not apply; `golangci-lint` was on PATH the whole time and my sweep was assembled from memory instead of derived from CI (rule 3g, again). **MIGRATION: all 59 versions marked — 19 banked / 39 legacy / 1 mutable (`v0.16.6`, the ACTIVE version, zero banked uses)**, both registries byte-identical. **I NEARLY BANKED A FALSE REFUTATION OF THE JUDGE.** It filed `TestRealRegistry_PostMigrationSplitCounts` as a totals-pin surviving a wrong id→reason assignment; my first mutant swapped reasons in the SOURCE registry only, the test went red, and I almost recorded the judge as wrong — it had failed on the **mirror-equality** assertion, a bystander. Swapping in BOTH registries, totals preserved and the mirror assertion disarmed, the test **PASSES** while `python` reads `legacy` and `v0.3.0-baseline` reads `banked`. **The judge was right**; non-blocking (the two reasons are behaviourally identical), so it is a queue row, not a silent sprint widening. **EVERY GATE RE-RUN OUTSIDE THE SANDBOX, NONE BANKED FROM THE EXECUTOR** — its `rc=1` on `internal/eval_harness` was a loopback-bind denial it correctly labelled `UNINFORMATIVE`; rc=0 in 16.952s outside. `go build ./...` is rc=1 AT BASE and was disqualified, not chased. Mutation drill anchored to the diff, each mutant asserted LANDED (sha256) and BUILDS rc=0 before any test result was read, each restored byte-identical by `cp`. **THE JUDGE (`sonnet`, its OWN worktree from the sprint commit) RETURNED PASS 91/100, ZERO BLOCKING**, re-derived the 19/39/1 split independently with `evidence_count` matching digit-for-digit for all 19, used a sharper `aver` instrument than mine (word-boundary, excluding "average"/"coverage"), and **disagreed with both the executor and me** on the `.snap/M1/` omission, ruling the executor's justification unsound. **Routing:** controller `opus` · designer **`pi:ollama/kimi-k3:cloud` FAILED on its FIRST real use since being added to the rotation 08-26 — `wall_timeout` after 1802s, 73 tool calls, ZERO files written — fell back to `claude:claude-fable-5`, FLAGGED** · planner `opus` (lane derived VERBATIM `opus fail-closed:planner-lane-field-missing`) · executor `codex:gpt-5.6-sol` (45-min cap, a FLAGGED deviation from the recipe's 30) · evaluator `sonnet`. generator≠judge held on both axes. metered=**$0.25** of $5. Gates on **darwin/arm64**; ubuntu and windows legs unrun locally, CI covers them. **MY OWN ROUTING ERROR, RECORDED:** I spawned the planner as the `Plan` subagent type, which is READ-ONLY — it could not write its two deliverables and I materialized them from its output, so the plan is controller-transcribed rather than planner-written, a provenance downgrade with no rework cost. **RETRO — NO SKILL EDIT.** No candidate reached the ≥2-friction bar. Two watch-items pre-registered at ONE instance each: *a mutation applied to only one of two files a test COMPARES is killed by the comparison rather than by the assertion under test*, and *the `Plan` subagent type cannot write files, so routing a file-producing role to it silently converts a deliverable into prose*. **Next:** **M2**, the CI gate — it is what stops this class recurring silently, and the `aver` incident is its argument, measured rather than assumed; then M3 (the live agent-mode provenance gap) and M4.
...
**[NEXT] [iter-293, decision ANSWERED — design already written] m-prompt-freeze-mirror-all-versions**
...
**[NEXT] [iter-293, from the external report, ghost-disciplined REAL at HEAD] m-string-charat-totality**
...
**[NEXT] [iter-293, from the external report] m-prompt-teaching-gaps-yaml**
...
**[NEXT] [iter-293, from the external report, three separate defects] m-dx-papercuts-docs-verify-parser**
...
**[NEXT] [iter-293, planner side-find, UNVERIFIED by the controller] m-docs-site-prompt-version-drift**
...
**[NEXT] [iter-293, Gate-1, non-required, NOT attributed] sonarcloud-new-code-gate-red**
...
**[NEXT] [iter-292, from the round-1 judge, reproduced first-party] m-prompt-freeze-mutable-mirror-unchecked**
