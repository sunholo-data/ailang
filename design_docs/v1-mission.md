...
## STATUS 2026-09-07 — ITERATION 347: **D-60 shipped: the three notification paths are bounded, the seven notify assertions are restored — and clearing that red exposed nine older ones the suite had been hiding.** Picked on two agreeing grounds: the standing `launchd drivers (bash 3.2)` red on `dev` (V1 owns this repo) and Mark's attended ruling **D-60 option B** (*"prefer this reliability repair early in the week"*), which also discharges the D-12 obligation iteration 346 left open — the row was still tagged PARKED. Reproduced at base `81abc956d` before routing: 20 passed/7 failed, rc=1, names matching the doc's causal table. **The design's central fact was measured here, not inherited:** `_mc_bounded` runs `( exec "$@" )` and **`exec` bypasses shell functions** (`( exec f )` → `exec: f: not found`; direct call prints `FUNCTION-CALLED` as the control), so D-60's production half and the fixture's function stubs are in direct collision, and the inherited doc said nothing about it. Designer `pi:ollama/deepseek-v4-flash:0731-cloud` (rotation entry after astra) revised three times; quorum r1 BLOCKED, r2 `gemini-3-1-pro` **PASS** with `gpt6-astra` and `oc-glm-5-2` rejecting on completeness only, r3 closed under the **narrow-refinement carve-out** applying both verbatim fixes. PR [#1090](https://github.com/sunholo-data/ailang/pull/1090): M1 restores observability on a UNIFIED `MC_TRACE_FILE` medium (20/7 → 30/0), M2 bounds the three D-60 paths at `NOTIFY_TIMEOUT` default 30s with an `env` prefix (`exec` cannot parse `VAR=val`) (30/0 → 37/0), M3-fixes close the judge's blockers (→ **38 passed, 0 failed**). **Bounding would have silently deleted a diagnostic and that is fixed too:** a cut-off command produces no output, so `MC_BOUNDED_OUT` is empty and the WARNING degraded to `FAILED … after 3 attempts:` — synthesis is now gated on an empty tail with any non-zero rc, covering `mktemp` rc=125. **CORRECTED AFTER THE FACT — the "nine hidden reds" finding was TRUE AT MY BASE AND ALREADY FIXED ON `dev`.** `make test-launchd-drivers` halts at its first failing suite, so while the notify suite was red **nothing behind it ran**; green, the target reaches three more suites (84/0, 17/0 — never executed on `dev` before — and `test_mission_heartbeat.sh` failing 9 arms, paired-controlled identical at base and head). The CLASS is real and is kept. The ROW is not: `063df9917`, inside **attended** PR #1082 merged at 14:15Z *while this iteration ran*, repaired exactly those arms — verified on `dev`, heartbeat 25/25 rc=0 and the whole target rc=0. The same PR also landed a competing test-only fix for MY item (`2a38f0948`), which D-60 had already rejected as a scope; it is not composable with the production bounding (`_mc_bounded` execs, so in-shell function stubs are unreachable), so the sprint SUPERSEDES it and carries its note forward. After rebasing onto that work, `make test-launchd-drivers` is **rc=0** at the sprint head with notify at **38/0** vs dev's 27/0, and the merge SHA `e5a325a20` came back **21 checks, zero not-green**. The lesson I am recording against myself: a measurement of *what else is broken* taken at a stale base has a shelf life, because that is exactly what other people are also fixing. **M3/M4 NOT delivered** — executor `wall_timeout` rc=13 at the 30-minute cap mid-M3; the partial failed its own tests, was discarded, and is banked at `~/.ailang/state/mission-v1-iter347-m3-partial/`. Independent judge `sonnet` **LAND 80/100** with **two BLOCKING** findings, both REAL and both reproduced by me before action: `run()`'s own RC-capture fix had zero coverage (the mutation left the suite green at 37/0), and the store/project guard was not hermetic (green with the CLAUDE.md-mandated ambient exports, red without). Both fixed and each proven non-vacuous by re-running its mutation. **Ruled out, and the error was mine:** I flagged the designer's line numbers as wrong — refuted, my grep had run in the pin worktree, which carries local-only driver commits shifting the file five lines. N=12, goal unmoved (HARNESS). Metered **$0.80** of $5. Ledger 60 rows, **ZERO open** — nothing waits on Mark. Full record: v1-mission-log.md iteration 347.
...
**[NEXT] [iter-348, from the design's own out-of-scope section, with the evidence] `m-coordinator-test-parallelism` — `internal/coordinator` runs 732 top-level tests with **zero** `t.Parallel()`, and it opens real SQLite stores and spawns processes on the slowest runner in the matrix.**
