...
## STATUS 2026-08-31 — ITERATION 308: **Two dev reds, one landed and one that refused a fourth calibration.** Gate 0/1: kill switch armed; billing CLEAN; gh `sunholo-voight-kampff`; running skill == origin (resolved symlink, not the pin's own copy); canonical cloud inbox empty with the invalid-store control firing; 0 directives since watermark. dev was RED at `0b35abd5d` on four required jobs, two causes, both absent from its green parent. Gate-2 traces found TWO orphaned iterations — 306 (PR #969, a fire that logged `rc=0` with zero charter/log rows) and 307 (PR #970). **Red A LANDED:** #969 verified first-party (red reproduced with a freshly built `-ldflags` binary; mutant BUILDS and reds on the inventory diff; byte-identical restore) and merged as `c29ec1d00`, Gate 3b green; #970 closed as the strictly narrower duplicate. **Red B PARKED, and the park is the finding:** the pi executor correctly decoupled the shared lane/discovery deadline (verdict `wall_timeout` rc=13 — FLAGGED — but with a complete in-scope diff, so it was verified rather than discarded; all 4 baselined gates rc=0, determinism drill 5/5 under 8 spinners). The sonnet judge then failed it twice — **62/100** on a pre-existing over-subscribed assertion (B1), then **35/100** hard-fail when B1's own remediation reddened CI — and the decisive datum came last: commit `8a384e81b` ran **success at 05:55Z and failure at 06:33Z**, byte-identical tree, plus a third distinct failure mode. My own "CI-green, proven" claim was retracted one run later. #971 is draft; three measured modes filed as #975. dev is GREEN at `8bb982220`, only the standing non-required SonarCloud red remaining. Weekly sweep: **5 orphans of 90** enumerated (negative control fired, fresh literal unpublished). Bookkeeping thread rotated #852 → **#972**. D-49 and D-50 remain OPEN. metered=**$0.00**.
...
**[IN-SPRINT &mdash; iter-310: design LANDED and quorum-cleared; sprint plan routed] `m-registry-interface-hash-blind-to-signatures` &mdash; a BREAKING API change is classified `patch` to every downstream consumer.**
