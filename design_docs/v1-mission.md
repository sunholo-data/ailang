...
- **Downstream-consumer reports** (all bot-imported via the agent-message channel, so they carry
  ...
  **the message is the bug**. The fix already existed at `internal/testing/executor.go:283-286`, the ONLY
  ...
  `#672` (housemove2026: `eparse thread` msg-id format — eparse-side, outside `MISSION_REPO`), `#694` (`ailang editor install
  vscode` installs an extension VS Code marks obsolete — **in-repo**, `cmd/ailang/editor.go`, well-diagnosed with a
  verified workaround), `#656` (`/api/v1/convert`, feature — ailang-parse-side).
