...
- **DX tooling** (Mark: both in → resolved 2026-07-18: M-TOOLING-DETERMINISTIC **CLOSED-SUPERSEDED**
  ...
  **m-ailang-fmt-phase2 [DOC CREATED + QUORUM-BLOCKED ×2 → PARKED needs-human-review; iter 59 (2026-07-19), Mark-greenlit]**
  — Phase-2 lossless comment preservation, the UNBLOCK for fmt on the 94.7% commented corpus. Doc authored +
  revised iter-59 (`design_docs/planned/v0_30_0/m-ailang-fmt-phase2.md`, `ad14dfc19`); design pivoted to a
  token-anchored envelope (AST spans proven unusable at design time). Quorum BLOCKED both rounds — R2 residual
  fixable defects: (a) "393 files parse" premise rests on Phase-1-fmt sweep (refuses 372 pre-parse) → re-verify
  parse-validity via `ailang check`; (b) left-widening rule over-consumes the parent open-delimiter (`[ /* C */ x ]`
  traps `C` in `x`). **Human fork (#399):** authorize one short revision to fix (a)+(b), or amend the design. See
  the doc's ⛔ Quorum Block. ·
  **m-ailang-fmt-adoption [DOC CREATED + QUORUM-BLOCKED ×2 → PARKED needs-human-review; GATED behind m-ailang-fmt-phase2; iter 59]**
  — discoverability + opt-in hooks. Doc authored iter-59 (`…/m-ailang-fmt-adoption.md`, `ad14dfc19`). Quorum
  BLOCKED both rounds — R2 residual fixable defects: (a) PostToolUse hook has no timeout (bounded-waits); (b) the
  first `jq` call still `2>/dev/null` (residual silent-swallow). Human fork: authorize one short revision. Original
  scope retained below.
