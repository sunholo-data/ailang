...
| 4 | Role/fallback resolution | `derive-planner-lane.sh` (182), `resolve-role-spawn.sh` (79) | `ailang models role` | **NO** | Re-verified 2026-09-21: **still no shell script consumes `models role`**, despite `models_cmd.go:79` claiming "the mission driver reads field 2". Two role tables, independently maintained. **Unifying them is M-MODEL-REGISTRY-SINGLE-SOURCE M8, PARKED by Mark 2026-08-27** — not unowned work, and not cheap: the park note records that the registry as it stands is *worse* than the shell table (it cannot express three-deep chains, the `pi` harness, the ollama-cloud flat-rate tier, or a mission-form string that is not `agent_cli:agent_model_name`, and `claude:fable` would silently downgrade the designer to opus). The 2026-09-14 handover calling row 4 "the cheapest real win" was wrong on that point. **What HAS been done instead, and is the actual precondition for this row:** the binary's half of the table was silently WRONG for all four roles, one role at a time — evaluator `7423434b4`, executor `e9e8ce32e`, designer+planner `5f67aa532`. `TestResolveRole_EveryRoleHasASkillCapableRung` now asserts the property over all four roles and both lanes, so the class cannot recur silently. Retiring this row still needs M8 un-parked. |
