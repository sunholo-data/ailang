...
| T6 | Invert the comparison (`==` → `!=` at the entry loop) | the SUITE refuses at startup, before arm 1: `not ok - REAL_LSOF resolved outside getconf PATH: /usr/sbin/lsof …` | not an arm killer; the gate's own refusal is the red. Listed so nobody expects arm 44/45 to name it | UNMEASURED — predicted from V15 |
...
| AC5 | `grep 'run_lane evidence' /tmp/s.out \| grep -o 'fixture-[0-9]*\\\|survivors=[0-9]*\\\|outer_cap_fired=[a-z]*' \| paste -sd' ' -` | one fixture, `fixture-2861 survivors=0 outer_cap_fired=no` | two: `fixture-2861 survivors=0 outer_cap_fired=no fixture-2863 survivors=0 outer_cap_fired=no` |
