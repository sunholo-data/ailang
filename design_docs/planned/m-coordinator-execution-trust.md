...
6. **Expect the clone step to throttle (V32).** Several image builds in one session is enough.
   The failure names credentials but is rate limiting; authenticating the clone (or caching the
   checkout) would remove a hard dependency on an anonymous fetch of a 161 MB repo.
