...
| **V32** | **The image pipeline clones a 161 MB public repo UNAUTHENTICATED on every build, and fails closed when GitHub throttles it.** `cloudbuild.yaml` step `clone-ailang` runs a bare `git clone --depth 1 --branch dev https://github.com/sunholo-data/ailang.git` with no credentials and no retry. After several builds in one morning it began returning `fatal: could not read Username for 'https://github.com': No such device or address` — GitHub answering 401/403 to a throttled anonymous client, which git reports as a credential prompt | build `c0a63ee9` cloned fine at 08:53; three attempts failed identically from 09:48 (incl. one after a 6-minute cooldown). **Two controls make it conclusive:** (a) an anonymous `git ls-remote` from a laptop succeeds and returns the expected `dev` SHA, so repo and branch are fine; (b) **in the same minute (09:48), `ailang-core-dev` and `ailang-registry-validator-dev` — which fetch source through Cloud Build's authenticated GitHub App connection — both SUCCEEDED**, while the three unauthenticated `ailang-multivac-dev` clones failed. Same egress, same window; the only difference is authentication | **A rollout-blocking fragility, not a code defect.** Every image build depends on an unauthenticated third-party fetch of a large repo from shared egress. The error text names credentials, which sends you looking for a broken token that does not exist |
...
| **V37** | **M7 VERIFIED LIVE IN PROD, on the scenario that produced V36.** Coordinator restarted mid-flight, did NOT cancel the task, and the completion was accepted and posted instead of discarded | prod 2026-09-02, `task-38fd0175`: `14:19:13 stale task detector: stopped` → `14:19:13 ✓ Coordinator daemon started` → **no `Recovered N stale task(s)` line** → `14:19:47 CompletionHandler: received completion (status=failed)` → `→ failed (error=pi exceeded hard timeout (15m0s))` → `posted completion notification`. Banked in the inbox | **The two runs prove it from opposite directions**: V36's task SUCCEEDED and was discarded; V37's task FAILED and was reported. What changed is not the outcome but whether an outcome comes back at all |
| **V38** | **The executor's hard timeout is 15 minutes, and that is now the binding limit on task size.** `task-38fd0175` was asked to be thorough and died on `pi exceeded hard timeout (15m0s)`; `task-c8126248` finished at ~16m47s only because nothing was enforcing the ceiling on its path | both prod runs | Relevant to the stated goal: "general async tasks needed by downstream apps" will hit this. It is per-agent config (`AILANG_TIMEOUT`, `agent.Timeout`), not a code limit — but it is a **deliberate choice nobody has made yet**, and the default silently caps what the plane can be asked to do |
| **V39** | **The ceilings live in two pipelines, and `promote-to-prod` carries only one of them.** Agent timeouts ship with the coordinator CONFIG (deployed to the bucket); the Cloud Run `taskTimeout` is TERRAFORM. `promote-to-prod` copies images and redeploys jobs — it does not run terraform. So promoting raised prod's agent ceiling to 2h while leaving its platform ceiling at 3600s | verified live after the promote: prod bucket config `33 agents at "2h"`, prod job `taskTimeout: 3600` | **This is the inverted state the guard exists to prevent, reached by shipping the two halves down different paths.** Only `ailang-multivac-prod` (full build + terraform apply) can raise the platform side |
| **V40** | **A green pipeline meant "the thing I wanted is true" three separate times today, and was wrong each time.** (a) the terraform `variables.tf` default I changed is inert — every env overrides it in tfvars, so a SUCCESS build left dev at 1800s; (b) a dev-plane absence-of-log looked like M7 verification until the positive control showed that line has never appeared there; (c) a successful promote left prod's platform ceiling untouched | all three caught by reading the LIVE value rather than the build status | **The habit, stated once: verify the artefact, not the pipeline.** Each of these would have shipped silently |
| **V41** | **The codebase sweep Mark's V29 report asked for is complete: no other command shares the silent-misroute defect.** (a) `normalizeArgsForFlags` has exactly two callers — `messages send` (`messages_send.go:58`) and `exec` (`exec.go:105`) — both fixed in M5. (b) The only other `strings.HasPrefix(arg, "-")` sites (`mission_activation.go:74`, `mission_confirm_stopped.go:34`, `mission_iteration_cmd.go:96`, `mission_retry_review.go:42`) are NOT value-consumers: each is a fail-loud duplicate-flag detector that delegates parsing to `fs.Parse`, and Go's flag package takes a value flag's next token unconditionally — so a dash-leading value cannot shift routing there | read all six sites; `grep -rn "normalizeArgsForFlags\|HasPrefix(arg, " cmd/ internal/` — no other hits | **The dash-means-flag assumption survives in those four detectors only as a loud false-positive edge:** a flag VALUE that itself looks like `--name=…` colliding with a real flag name (e.g. `--attestation "--work-item=oops"` after a real `--work-item`) errors `duplicate flag` instead of misrouting. Fails loud, so recorded and deliberately left as-is — M5's own rule, applied to M5 |
