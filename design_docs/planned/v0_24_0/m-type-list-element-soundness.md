...
- **Round-2 gap (open, tracked by `TestListElementSoundness_KnownGap_OptionExtraction`):** values extracted from a polymorphic constructor returned by a function — `match getObject():Option[Json] { Some(name) => …[name]… }` (the `json_parse` shape) — still decouple through generalization. Same root family, but a structural binding rather than a class constraint.
