...
- **Round 3 (CLOSED — compound fields):** `type Bag[a] = Bag([a])` with an annotated scrutinee — `passthru(b: Bag[int]) { Bag(xs) => …xs… }` — type-checked because the **factory-scheme builder** only remapped a field when the *whole* field was a bare type var; a compound field (`[a]`, `(a,a)`, `Option[a]`) kept the *declared* param name, disconnected from the result type's renamed var, so the destructured value escaped its concrete type. The real fix is one level up from round 2's verdict recipe: a shared `internal/types.BuildConstructorFactoryScheme` (used by both `pipeline.buildConstructorFactoryTypes` and `repl.registerAdtFactory`, deduping them) that **substitutes the declared-param → result-var mapping across the *entire* field type** (identity on bare-var/concrete fields), with a free-var safety net so schemes stay closed. Tests: `TestListElementSoundness_CompoundField` (list/tuple/Option fields reject) + `_NoOvertighten`. Verified: full suites green, `make verify-examples` at 181/5 baseline, zero false positives (valid compound-field uses + genuine polymorphism still compile).
- **Round 4 (CLOSED — let-generalization side-condition):** the `join`/`concat`/`++` wrapper leak — reproducible with **no constructor pattern at all** (`func f(b) -> string { concat([b]) }`, `f(5)` crashes in `_str_join`) — was **not** `_str_join`-specific: a plain `needStrs(xs: [string])` leaks identically. Live tracing (worktree-isolated) showed the mechanism precisely: the non-atomic arg `[b]` is ANF-lifted to `let $tmp = [b] in g($tmp)`; `[b]` has element type α (the enclosing param `b`'s type, bound by the surrounding lambda), but `generalizeWithConstraints` generalized **every** free var (its own comment admitted *"simplified generalization … would check against environment free vars"*), so `$tmp:[α]` became `∀α.[α]`. Re-instantiation at the use site minted a fresh element var, severing the link to `b`, and `f` over-generalized to `∀α. α -> string`. Fix (`typechecker_functions.go`, `typechecker_core.go`, `inference.go`): apply the HM side-condition precisely — withhold the vars introduced by binders *inside the declaration* = `freeVars(currentEnv) \ baseEnvFreeVars`, where `baseEnvFreeVars` is snapshotted at the decl's `InferWithConstraints`/`CheckCoreExpr` entry. The subtraction is **essential**: AILANG's shared module env carries spurious "free" rigid type-parameter names (`a`, `k`, `v`, …) leaked from other bindings' schemes; naively subtracting the whole env breaks `map`'s own `a` generalization (occurs-check in `zipWith`). For a top-level decl `currentEnv == baseEnv` so nothing is withheld (full generalization preserved); only enclosing-lambda params are held back. Tests: `TestListElementSoundness_WrapperListLiteral` (+ `_NoOvertighten`). Verified: full suites green, `make verify-examples` at 181/5 baseline, valid string-joining + polymorphic wrappers (`zipWith`) still compile.
