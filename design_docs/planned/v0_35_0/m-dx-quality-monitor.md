...
| V9 | **No existing extension in this repo monitors output quality, detects tool-call loops, or rewrites oversized results** | Historical pre-implementation check: `grep -il "compact\|quality\|hallucin" .pi/extensions/*.ts` → empty; the then-current README table listed gate, freshness, sprints, dirty-warn, builtin-finish, quota, lsp-lite, and prepush | Confirmed (negative-existence at verification time) |
