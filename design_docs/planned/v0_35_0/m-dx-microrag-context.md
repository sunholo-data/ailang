...
**Status**: Implemented (pi-layer M1–M3 landed 2026-08-31; D3 fallback measured unneeded for real diagnostic shapes — see implementation note; rig A/B pending next rotation window)
**Target**: v0.35.0
**Priority**: P1
**Estimated**: ~1 session (M1+M2 ~0.5d, M3 ~0.25d, e2e + A/B ~0.5d)
**Sprint plan**: [m-dx-microrag-context-sprint-plan.md](m-dx-microrag-context-sprint-plan.md) — executed same day
**Implementation note (2026-08-31)**: shipped as `.pi/extensions/microrag-context.ts` (suite member #11) with 22 unit tests (`.microrag-context.test.ts`) and live e2e on `ollama/gemma4:e4b` (headless `--mode json`). API deltas vs this doc, verified live: (R1) gate default when `AILANG_MICRORAG_ENABLED` is UNSET = **inert** — diverges from the engine/shim default-on deliberately, so `--microrag auto` arms keep today's baseline and only explicit `=1` injects; (R3) prompt handed to the engine via `--prompt @tmpfile` (4096-char cap, shim parity), not argv/stdin; (R5) session key `AILANG_MICRORAG_SESSION=pi:<id>` set through an `env` argv wrapper because pi's `execCommand` has no env option — without it every engine call derives a fresh pid key and the V9 ledger never dedups; (R9) MEASURED template finding: code-citation queries ("AILANG error PAR015: …") fall below the engine floor while path-stripped message + fix-intent phrasing clears it (hit the Let Bindings chunk where the cited-code form missed) — the V9 ledger was also observed live (`dedup_suppressed`); the D3 Go fallback is therefore NOT triggered for real diagnostic shapes (type/parse messages with fix intent clear the floor), only multi-line module-loading wrappers may floor — revisit with its own A/B if the rig shows that shape mattering. Live e2e arms: known prompt → `std/string.concat` chunk injected once and used in the answer; below-floor → silent `microrag:skip{reason:below_floor}`; IMP010-shaped check failure → buffer restored on /resume, synthetic query injected next turn (`from:error`, score 0.72), served-flag persisted; `microrag_search` tool called by the model and answered from hits (score 0.84); unset-env and `PI_MICRORAG_CONTEXT=0` arms banked ZERO microrag entries (fully inert). Kill switches: `PI_MICRORAG_CONTEXT=0` (all), `PI_MICRORAG_INJECT=0` (M1/M2 only), `PI_MICRORAG_TOOL=0` (tool only), plus the engine's `AILANG_MICRORAG_ENABLED` (arm parity).
