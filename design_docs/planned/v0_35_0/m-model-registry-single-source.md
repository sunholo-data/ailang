...
Any agent whose pin is dropped silently runs a model nobody chose, on an account nobody is watching. That is a silent fallback on a billing path (CLAUDE.md Critical Principle 2), and it is invisible until an invoice or an outage.

> **Corrected 2026-08-27.** This paragraph originally justified the defect by "the fleet was migrated off Anthropic because Claude-CLI OAuth for headless agents is being retired". Verified against Anthropic's help centre the same day: that change was announced for 2026-06-15 and **paused on the day** — `claude -p` still draws on subscription limits, and the separate API-rate credit was never issued. Cloud Run jobs also **do** run Claude on subscription OAuth (`CLAUDE_CODE_OAUTH_TOKEN`, `agent_executor` job template). The argument for removing the defaults never needed that premise and is unaffected; the premise was simply repeated from a commit message without being checked.
