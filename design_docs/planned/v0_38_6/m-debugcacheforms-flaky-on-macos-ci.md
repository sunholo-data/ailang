...
| H1 (teardown) | **MUT-3b**: the bounded wait replaced by a bare receive on the completion channel | `TestCapturePipelineStderr_DrainTimeoutIsLoud` — the never-released wrapper makes the bare wait hang; the test is red via `go test`'s `-timeout` (AC2 passes `-timeout 60s`; the mutant exits `rc=1` with `panic: test timed out after 1m0s` and a goroutine dump — measured by executor, controller and judge; the r2 text said `rc=2`, corrected). Non-hermetic kill, declared as such: the mutant is red in 60 s, not instantly — and under CI's un-scoped invocation a regressed bounded wait would hang the whole `internal/pipeline` binary to `go test`'s default 10-minute timeout (judge finding), which is loud, not silent | Structural; probed green at the fixed form in 50 ms (V20). Note: the test deliberately leaks one goroutine parked on its own gate; `os.Stderr` is restored and `r`/`w` are closed before the panic, so nothing leaks into later tests |
...
| V21 | (measured by the EXECUTOR at 3aa7822a2 and independently by the round-1 judge at 36a5cc8ee) `TestCapturePipelineStderr_CopyErrorIsLoud` green at the fixed form; MUT-4 red; MUT-1 red with the explicit copy-error panic | `go test ./internal/pipeline/ -run 'TestCapturePipelineStderr_' -count=1 -timeout 60s -v`; the AC3 drills | fixed form: four `--- PASS:` lines, rc=0. MUT-4: rc=1, `expected the helper to panic on a non-nil copy error`. MUT-1 ×5: rc=1 each, panic text `capturePipelineStderr: copier failed: read \|0: file already closed` (executor 5/5, controller 5/5, judge 5/5 + 3 solo). Judge-measured red SETS (rule 3j): MUT-1 = {GatedReaderLosesNothing, TestPipelineModulePhases_DebugCacheFormsAndCounters} — the HEAD ordering reddens the production test LOCALLY once the copy error is propagated; MUT-2a = {GatedReaderLosesNothing, DrainTimeoutIsLoud, CopyErrorIsLoud}; MUT-2b, MUT-3a, MUT-4 sole-killers; MUT-3b non-hermetic (go test timeout) |
...
| judge r1 | agent-tool `sonnet` (own worktree at 36a5cc8ee) | PASS 88/100, zero blocking | five non-blocking: V21 left UNMEASURED after M2 produced it; MUT-3b `rc=2` claim; `GatedReaderLosesNothing`'s post-loop `os.Stderr == nil` check cannot fail (proved by skipping the restore step); MUT-1/MUT-2a red sets broader than the single named killer; MUT-3b regression would hang CI's un-scoped run for 10 min | r3 (controller, reproduced each first-party before acting): V21 transcribed; rc corrected; assertion now compares to the ORIGINAL stderr and is proven red against the restore-skip mutant (rc 0→1); red sets recorded in V21 and the audit; hang note recorded above |
