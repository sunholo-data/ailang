...
- **W2 (CLI done ✅ · dashboards ⏳)** — `ailang eval-elo` now tracks per-model AILANG coverage and flags models below 50% of `max_coverage` as `provisional` (new `Cov` column + `--json` fields; commit 7abbb3218). **Remaining (frontend):** 6 dashboard components still rank without coverage gating — `ModelChart` (headline bar chart; the live bogus-#1 bug: `motoko-local-qwen` ELO 2488 on 6 benchmarks), `ValueScoreTable` (medals), `QualityScatter` (Pareto), `DollarsPerPassTable`, `ModelComparisonTable`, `ModelDeltaTrend`. **Approach:** the `.ratings.<mode>.models` block already carries `benchmarks` + `maxCoverage` (W1), so gate frontend-only — a shared helper builds an `id → {benchmarks, provisional}` lookup + `isProvisional` (mirror `EloLeaderboard`), applied uniformly. Verify each with a local Node-20 build + headless-Chrome render before deploy (never SSR-only — see the OSVersionTrend post-mortem). `ComparisonTable`/`BenchmarkChampionsTable`/`OSLocalLeaderboard` are already safe.
