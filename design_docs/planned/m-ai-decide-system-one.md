...
14. **Labelled ground truth for the first consumer exists in-repo.** 45 design docs under `design_docs/{implemented,planned}` carry a `PROGRAM.md Routing` section naming the lane (extension / AILANG / core / mission …) — a free agreement set for the shadow lane-router (M3). Their Problem Statements are the `state`; the declared lane is the label.
...
- [ ] **D5 — first consumer.** Recommendation: the shadow lane-router (M3) with the LLM control arm. Alternatives considered: (b) sub-classifying banked `api_error` rows (no ground truth — would need hand labels first); (c) a `on_solver_candidate` done-gate second opinion in motoko (acts on the loop; Phase 2 material once (a) shows the model is good at AILANG-shaped state). Mark to confirm or redirect.
...
---
*Spike, measurements and this doc: 2026-09-18, attended session.*
