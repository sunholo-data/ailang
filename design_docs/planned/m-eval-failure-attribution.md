...
> **Naming note (2026-08-13):** the candidate ran as `or-deepseek-v4-pro-0813`. That id no longer exists — Mark folded it into `or-deepseek-v4-pro` the same day ("we need only one v4 deepseek pro"), repointing the incumbent entry at the GA slug per the flash-0731 precedent. References to `or-deepseek-v4-pro-0813` below are historical run ids, not current models.yml keys.
...
- **A live example, and it was confirmed by escalation.** In a 2026-08-13 core head-to-head, `or-deepseek-v4-pro-0813` scored 20/23 vs the incumbent's 21/23 and read as "the candidate is worse." Decomposed: one of its two losses was the `Fractional::Int` interpreter crash, the other correct values with labels, while the incumbent's single loss was a genuine incomplete solve. **Re-run at N=3 the ordering flipped to 67/69 vs 65/69 — the candidate wins**, exactly as the decomposition predicted and the raw N=1 score denied. The model-add gate would have rejected a better, cheaper model on a number that was measuring our interpreter. This is the cost of unattributed failures stated in one case.
...
2. **Signature table** (new, data not control flow) — ordered `(pattern, attribution, reason)` rows matched against `stderr` + row shape. Seeded from the audit:
   ...
   | *(NOT a signature)* `IMP010: symbol … not exported` | **model** | hallucinated stdlib API — V10/V11a. Listed here only to record that it was considered and rejected |
...
| V11 | ~~`dense_operator_program` is unsolvable — AILANG has no bitwise support~~ | ORIGINAL (wrong instrument): `grep -ciE 'bitwise\|shift' std/math.ail` = **0**. CORRECTED with `ailang check --relax-modules` on real programs | `12 & 10`, `12 ^ 10`, `1 << 4`, `16 >> 2`, `~12` → **✓ No errors found**. Only `12 \| 10` fails (`PAR_UNEXPECTED_TOKEN`). De Morgan `~((~a) & (~b))` → **✓ compiles** | **FALSE — REFUTED 2026-08-13.** AILANG HAS `&`, `^`, `~`, `<<`, `>>`; it lacks only bitwise **OR** (`\|`), and OR is expressible via De Morgan. The benchmark is SOLVABLE. Grepping stdlib for functions cannot see operators — wrong instrument for the claim, and the repo's own rule (verify every language claim with `ailang check`) exists for exactly this |
| V11a | Then why does every model fail `dense_operator_program`? | the IMP010 signatures from V10 | models import a non-existent `bitwiseOr` from `std/math` rather than composing OR from the operators that exist | **model failure with a language-ergonomics contributor** — NOT `benchmark` attribution. The missing `\|` operator is a genuine AILANG gap worth routing on its own |
