...
| V34 | the early smoke keeps invalid treatment out of measured fmt rows without reimplementing treatment logic, **on all three of its refusal branches** | `/bin/bash tools/launchd/test_fmt_ab_schedule.sh` | `fmt_hook_state=off` → 0 five-trial calls + the named contract; `fmt_hook_state=on` with `validity.valid=false` → 0 + the named contract (this is §5.2 requirement (a), *firing is not delivering*); zero banked rows → 0 + the named contract; a valid ON row with a `status=formatted` event → all 12. **Each branch pinned by its own neutering mutation** (`if false && …`, mutant LANDED by grep and VALID by `bash -n` before the result was read): as first delivered, the `validity` and row-count branches survived neutering with the whole suite **green** |
...
| V36 | the probe's sampling layer is SOUND on this rig — scoped `lsof` sees an ESTABLISHED peer of a child process, so an empty peer set is a fact about the RUN, not about the instrument | hold a live stream open (`curl -sN … 127.0.0.1:11434/api/generate`), then run the probe's own command shape `lsof -nP -iTCP -sTCP:ESTABLISHED -a -p <pid>`, paired with the unscoped form as a same-call control | scoped → **2 lines**, `curl … TCP 127.0.0.1:49914->127.0.0.1:11434 (ESTABLISHED)`; unscoped control → **67 lines** and the same socket from ollama's side. An earlier reading of "0 lines" was taken after the holder had already exited — a stale-artifact error in my own control, corrected by re-taking it against a holder asserted ALIVE (`kill -0`) and streaming (95,869 B) |
| V37 | the treatment lane WORKS and the required observable IS reachable under the probe's own `run_lane` shape — so AC-M2-treatment's positive half is not unsatisfiable in principle | (a) standalone `ailang eval-suite --agent --models motoko-local-qwen3-6-fmt --benchmarks numeric_modulo --trials 1 --dry-run=false`; (b) the identical launch wrapped in a faithful replication of `run_lane` (backgrounded + per-second `descendant_pids`/`lsof` tree sampling), both under `rig.lock` | (a) **rc=0**, 2/2 success, 1m53s, free-local; (b) **rc=0**, 2/2, 1m1s, **244** lsof lines, and the unique peer set CONTAINS **`127.0.0.1:11434`** (also `:8080`, `:1957`, ephemerals). Both arms on darwin/arm64 |
| V38 | **the probe AS SHIPPED breaks the very runs it observes — AC-M2-treatment is therefore VOID, not failed, and the instrument refusing to certify is the doc's §12.4 rule working** | `tools/eval/motoko_connection_probe.sh motoko-local-qwen3-6-fmt motoko-claude-haiku-4-5 <artifact> numeric_modulo` under `rig.lock`, `AILANG_BIN` an explicitly built absolute path | **both** lanes `driver_rc=1` with peer set `[]`; treatment 10:41:01→10:49:16Z, control 10:49:16→10:57:33Z — **8m15s / 8m17s**, against 1m1s–1m53s for the same lane in V37. `OR_IPS` resolved and recorded (`104.18.2.115`, `104.18.3.115`). The probe wrote its artifact BEFORE evaluating the verdict, then exited 1 on `INSTRUMENT FAILURE: empty peer set; absence of evidence cannot prove routing`. Per AC-M2-control, a control that does not fire makes AC-M2-treatment **VOID** — the probe proved nothing, and said so rather than reporting a green. **Mechanism NOT isolated** (V37 shows the launch shape and the sampler are both fine); the difference lies in what V37 did not replicate — the deadline-carrying `descendant_pids`, the `classify_lsof` pipeline, or the two-lanes-in-one-process sequencing. The probe additionally DISCARDS both driver logs via its `trap … EXIT`, which is why the first diagnosis needed a re-run: for a lane that exits non-zero, that log is the whole diagnostic |
| V39 | **the probe as first delivered could not see an OpenRouter leak over IPv6 — a FALSE NEGATIVE in exactly the half of AC-M2-treatment that must not fail** (found by the sprint evaluator, reproduced first-party before acting, then fixed and gated) | `dig +short openrouter.ai` vs `dig +short AAAA openrouter.ai`; then `--classify-fixture` over a seeded IPv6 OpenRouter peer, with an IPv4 positive control and a non-OR IPv6 negative control in the same call | `dig +short` returns **IPv4 only** (`104.18.2.115`, `104.18.3.115`) while openrouter.ai has **real AAAA records** (`2606:4700::6812:373`, `:273`), so `OR_IPS` could never contain a v6 address; and `lsof` brackets an IPv6 peer (`[2606:…]`) where `dig` emits it bare, so even a seeded entry failed `grep -Fqx`. Measured: the v6 OpenRouter peer classified **`other`** while the IPv4 control in the same call classified **`openrouter`**. **Fixed** by unioning A+AAAA (with `+time=5 +tries=2`, which also bounds the one wait that lacked an explicit bound) and comparing on the bracket-stripped host. Re-measured: v6 → `openrouter`, IPv4 control → `openrouter`, non-OR v6 → **`other`** (no over-match), loopback → `loopback`. **Gated, not merely fixed:** a new arm asserts a v6 leak is refused, and reverting the normalisation reds it (`missing openrouter [2001:db8::8]:443`) — mutant LANDED (sha256 differs), VALID (`bash -n` rc=0), restore byte-identical, suite 8/8 green again |
