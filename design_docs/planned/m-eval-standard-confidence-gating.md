...
- [x] Confidence-gating default: **opt-in** — new `--confidence-gate` flag on `run_eval_baseline.sh`, off by default. First release run gated is a deliberate choice, not the new normal, until Phase 4's live comparison is reviewed.
