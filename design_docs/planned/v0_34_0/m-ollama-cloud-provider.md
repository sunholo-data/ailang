...
| **D4**: How serialization should treat cloud rows — **NOT a blanket exemption (V39)**: GPU contention is local-only, but the motoko port-8080 backend binds for cloud rows too | The clamp exists for GPU thrash, which does not apply to cloud rows — but it keys off `SupportsAgentEval && !SupportsStandardEval`, not on the route ([eval_suite.go:271](../../../cmd/ailang/eval_suite.go#L271)), so a cloud row would be wrongly serialized. Exempting it trades rotation wall-clock against the plan's concurrency cap. | agent | compile | med |
...
| V39 | **NEG — D4 CORRECTION: "exempt cloud rows from serialization" would be WRONG.** The rig lock protects more than the GPU | attempted a motoko agent run on cloud rows while `os-rotation-filler.sh` (PID 24713) held the rig lock | Refused: *"the rig is busy — PID 24713 ... holds the rig lock"*. The filler defaults to `opencode-qwen3-8-27b,pi-qwen3-8-27b,**motoko-local-qwen3-8-27b**`, and motoko binds **fixed port 8080** — the documented zombie that kills the rig. So a cloud row driven *via motoko* still needs mutual exclusion, for the backend port, not the GPU. **Correct distinction for D4: GPU contention ⇒ local rows only; motoko backend port ⇒ ALL motoko rows, cloud included.** Standard-mode cloud runs need neither and ran fine alongside the lock |
