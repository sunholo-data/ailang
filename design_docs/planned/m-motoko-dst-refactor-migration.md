...
| G4 | Compile probe: the pilot port compiles against the *published* ABI | port `motoko_ext_test_dummy`'s trivial hooks to the `Outcome` shape in a scratch worktree, repin its `motoko_ext_abi` dep to the registry 5.x, `ailang check` exits 0 | unrunnable while G3 is FALSE → **FALSE**. **BUT ITS SUBSTANCE WAS MEASURED 2026-08-23 VIA A PATH DEP — IT PASSES (V30–V32).** G4 stays FALSE as written, because "the *published* ABI" is its literal wording and nothing is published |
...
| V30 | **THE ABI 5.0 "BREAK" IS ADDITIVE AT THE TYPE LEVEL — ZERO TYPES ARE REMOVED, AND THE PREMISE "2.2.0→5.0 BREAKS ALL 12 EXTENSIONS" IS MUCH SOFTER THAN THIS DOC AND THE CHARTER RECORD** | pinned `main_dst@80611514ee8acbf5c4d5bdf46112051bf369e4ee`; fetch `packages/motoko-ext-abi/types.ail` at that SHA; extract `^(export )?type X` from it and from our published 2.2.0 (`ailang-packages/packages/motoko-ext-abi/types.ail`); `comm` the two sorted sets | 2.2.0 = **20** types, 5.0 = **36**. `comm -23` (removed) = **0**. `comm -13` (added) = **16** (`AiStepOutcome`, `DiagnosticField`, `ExtClockReading`, `ExtDirListing`, `ExtFileMutation`, `ExtFileRead`, `ExtPathKind`, `ExtPathStat`, `ExtPorts`, `ExtProcOutcome`, `ExtWorld`, `FinalizeOutcome`, `PreStepOutcome`, `ResponseInterceptOutcome`, `TokenTelemetry`, `ToolHandleOutcome`). All 20 original types survive **by name**, and the four new `*Outcome` types are two-field wrappers over the surviving `*Decision` types: `{ decision: XDecision, next_state: ExtWorld }`. `ExtCtx` gains `world: ExtWorld` to supply `next_state`. **Shape change is real but confined**: of `ExtensionHooks`' ten slots, four change return type (`on_pre_step`, `on_tool_handle`, `on_response_intercept`, `on_solver_candidate`) and five change effect row (four narrowed; `on_tool_handle` **widened** by `Rand`). **File size is NOT a proxy for surface change**: 155 → 971 lines, but ~800 of those are upstream's internal work-item log (`WI-D5`…`WI-D14`) carried inline in the ABI source |
| V31 | **G4's SUBSTANCE PASSES — the pilot ports in 25 changed lines and type-checks clean against 5.0, including every effect row** | scratch `/tmp/g4probe` from `packages/motoko-ext-test-dummy`; mechanical port (4 return types → `*Outcome`, bodies wrapped as `{ decision: <existing>, next_state: ctx.world }`, `_ctx`→`ctx` on those four, rows per 5.0, `on_budget_plan`'s `! {Env, FS}` dropped); `ailang.toml` dep repinned to `{ path = "/tmp/abi50pkg" }`; `ailang lock`; `ailang check register.ail` | Baseline first: pilot **green** against 2.2.0 (`✓ No errors found!`). `ailang lock` → `sunholo/motoko_ext_abi@5.0 (path: /tmp/abi50pkg)`, so resolution is asserted and not assumed. Ported → **`✓ No errors found!`**. Port diff = **25 changed lines**. **The shared registry cache was NOT touched** — `~/.ailang/cache/registry` is fleet-shared and a path dep avoids it entirely |
| V32 | **AND THE PROBE CAN GO RED, so V31 is a measurement and not a vacuous green** | revert `on_pre_step` alone to its 2.2.0 shape, re-run `ailang check`; then restore and re-run | Reverted arm **FAILS** with the compiler naming the exact row: *"failed to unify record field `on_pre_step`: failed to unify effect rows: incompatible closed rows: r1 has extra labels [Clock Env FS Net Process SharedMem Stream], r2 has extra labels [Trace]"*. Restored arm **green** again, restore proven byte-identical by sha256. **CAVEAT, AND IT BOUNDS THE CLAIM**: `test_dummy` is the *trivial* pilot — every hook returns a constant. 5.0's own inline commentary names `compaction_ai`, `compose` and `context_mode` as the three bindings with non-empty effect rows, so 25 lines is the FLOOR of the port cost, not its average |
| V33 | **G3 IS A PREDICATE ON OUR OWN REGISTRY, NOT ON ARNI — but publishing is refused on evidence, and `ailang publish` cannot soften it** | `curl` the registry index for `.index.repository`; `ailang publish --help`; `git -C ailang-packages` read `packages/motoko-ext-abi/ailang.toml`; count `main_dst` commits/day via the commits API | Registry names the canonical source as **`sunholo-data/ailang-packages`** — **ours**, not `motoko_agent`; our copy is at **2.2.0** while `main_dst` carries a modified copy at **5.0**. So G3 was written as an external observation and is actually a publish step we own. **THREE REASONS IT WAS STILL NOT DONE**: (a) `ailang publish` has exactly two flags (`--dry-run`, `--allow-dotted-tool-names`) and every registry version is a plain `X.Y.Z` — **there is no prerelease**, so a "5.0-rc at a pinned digest" is not expressible and any publish is a permanent public `5.0`; (b) the source moved **30 commits on 2026-08-23 alone** (21 on 08-22, 30 on 08-17), so a digest minted today is minted from a moving branch and G3's own digest rule would flip it back to FALSE; (c) it would import ~800 lines of another project's internal work-item narrative into our public package. **Held for Mark; the information the publish was meant to produce was obtained without it (V30–V32)** |
...
Rows **V30–V33** were measured first-party in an attended session on **2026-08-23**, against
`main_dst@80611514ee8acbf5c4d5bdf46112051bf369e4ee`. They do not open the gate — G1, G2 and G3 are
all still FALSE — but they retire the gate's *expensive* unknown: the port cost is now measured
rather than feared, and the ABI break is mechanical rather than a redesign.
