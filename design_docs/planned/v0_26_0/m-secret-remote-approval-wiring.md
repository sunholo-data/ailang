...
- `terraform/cloud_run_ntfy.tf` (new) — `${var.prefix}-ntfy` service: ntfy image, scale-to-zero (`ntfy_min_instances`, default 0; cold start is fine on the push-driven path — bump to 1 for guaranteed iOS fetch-back), public invoker, `NTFY_*` env + `upstream-base-url`, auth token from Secret Manager. Gated by **`var.enable_secret_approvals`**.
...
- **Audit hardening (post-deploy, end-to-end validated live) — DONE**: surfaced while testing the real ntfy flow. (a) **Terminal-state lock** — a resolved secret approval can no longer be re-flipped (the Approve/Deny buttons carry different single-use tokens, so a second tap of the *other* button previously corrupted the record); `resolveSecretApproval` now returns 409 for a non-pending request. (b) **History list fix** — `GET /api/approvals?status=all` failed `FailedPrecondition` (missing Firestore composite index `approvals(status, resolved_at DESC)`); index added in `ailang-multivac/terraform/firestore.tf`. (c) **Confirmation push** — ntfy action buttons can't reflect their own outcome, so resolution now sends a value-free follow-up notification (`BuildSecretApprovalResolvedNotification` → "✅ Approved" / "❌ Denied") via the same publish→bridge→ntfy path. Rationale: ntfy is fire-and-forget by design (the durable, tamper-resistant record lives in the backend, not the notification); these make that record trustworthy + viewable while staying self-hosted.
