...
**Picked.** `m-launchd-notify-subshell-observation`, on two independent grounds that agree. It is
the standing `launchd drivers (bash 3.2)` red on `dev` and V1 owns this repo; and human ruling
**D-60** (attended 2026-09-07) answered it **option B** with *"prefer this reliability repair early
in the week"*, which outranks the queue. Iteration 346 named it as the next pick and left the queue
row tagged `[PARKED — needs-human-review, D-60]`, so this iteration also discharges the D-12
obligation that a ruling un-parks its row in the iteration that consumes it. Ledger: 60 rows, ZERO
open, `mission_decisions.sh --check` valid — so there were no DECISIONS to carry and no directive on
`#1072` since the watermark.

**Reality check.** Reproduced first-party at the exact base `81abc956d` in a fresh worktree, not
transcribed: `bash tools/launchd/test_driver_notify.sh` → **20 passed, 7 failed**, rc=1, and the
seven names match the doc's causal-mapping table exactly. Three further premises measured before any
routing, each with its own control:
- `_mc_bounded` runs `( exec "$@" )`, and **`exec` bypasses shell functions**: `f() { echo
  FUNCTION-CALLED; }; ( exec f )` → `exec: f: not found` (rc=127), while calling `f` directly prints
  `FUNCTION-CALLED`. So the production half of D-60 and the fixture's function stubs are in direct
  collision, which nothing in the inherited design said.
- `_mc_bounded 30 env VAR=v <PATH-stub> …` → rc=0 with the env reaching the child and
  `MC_BOUNDED_OUT` populated; a never-returning stub at bound 2 → rc=124 in **5 s**, not the ~3 s
  the designer first wrote.
- The unbounded-call inventory: three sites inside `_mc_notify`/`_mc_drain_notices` (the ones D-60
  names) and **seven more** elsewhere in the driver, with the two already-bounded slot-verdict sites
  at `:1817`/`:1820` as the positive control that the grep can see a bounded call.

**Shipped.** PR [#1090](https://github.com/sunholo-data/ailang/pull/1090), two commits.
**M1** [`706dc824c`](https://github.com/sunholo-data/ailang/commit/706dc824c) is test-only: the
fixture's variable spy becomes a file-backed one on a **unified** medium — `ailang`/`gh` become
PATH-executable stubs (required by M2, above), `log()` appends to the same `MC_TRACE_FILE`, and the
teardown loads the file into `TRACE` before the assertions run; the block's real status is captured
instead of `printf`'s. 20/7 → **30 passed, 0 failed**, rc=0.
**M2** [`58af47580`](https://github.com/sunholo-data/ailang/commit/58af47580) is the D-60 production
change: `NOTIFY_TIMEOUT` (env `MISSION_NOTIFY_TIMEOUT`, default 30 s — the value the two already-
bounded sites use) and the three named paths each through `_mc_bounded`, with a leading `env`
because `exec` cannot parse a `VAR=val` prefix. 30/0 → **37 passed, 0 failed**, rc=0.
**M3 fixes** [`cac360d5d`](https://github.com/sunholo-data/ailang/commit/cac360d5d) close the judge's
two blocking findings, both reproduced first-party first (below). Suite ends at **38 passed, 0
failed**.

**The finding this iteration did not set out to make: clearing one red exposed nine older ones.**
`make test-launchd-drivers` runs its suites in order and halts at the first failure, so for as long
as the notify suite was red **nothing behind it ran**. With it green the target reaches three more
suites — two of which pass (**84/0** and **17/0**, in CI, having never been executed on `dev` before)
and one of which, `test_mission_heartbeat.sh`, reports **`FAIL: 9 heartbeat arms failed`**. Attribution
is a paired control, not an inference: run alone at base `1825fd8c7` and at head `58af47580` the suite
gives the **identical nine named arms**, and CI on the merge-base showed the same. So this PR does not
regress the job — it makes a second, older red visible, and `launchd drivers (bash 3.2)` stays red for
a different reason than it did this morning. Filed as its own queue row at the top.

**Not delivered.** M3 (aggregate drain budget `DRAIN_BUDGET`) and M4 (mutation sweep) were **not
executed**: the executor hit its 30-minute cap mid-M3 (`mission_pi_run.sh` verdict `wall_timeout`,
rc=13, 84 tool executions). The partial M3 failed its own two new tests (`37 passed, 2 failed`) and
was discarded from the tree rather than committed; it is preserved outside the ephemeral worktree at
`~/.ailang/state/mission-v1-iter347-m3-partial/` and queued. The plan's own Budget section named
M1+M2 as the cut set, so this is the prescribed degradation, not an improvised one. Reconstruction of
both milestone commits was done from the executor's `.snap/M<k>/` snapshots and verified by
`cmp`/sha256, with the suite re-run at each boundary (M1 30/0, M2 37/0).

**Routing evidence.** Gate 4 base=`8e392795052cdf829e8b2e9b0890e0974911af66@2026-09-07T14:33:01Z`.
Controller `claude:claude-opus-5` (tok: not reported).
**Designer:** resolver said `recipe claude:claude-fable-5-1 declared:provider-pin`, which is the
rotation SEED, not a pin — the pointer `~/.ailang/state/mission-v1-designer-rotation` read
`codex:gpt-6-astra`, so the next entry is `pi:ollama/deepseek-v4-flash:0731-cloud`, and that is what
ran. Three runs on that lane, all verdict `ok`, all flat-rate $0: authoring 334 s (988,734 in /
26,995 out), revision r2 172 s (520,450 / 18,679), refinement r3 294 s (1,205,932 / 27,424).
Deepseek is vendor-independent of all three quorum reviewers, so no self-marking collision.
**Quorum:** round 1 **BLOCKED** — `gemini-3-1-pro` reject, `oc-glm-5-2` reject, `gpt6-astra`
**ABSENT on budget**. Round 2 at a raised cap (`--max-cost-usd 0.25`) — `gemini-3-1-pro` **PASS**
(flipped), `gpt6-astra` reject, `oc-glm-5-2` absent for **`invalid`**, i.e. its own response failed
to parse; re-run alone it rejected too, so the round is honestly 1 pass / 2 reject with no hole.
Round 3 applied the **narrow-refinement carve-out** — both surviving objections carried concrete
reviewer-authored `proposed_fix` text and neither disputed the design DIRECTION — so the fixes were
applied verbatim and the doc routed straight to the planner with no fourth round.
**Planner:** resolver `agent-tool opus fail-closed:planner-lane-field-missing`; the spawn-pin hook
would deny that alias for a `provider:model`-pinned role, so per the amendment the pin was used
directly. `pi:ollama/kimi-k3:cloud` probed **rc=143** (SIGTERM at the 120 s bound — timeout, not a
refusal), so the chain advanced one link to `pi:openrouter/moonshotai/kimi-k3`, probe rc=0. Verdict
`ok`, 346 s, 38,463 in / 23,126 out, **metered $0.6203**.
**Executor:** `recipe pi:ollama/deepseek-v4-flash:0731-cloud declared:provider-pin`, verdict
`wall_timeout` rc=13 after 1808 s, 84 tool executions, 8,225,795 in / 123,513 out, flat-rate $0.
**Evaluator:** `agent-tool sonnet declared:alias-pin`, spawned through the Agent tool, 215,385 tok /
121 tool uses / 2937 s, in its own dedicated worktree. Generator (pi deepseek) != judge (sonnet).

**Ruled out.** *"The designer's line numbers for the drain call site and `HARD_TIMEOUT` are wrong"* —
**REFUTED, and the error was mine.** I read `:977`/`:800`/`:807-808` and patched the doc's
`:972`/`:795`/`:802-803` to match; the designer was right. My grep had run in the **pin worktree**
(`~/.ailang-driver-pin/v1-codex-quota-8e5899aff`), which carries local-only driver commits
(`92ab70921`, `8c56c5863`) that shift `tools/launchd/mission-control.sh` by five lines. Reverted in
the same breath. The generalisable form is this loop's own: *the tree you measure in is part of the
instrument*, and the pin worktree is not `origin/dev` even when its name says it is. Note M2's line
numbers (`:154`/`:184`/`:198`/`:496`/`:1817`) are identical in both trees, which is exactly why the
error was invisible until a number happened to fall past the divergence.
*"The 9 heartbeat failures are a regression from this diff"* — refuted by the paired base/head
control described above; the judge independently re-ran it and initially got a contradicting
15-vs-10 reading, then traced that to its own incomplete test isolation and reconfirmed the identity.
*"`_mc_bounded` fixes the lost-`TRACE` observation on its own"* — refuted: it too forks a background
subshell, so the file-backed spy is still required; the two halves compose, neither substitutes.
*"A `checkno` that passed before and passes after is fine"* — refuted by the judge's combined (g+h)
mutation: the two negative assertions survive for the right reason only because the positive-control
tripwire exists.

**Retro lane.** The Fable-diet clause and Gate 2's quorum protocol collided again, in a new way: the
diet's unit is *"one design DOC — the initial authoring run plus at most one protocol-mandated
revision run"*, and the narrow-refinement carve-out authorises a **second** revision, so a doc that
takes the carve-out is a diet overspend **by construction**. Flagged rather than hidden. It cost
nothing here — the designer lane was flat-rate pi, $0 metered, not the Fable bucket the diet exists
to protect — which is precisely why it is worth recording now rather than the first time it lands on
Fable. This is evidence row 1 for widening the diet's unit to "one doc, including any carve-out
revision"; it is a routing-policy clause, so it needs a human, not a controller edit.

**Progress.** N=12 design docs before v1.0.0 (was 12, change 0); this is a HARNESS item outside the
ratified clauses 2–5 inventory, so the goal is unmoved.

**Cost.** Metered **$0.80**: quorum round 1 $0.0359, round 2 $0.1293, the `oc-glm-5-2` solo re-run
$0.0117, the OpenRouter kimi probe $0.0070, and the planner $0.6203. Under the $5 ceiling. Designer
(3 runs) and executor rode flat-rate ollama-cloud at $0; controller and evaluator are Anthropic
subscription. No unreported cost was invented.

**Next.** `m-launchd-heartbeat-arms-red-behind-the-notify-red` (a red on `dev`, and now the only
thing keeping `launchd drivers (bash 3.2)` from green), then
`m-launchd-drain-aggregate-budget` (this sprint's unexecuted M3/M4, with the partial banked), then
`m-debugcacheforms-flaky-on-macos-ci`. Ledger 60 rows, **ZERO open** — no decision is waiting on
Mark from this iteration.

**Independent evaluation.** Judge `sonnet`, own worktree, **LAND 80/100**, no hard fail, **two
BLOCKING findings** — and both were real. It re-derived C1–C4 first-party, ran **11** mutations
(each restored by `cp` from a pre-captured backup and proven byte-identical by sha256, never by a
git write), and found 8 of 11 correctly non-vacuous, 1 non-vacuous but mis-attributed in the design's
own Mutation Table, and 2 defects.
**B1: `run()`'s own `_blk_rc` capture had ZERO coverage.** The dedicated G2 assertion exercises a
standalone reimplementation that never had the printf-masking bug, and every other check against
`run()`'s RC only looks for `RC:0` — so moving the capture back after the printfs left the suite
**fully green at 37/0**, in the harness driving 20+ of the 37 assertions. I reproduced it exactly.
**B2: the store/project guard was not hermetic.** `_mc_bounded` runs `( exec "$@" )` and a subshell
inherits the caller's exports regardless of the `env` prefix; CLAUDE.md instructs every machine doing
AILANG work to export `AILANG_MESSAGES_STORE`/`_PROJECT` at session start. I reproduced both
directions: with the `env` prefix deleted from production the suite was **green 37/0 with the vars
ambient** and **red 36/1 without** — the guard defeated by the documented operating environment.
Both fixed in `cac360d5d` and each proven non-vacuous by re-running its mutation: B1's now kills
exactly `RC capture: run() itself surfaces a failing block, not printf's status`, and B2's now fails
**with the vars deliberately exported**. Three non-blocking findings also actioned: the `rc=124`-only
synthesis gate (now any non-zero rc with an empty tail, so a `mktemp` rc=125 is covered — measured
`no output (rc=1)` where the tail was blank), `local _rc`, and the Mutation-Table victim correction.
...
## 351 — 2026-09-08 — Inherit iteration 350's orphaned design doc, land it — and let the judge's round-1 FAIL catch a production regression the plan had flagged and shipped anyway [HARNESS]

**Picked.** `m-coordinator-test-parallelism` — **inherited, not chosen**. Iteration 350 sent a CLAIM
message at `2026-09-08T01:14:40Z` and then died mid-flight, leaving **zero** charter rows, **zero**
log entries, no PR and no remote branch: the died-mid-flight shape Gate 2 names, for the second
consecutive slot (349 attempt 1 died at Gate 3b holding a green PR). Gate 2 trace (b) found it —
`git worktree list` in **this clone** carries `.wt-v1-iter350` on
`sprint/v1-iter350-coordinator-test-parallelism` at `5935053b2`, which is ownership proof `--author`
cannot give on a repo where V1 and motoko share the push identity. The physically top `[NEXT]` row,
`m-fleet-sha-pin-freezes-every-driver-fix`, was **not** picked: it is blocked on **D-61**, the
ledger's one open row, and the loop may not repoint the fleet's env pins on its own recommendation.

**What iteration 350 left, and why it was routable without a designer.** A 416-line **revision-3**
design doc, committed on the branch. Its quorum artifacts sit in that worktree's own
`.ailang/state/mission-quorum/`: round 1 `blocked` with two absentees (`gpt6-astra` on budget,
`oc-glm-5-2` invalid), round 2 `blocked` with **3/3 REJECT and `absent_reviewers` EMPTY** — a full
panel, no hole to re-run. I read all three round-2 objections before trusting the r3 label. None
disputes the design DIRECTION (all three accept "inject the timers, do not parallelise"); each
carries a concrete reviewer-authored `proposed_fix`; and the doc's own quorum log shows the three
applied verbatim — astra's and gemini's converging FIX 1 (replace elapsed-time guards with a
test-controlled wait/ticker seam, and give the async path cancellation) and GLM's FIX 2 (remove the
`if baseDelay == 0 { baseDelay = time.Second }` silent fallback, rebase the production caller on
`DefaultExecuteOptions()`). That is the narrow-refinement carve-out spent legitimately, so no fourth
round and **no designer run** was owed.

**Reality check — verify, do not adopt.** Re-measured first-party at `f3783c976` before routing:
`t.Parallel()` in `internal/coordinator` = **0** against a repo-wide control of **17** under
`internal/`; `baseDelay := time.Second` at `task_executor.go:158`; `pollInterval: 2 * time.Second` at
`approval_checkpoint.go:338`; `>= time.Second` at `event_handler.go:282`; `IsThrottled()` present at
`:306`; `NewStoreBackedApprovalCheckpoint` **0** production callers (the doc's V16 is right — my
first grep read 1 and the hit was the definition line). The headline: the four timer-bound tests sum
to **8.13 s** of a **12.65 s** summed package total over **738** top-level pass events. All four
gates green on the pristine base before any directive was written, which is what makes a later red
attributable.

**Shipped.** PR [#1111](https://github.com/sunholo-data/ailang/pull/1111) → squash [`5f95a3814`](https://github.com/sunholo-data/ailang/commit/5f95a3814399527c081b0b6b2109f3bc614fc0a7), **21 checks on the PR head, ZERO not-green** — three timer seams as
per-call config: `ExecuteOptions.RetryBaseDelay` plus a cancellable `Wait func(ctx, d) error`;
`pollInterval` + `tick` constructor parameters on the store-backed approval checkpoint; a `WithClock`
functional option on `CoordinatorEventHandler`. **No production default changes** (1 s backoff, 2 s
poll, 1 s window, 10 events/s) and each is set **explicitly** rather than by a zero-value fallback.
Measured by the controller **outside** the executor sandbox, `-count=1`, same box: the four tests
**8.13 s → 0.23 s**, summed package time **12.65 s → 4.65 s**, full-package wall **16 s → 6 s**. No
`t.Parallel()` anywhere — the row's title is a misnomer the design corrected, and the win is not
sleeping rather than concurrency.

**Independent judge, round 1: FAIL — and it was right.** `agent-tool sonnet`, in its own worktree,
handed neither my candidate findings nor my method. One BLOCKING finding, which I reproduced
first-party from the diff *before* acting on it: `ExecuteWithRetry` had replaced

    select { case <-time.After(delay): case <-ctx.Done(): return nil, ctx.Err() }

with an uncancellable `opts.Wait(delay)` (production default `time.Sleep`) followed by a
**non-blocking** ctx check. A context cancelled 20 ms into a 1 s backoff was then noticed only after
the full second. The judge later measured all three states end to end: pre-sprint **22.1 ms**,
round 1 **1.001 s**, round 2 **22.077 ms** — restored exactly. This contradicted the design doc's own
**Non-Goal 4** ("no change to production timing semantics"), and the sprint plan had flagged the
tension as *"noted, not decided"* and then shipped it decided-by-default with zero coverage. That is
the whole argument for generator≠judge in one finding: three lanes and a full quorum had passed over
it, and the only thing that caught it was a judge with no stake in the diff.

**The judge's surviving mutant, and why a grep could never have caught it.** Doubling
`DefaultExecuteOptions().RetryBaseDelay` passed the **entire** suite, because the injection guards
assert that an *injected* value is honoured — which stays true when the *default* moves. The design
doc's own grep backstop could not catch it either: `RetryBaseDelay:.*time.Second` **matches**
`RetryBaseDelay: 2 * time.Second`. Measured three ways: loose pattern **1** on the mutant, precise
pattern **0** on the mutant, precise pattern **1** on the clean tree. The executor had reported this
honestly as its own deviation D1 without fixing it; the judge found it independently as a survivor.

**Round 2: PASS 97/100, zero blocking — and its finding was about MY code.** `TestExecuteWithRetry_BackoffIsCancellable`
and `TestProductionTimerDefaultsPreserved` were controller-authored, so I told the judge so and asked
it to be harder on them than on the executor's work. It was, and it earned it: the defaults test
claimed in its docstring to pin "the three production defaults" and pinned one and a half. The judge
proved the poll sub-check **vacuous** by swapping *both* the constructor argument and the expectation
to `999*time.Hour` — still green — because `NewStoreBackedApprovalCheckpoint` has zero production
callers, so there is no code-level default to read and the assertion only proved that Go threads a
constructor argument into a struct field. **I narrowed the claim rather than widening the code**: the
vacuous assertion is deleted, the docstring now states exactly what is and is not pinned, and the two
remaining sub-assertions were re-proven RED against compiling mutants (`RetryBaseDelay` → 2 s;
`maxEventsPerSec` 10 → 20) with each file restored byte-identical by sha afterwards.

**Ruled out.**
- *"mission-motoko's warning that Gate 4's STATUS rotation will red CI on V1's next fire — the fleet
  convention `ITERATION <n> COMPLETE:` has no colon, so every stamp is non-canonical the moment it
  rotates"* — **REFUTED for V1**, and then re-refuted the strong way. All three of V1's live stamps
  already carry the canonical colon, `TestMissionDocHeadingsStayCanonical` was rc=0 at exactly the
  ratchet (`knownNonCanonical = 12`) before the rotation, and rc=0 again **after** this iteration's
  rotation moved ITERATION 347 across the lint boundary. Replied to motoko with the measurement and
  with the warning it will need: because the test passes at *exactly* 12, the ratchet's DOWNWARD arm
  is armed for whoever normalises next.
- *"the plan's `NewStoreBackedApprovalCheckpoint` has one production caller"* — **REFUTED, my error**:
  my grep counted the definition line. The doc's V16 (zero callers) is correct.

**Round 3 — PASS, narrow, on the two commits that landed AFTER round 2's PASS.** Those two were mine and had had no review at all, so I resumed the judge on `git diff 40227f958..519aad5d3` alone and asked it to assume I had overclaimed again. No blocking findings; it re-ran the gates plus the `unused` linter and found no divergence from my local green; and on the load question it did better than I asked, re-running the new liveness assertion six times while the box's real load average was **21-23** (unrelated GPU work), firing at ~0.01 s each time, 100x inside its bound. Two non-blocking findings, both real and both kept: `TestDefaultPollTickFires` does **not** pin the `interval` argument (a mutant hardcoding 50 ms survives the 500 ms bound), and the poll path's two production defaults have never been exercised TOGETHER through the real constructor. Filed as `m-approval-poll-production-defaults-unexercised` rather than fixed — a third un-reviewed test on a just-passed path is the same trade I declined twice already this iteration.

**And then SonarCloud found what none of the four gates could, with its negative control firing.** The PR went red on `new_coverage` **78.8%** against a threshold of 80 — and unlike the standing `dev` Sonar red this mission already tracks (`m-sonar-dev-branch-security-rating-c-on-new-code`), this one was **mine**: SonarCloud reads `success` on the last **five** `dev` commits and `failure` only on my PR. **7 of 33** new lines uncovered, and the split is the whole finding: **1** in `provider.go` — `defaultWait`'s ELAPSED arm, which nothing exercised because every test drives the cancelled arm (the round-2 guard) or bypasses it with a recorder, so a wait returning an error unconditionally would have satisfied the cancellation guard and broken every real retry — and **6 of 6** in `daemon_tasks_exec_run.go`, which is the *only* production caller of the retry path and has **no unit test at all**. The first is closed here by `TestDefaultWaitBothBranches`, two compiling mutants RED (`return context.DeadlineExceeded`; and dropping the ctx arm, which failed by taking 10.4 s). The second is queued as `m-daemon-task-exec-run-untested`, and it carries an admission worth stating plainly: the sprint's **FIX 2** — the production-caller rebase that quorum reviewer `oc-glm-5-2` demanded verbatim — is verified only by code reading and by the judge's contract analysis. **No test executes it.**

**A correction against myself, recorded because a surviving mutant is a claim about the MUTANT first.** My initial mutant for the new coverage was `case <-t.C: return ctx.Err()`, and it SURVIVED. The test was not weak: `ctx.Err()` is nil for a live context, so that mutant is semantically identical on the arm under test. I had chosen a no-op and would have read it as a vacuous assertion — the mirror of rule 3d, where a red in the predicted direction seduces; here a GREEN in the predicted direction nearly did.

**Queued from this iteration.** `m-ratelimit-window-default-unpinned` — the hardcoded `time.Second`
reset window in `checkRateLimit` has no tripwire anywhere; the judge mutated it to 500 ms and the
whole package stayed green, including both rate-limit tests and the new defaults test. PRE-EXISTING,
disclosed by the design doc itself as O2c, and deliberately not fixed in-flight.

**Routing evidence.** Gate 1 base=`f3783c9765d1fcdca51533c401bea6a7e53d19b8@2026-09-08T03:03:48Z`;
Gate 4 base recorded from the same helper after a fresh fetch. **Skill-staleness, both copies
checked:** the resolved symlink target (`~/dev/sunholo-data/ailang/.claude/skills/mission-control`,
inode `70895047`) is byte-identical to `origin/dev` on **all 12** files, while the **pin worktree's**
copy (inode `75367340`) drifts on **four** — `SKILL.md`, `gate-1-observe.md`, `gate-3-route.md`,
`gate-3b-ci-green.md`. This is exactly the split iteration 349's Gate-5 edit and motoko iter-39's
Gate-1 edit describe, observed live: I read every resource file from the **resolved** path per rule
(b)/(c), and the pin is 46 commits behind `origin/dev` because `AILANG_DRIVER_REF` is still the fixed
SHA `48c4a6e49` (the D-61 item, now 46 rather than 43 commits stale).
**Designer:** deliberately **NOT spawned** — the inherited r3 doc is complete and re-quorumed; a
designer run would have been a second Fable-class spend on a doc already past its carve-out.
**Planner:** `pi:ollama/kimi-k3:cloud` (the declared `MISSION_PLANNER_MODEL`; the resolver returned
`agent-tool opus fail-closed:no-doc` because the doc path resolves against the driver's CWD and the
doc lives only in the sprint worktree — the documented resolver/hook disagreement, so I followed the
pin per role-spawn-routing §2(a)/(c) rather than burning a spawn on a guaranteed denial). Verdict
`ok`, 547 s, 35 tool executions, 1 changed file, $0 (flat-rate ollama-cloud). A 1,145-line
executor-ready plan. **kimi's second consecutive successful planner run** — the first was iteration 348.
**Executor:** `pi:ollama/deepseek-v4-flash:0731-cloud recipe declared:provider-pin`, twice, both
through `scripts/mission_pi_run.sh` for the typed verdict. Round 1 `ok`, 567 s, 110 tool executions,
10 changed files. Round 2 `ok`, 233 s, 32 tool executions, 3 changed files — but it delivered only
FIX 1 of four and wrote no report file, so the controller completed FIX 2–4 inline as mechanical work
and handed all of it to the judge as controller-authored. **This is deepseek's second consecutive
sprint execution returning `ok` with a non-empty worktree diff, which is the promotion bar the
2026-08-26 rule sets** — recorded here, not acted on unilaterally.
**Evaluator:** `agent-tool sonnet declared:alias-pin`, **two rounds**, in its own dedicated worktree
`.wt-v1-iter351-eval`, round 2 resumed by `SendMessage` so nothing was re-run. Round 1: 163,922 tok /
69 tool uses / 792 s. Round 2: 202,569 tok / 25 tool uses / 424 s. Generator≠judge held at the agent
level in both rounds and, for the round-2 changes, is the *only* review those lines will ever get.
**Quorum:** none run this iteration — the inherited doc carries iteration 350's two artifacts and the
carve-out was already spent; re-quorumming an r3 doc no reviewer has objected to would be
re-litigation, which Gate 2 forbids.

**And my own correction cost a CI red, which is the honest half of this entry.** Deleting the
vacuous poll assertion took `defaultPollTick` to **zero** references repo-wide, so
`golangci-lint unused` reddened the PR (`approval_checkpoint.go:350:6: func defaultPollTick is
unused`). The linter was right, and what it was reporting is the sprint's own shape: this work made a
production default injectable and then tested only the injected side. Per CLAUDE.md's rule I did not
delete the function or `//nolint` it — I asked why it was unused (the constructor has zero production
callers, design doc V16) and covered it by BEHAVIOUR: `TestDefaultPollTickFires` asserts the returned
channel actually delivers a tick, proven non-vacuous by a compiling mutant returning nil (RED), file
restored byte-identical. Local `golangci-lint run --enable-only unused ./internal/coordinator/...`
now reports **0 issues**. Two lessons, both mine: a "purely subtractive" edit is not subtractive when
something else references what you removed, and the local gate list I baselined at Gate 2 did not
include the linter that caught it — rule 3g's hand-picked-subset gap, in my own sweep.

**Retro lane — process fix, not a skill edit.** The friction worth recording is that the **executor
lane silently under-delivered a multi-part directive**: round 2 asked for four numbered fixes, the
runner returned verdict `ok` (which is a statement about the worktree diff being non-empty, exactly
as designed), and only fix 1 was present — no report file, `agent_end_events: 4` rather than 1. The
typed verdict is doing its job and cannot see this: `ok` means "something changed", not "what you
asked for changed". The cheap guard is on the controller's side and costs nothing: **when a directive
enumerates N deliverables, check the changed-file set against the N before reading the report** — the
file list alone showed three files where four fixes needed at least six. Recorded here rather than
spent as this iteration's skill edit, because one instance is not the ≥2 the Gate-5 rule requires.

**Progress.** N=12 design docs before v1.0.0 (was 12, change 0). This is a HARNESS item outside the
ratified clause set, so the goal is unmoved; what moved is CI wall time on the slowest package in the
matrix and one production cancellation regression that never reached `dev`.

**Cost.** Metered **$0.00** of the $5 ceiling — both pi lanes are flat-rate ollama-cloud, the
evaluator is subscription-billed, and no quorum round was run.

**Next.** `m-fleet-sha-pin-freezes-every-driver-fix` if D-61 comes back (A); otherwise
`m-headroom-blocking-threshold-calibration` or `m-headroom-residual-mutations`, both iteration 348
residue with first-party evidence already attached. `m-ratelimit-window-default-unpinned` is cheap
and now has a measured mutation to guard against.

## 353 — 2026-09-14 — First fire in six days: the macOS flake was the capture helper, not the assertions; orphaned iteration 352 credited; three attended rulings acknowledged [HARNESS]

**Picked.** `m-debugcacheforms-flaky-on-macos-ci` — the row labelled `TOP OF QUEUE`, a red V1 owns
(`#1071` introduced the test) and one that was actively costing re-runs. The physically top `[NEXT]`
row, `m-fleet-sha-pin-freezes-every-driver-fix`, is blocked on D-61 — which came back **(A)** on
2026-09-08 **and was executed in the same attended session** (all four `mission-*.env` files now carry
only the `origin/dev` default; this fire logged `driver pin drift: 15`, i.e. the instrument reads
correctly once the ref is a branch). So that row is RESOLVED by attended action, not by the loop; its
instrument residue is re-queued as `m-pin-drift-blind-under-sha-pin`. That is bookkeeping, which is
what allows a second pick.

**Gate 0/1.** Kill switch armed — it had been present on every fire from 2026-09-08 to 06:28 today
and was gone by 07:58, so this is the first iteration in six days. Billing tripwire CLEAN. gh
`sunholo-voight-kampff`. Skill staleness: `SKILL.md` and all 12 `resources/*.md` byte-identical to
`origin/dev` in BOTH the resolved-symlink copy (`~/dev/sunholo-data/ailang`) and the CWD pin copy
(per-file verdicts printed, 13/13 SAME). `#1072`: 0 directives of 15 comments since the
`2026-09-07T23:20:56Z` watermark. `dev` `05a6457d1`: CI / Build and Release / Docs all `success`;
`commits/<sha>/check-runs` = 17 with one NOT-GREEN, `SonarCloud Code Analysis: failure`, inherited
(walked back: `90a1f5d7d`, `8bfba6fbf`, `e82665e47` all `failure`) — the standing branch-gate red the
charter already tracks. Main checkout `9845acf2c`: 0 ahead / 15 behind origin, 4 dirty paths
(two benchmark JSONs, two untracked docs) — untouched. **Attended ledger rulings acknowledged:**
D-61 (A, pins returned to `origin/dev` the same session), D-62 (generator≠judge PREFERRED at vendor
level, REQUIRED at model level), D-63 (NO-SINGLE-PROVIDER-ROLE retired) — all stamped
`attended 2026-09-08 (Mark)`; the flip commits carry the fleet identity, which the contract's own
2026-09-02 clarification says cannot distinguish the channels, so the stamp is the evidence. Ledger
63 rows, **ZERO open**.

**Gate 2 — an orphaned iteration.** `git worktree list` in this clone carries
`.wt-v1-iter352-eval` and `.wt-v1-iter352-eval-r2`; `origin/sprint/v1-iter352-daemon-task-exec-untested`
exists; PR [#1113](https://github.com/sunholo-data/ailang/pull/1113) MERGED at `11:33:53Z` on
2026-09-08 as [`45f02deb3`](https://github.com/sunholo-data/ailang/commit/45f02deb3). And `grep -c
"ITERATION 352"` = **0** in the charter and the log; the slot-verdicts log ends at 351's
`COMPLETED`. Iteration 352 ran the entire inner loop on `m-daemon-task-exec-run-untested` (design r2,
quorum r1/r2 blocked → carve-out, plan, executor, judge `sonnet` r1 **PASS 85/100**, two surviving
mutants closed in `25d317650`), merged it, and died leaving no record. Verified before crediting:
`test`/`lint`/`build`/`docs-gate` all `success` on `45f02deb3`; the 325-line r1 evaluation is in the
PR; the only NOT-GREENs on the merge SHA are the standing Sonar red, a `Build windows-latest`
fail-fast cancel, and one `Build macos-latest` failure — **this iteration's own pick, biting the
previous iteration's landing**. Whether a round-2 verdict was obtained after `25d317650` is UNKNOWN
(no artifact anywhere); recorded as such. Docs moved to `implemented/v0_35_3/` together with
iteration 351's `m-coordinator-test-parallelism` pair, which had also never been moved.

**Reality check — measure the mechanism before routing.** The charter row said "28 local runs cannot
reproduce it" and guessed at timing-dependent text. Measured: `-count=20 -race` green locally
(`--- PASS:` present, so the selector matched). The one fetchable CI log showed the cold capture
containing ONLY the first line the pipeline writes. The helper (`pipeline_module_phases_test.go:28-44`)
tears down as `os.Stderr = old; w.Close(); r.Close(); <-done` — the READ end closed before the copier
goroutine is waited on. A 60-line standalone probe with a reader that is slow between reads loses
the tail **50/50** under that ordering and **0/50** with `<-done` before `r.Close()` — same reader,
same writes, one variable changed. Systemic audit: 14 `_test.go` files use `os.Pipe()`; exactly one
carries the close-before-wait order (awk window, positive control on `prelude_cap_gate_test.go`'s
correct order). Not the mechanism: no goroutines in the module-phase path (0 hits, control fires on
`metrics.go`), zero `t.Parallel()` in the package.

**Shipped.** PR [#1161](https://github.com/sunholo-data/ailang/pull/1161) → squash
[`3faaf47fa`](https://github.com/sunholo-data/ailang/commit/3faaf47fa06549c94eac95432a6933ee11df4cf6),
**22 checks on the PR head, ZERO not-green**, `MERGEABLE`/`CLEAN` re-read at merge time.
`capturePipelineStderrWith(opts, f) (out string)`: a named-return `defer` that restores `os.Stderr`,
closes the writer, runs a test-only drain-wait hook, waits (bounded 10 s) on the copier's completion
channel, closes the reader, **panics on a non-nil copy error**, then assigns the return;
`capturePipelineStderr(f)` unchanged at its three call sites. Four helper tests with a channel-gated
reader (no sleeps): `GatedReaderLosesNothing`, `PanicRestoresStderr`, `DrainTimeoutIsLoud`,
`CopyErrorIsLoud`. Changelog entry under `## [Unreleased]`. Six-mutant audit.

**The sharpest measurement came out of the drill, and it is about HEAD, not about the fix.** With
MUT-1 applied (the old ordering restored, copy error now propagated) and the four new tests
`-skip`ped, `TestPipelineModulePhases_DebugCacheFormsAndCounters` is **red locally on the first
run**: `copier failed: read |0: file already closed`. The read-on-closed-fd error was happening on
every run on this laptop too; HEAD swallowed it, and the buffer merely happened to be complete. The
CI runner differs only in WHEN the close lands relative to the data. Recorded in the doc's V21 and the
audit; the judge measured the same red set independently.

**Independent judge** `agent-tool sonnet`, in its own worktree, both rounds. **Round 1: PASS 88/100,
zero blocking**, five non-blocking findings, each reproduced by me before disposition: (1) V21 left
`UNMEASURED` after M2 had produced the block — transcribed; (2) the doc's MUT-3b `rc=2` claim — rc=1
measured three ways, corrected; (3) `GatedReaderLosesNothing`'s post-loop `os.Stderr == nil` check
could not fail — reproduced (restore-skip mutant landed, vet rc=0, test rc=0), fixed to compare with
the ORIGINAL, same mutant now rc=1, fixed form four `--- PASS:` lines; (4) MUT-1 and MUT-2a red sets
broader than the single named killer (rule 3j — recorded, not a defect); (5) a regressed bounded
wait would hang CI's un-scoped `internal/pipeline` run to the 10-minute default — loud, declared.
**Round 2 on the r3 delta (`SendMessage` resume, same transcript): PASS 98/100, zero blocking, all
five CLOSED**, every number in the three doc changes re-verified against `e15d27a3b`.

**Ruled out.**
- *"Only 1 of the 6 failed macOS jobs failed on this test; the rest are an external cancellation
  mode"* (the designer's first draft, refuting my briefing) — **REFUTED by the raw job logs**:
  `gh api --allow-escape-sequences …/jobs/<id>/logs` for 103770676134 / 103763201443 / 103758453565 /
  103770669326 → **4 of 4** `--- FAIL: TestPipelineModulePhases_DebugCacheFormsAndCounters` (three
  `:245 warm skip diagnostic missing: ""`, one `:232`); the `cancelled` jobs are the other matrix legs,
  completing 8–25 s after the `failure` leg under the default `fail-fast: true`. The designer's
  instrument (`gh run view --log`) errors on logs containing terminal escape sequences — my own first
  `gh api` call returned the same 99-byte error — and the empty grep was read as absence (rule 3a).
- *"14 test files call HangGuard"* (my directive) — **REFUTED by the designer**: 14 LINES, 1 external
  file (`internal/eval_harness/reference_solutions_test.go`); a 3b(ix) scope error, corrected in V18.
- *"Timing-dependent `cached 0s ago` text trips the assertion"* (the queue row) — the warm assertion
  is a prefix `strings.Contains(warm, "[CACHE] answer: SKIP (cached ")`; no duration is compared.
- *"Assert on the counters structurally"* (the queue row's remedy) — a structural oracle reads
  through the same truncated capture; declined as scope, root cause fixed instead.
- *"Open a `## [v0.38.6] - 2026-09-14` changelog header"* (the planner's D-2 resolution) — the
  section the doc named had been consumed by the v0.38.5 release; `git log -S'## [Unreleased]'`
  shows every release renaming it, so the convention is `## [Unreleased]`, corrected before routing.
- *"MUT-3b exits rc=2"* (the r2 doc) — rc=1 (executor, controller, judge ×3).
- *"A sleep-based slow reader makes MUT-1 deterministically red"* (the r1 design) — astra's
  objection stands: a descheduled writer lets a sleeping reader drain first; replaced by the
  channel-gated reader whose first `Read` cannot precede the release hook.

**Queued from this iteration.** `m-pin-drift-blind-under-sha-pin` (instrument residue of the
resolved fleet-pin row) and `m-weekly-sweep-orphans-2026-09-14` (7 orphans of 85 enumerated — five
daneel-filed issues, four of which already carry design docs landed 2026-09-13; `#1096`, the pi
runner's `empty_worktree` verdict on a committing executor, a sibling of D-58's parked row; and the
docs mission's own bookkeeping thread, which is not an orphan). Also owed by later iterations:
...
rotation pointer `pi:ollama/deepseek…` → `claude:claude-fable-5-1` (advanced below) (tok: not
reported — `claude -p` prints no usage) · **planner**: resolver `agent-tool opus
fail-closed:path-not-in-codex-allowlist` vs `MISSION_PLANNER_MODEL=pi:ollama/kimi-k3:cloud` → the pin
followed under the pi recipe (role-spawn-routing §2(a)); probe rc=0; `mission_pi_run.sh` verdict `ok`
709 s / 39 tool executions / 31 turns, in=64,532 out=27,981 cacheRead=859,193 — **kimi's third
consecutive successful planner run** · **executor** `recipe pi:ollama/deepseek-v4-flash:0731-cloud
declared:provider-pin`; probe rc=0; verdict `ok` 802 s / 110 tools / 100 turns, in=104,949
out=55,912 cacheRead=6,379,994, 5 changed files — **deepseek's third consecutive `ok`** with a
non-empty diff · **evaluator** `agent-tool sonnet declared:alias-pin`, own worktree: r1 190,632 tok /
92 tool uses / 1,226 s; r2 221,367 tok / 24 tool uses / 334 s · **quorum** r1 `blocked` (gemini
reject $0.0288, glm pass $0.0208, astra ABSENT budget → `design-review --max-cost-usd 0.40` reject
$0.1322), r2 `blocked` 3/3 zero absentees (astra $0.1733, gemini $0.0377, glm $0.0570) → r3 by the
narrow-refinement carve-out (every objection concrete, none on direction). codex lanes ration-blocked
this fire (`gpt-5.6-sol` probe rc=75, driver-substituted before the controller ran). Generator (pi,
DeepSeek) ≠ judge (Anthropic sonnet): vendor-distinct, so D-62's PREFERRED level held. task-class
mix: design ×2, plan, execute, evaluate ×2. provider=anthropic+ollama-cloud+openrouter-quorum
metered=**$0.4498** quota-buckets: fable (2 runs), sonnet (2 runs), opus (controller).

**Retro lane — skill fix (one edit, two frictions).** Both the controller's first job-log fetch and
the designer's V2 hit the same instrument trap: GitHub job logs that carry terminal escape sequences
make `gh api …/logs` return a 99-byte refusal (rc=1) and `gh run view --log` an error line, and an
empty `grep` over that was read as "the test did not fail here". Gate 1 / ci-health prescribe both
commands and neither names the flag. Edit: `resources/gate-1-observe.md` — when reading a job log,
pass `--allow-escape-sequences`, strip ANSI before grepping, and treat a sub-200-byte log body as an
instrument failure, never as an absence. Landed via this record's PR; the running copy resolves to
the main checkout, which is 15 commits behind origin and not this loop's to reconcile (Gate 1: a
human authorises a reconcile), so it takes effect when that checkout is advanced.

**Progress.** N=12 design docs before v1.0.0 (was 12, change 0). HARNESS: goal unmoved. What moved:
a red the loop owned is fixed at its root, one dead iteration's landing is on the record, and three
attended rulings are acknowledged so no later iteration re-asks them.

**Cost.** Metered **$0.45** of the $5 ceiling (quorum ×2 + one raised-cap astra review); pi lanes
flat-rate ollama-cloud; Fable one doc (authoring + one revision) — diet intact; sonnet two judge
rounds.

**Next.** `m-sonar-dev-branch-security-rating-c-on-new-code` (the standing branch-gate red, deferred
four iterations now) or `m-approval-poll-production-defaults-unexercised` / `m-ratelimit-window-default-unpinned`
