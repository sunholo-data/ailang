...
**Landed** (doc-only, dev): the reshaped doc (`Status`→PARKED-needs-human-review + PARK-NOTE + new Phase-2 + superseded banners) + the two quorum machine artifacts + this log. Committed to dev this iteration (SHA in the #399 report). Gate-3b: design_docs/*.md is **not** under any CI `on.push.paths` filter → no CI/Build/Docs run is EXPECTED for a docs-only `design_docs/` change (recorded N/A, not pending — per the "poll only checks that CAN complete" rule); nothing to wait on.
...
**Retro lane**: **WATCH signal reaches its 2nd instance but I deliberately hold the skill edit** — the roles-table Controller-row parenthetical ("+ design-doc-creator, run inline") vs the Designer-ROTATION row / Gate-3 "spawned pinned, never inline" contradiction was flagged as a 1-instance process-fix candidate at iter-47; this iteration followed the newer rule (spawned rotation designer) again = 2nd instance. Per Gate-5 (max ONE skill/process edit per iteration, ≥2 frictions), this now MEETS the bar — but this iteration's single edit budget is better spent NOT touching the skill mid-park; recording the 2nd instance here so the NEXT design-routing iteration makes the parenthetical-removal its retro deliverable (delete "+ design-doc-creator, run inline" from the Controller row; the ROTATION row governs). No other ≥2-convergence. No routing-policy change (needs ≥3 rows).
