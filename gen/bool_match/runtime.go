...
// ConvertToContentSlice converts []interface{} to []*Content.
// M-DX12: Fail-fast - panics on type mismatch (compiler bug detection).
func ConvertToContentSlice(v interface{}) []*Content {
	if v == nil {
		return nil
	}
	src, ok := v.([]interface{})
	if !ok {
		panic(fmt.Sprintf("ConvertToContentSlice: expected []interface{}, got %T", v))
	}
	if len(src) == 0 {
		return []*Content{}
	}
	...
	for i, e := range src {
		elem, ok := e.(*Content)
		if !ok {
			panic(fmt.Sprintf("ConvertToContentSlice: element %d: expected *Content, got %T", i, e))
		}
		out[i] = elem
	}
	return out
