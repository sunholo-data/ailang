...
| **Is the codebase getting more complex?** | `make simplicity-audit` — re-measures the v1.0.0 simplicity gates (core closure, command/env-var routes, duplicate symbol names, undocumented packages, instruction surface), diffs against the last banked snapshot in `.ailang/state/simplicity/`, exits 2 on a regression. `make test-core` is the language inner loop (<15s). Program: `design_docs/planned/m-v1-simplification-program.md`. |
