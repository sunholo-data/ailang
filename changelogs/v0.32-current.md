...
### Added — persistent browser auth profiles (in progress)
...
`internal/browser/auth` holds the contract: immutable profile versions, an
opaque `SensitiveProfileMaterial` whose only extraction path is `Materialize()`,
exact-origin policy that fails closed on every unset field, and ten stable
`browser_auth_*` failure categories. Saved browser state — cookies, local
storage, IndexedDB tokens, virtual passkeys — is classified as a credential
equal to the password that produced it, so `SanitizeDiagnostics` now redacts
storage state, provider context IDs, sealed blobs, passkeys, and recovery codes
alongside the existing markers.

`latest` is resolved to a concrete version before a run starts, so a result
records what actually ran rather than a moving reference.
