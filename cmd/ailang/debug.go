...
func runDebug() {
	...
	case "types":
		// Point to the --debug-types flag (v0.5.11+)
		fmt.Println(cyan("=== Type Debugging ==="))
		fmt.Println()
		fmt.Println("Use the --debug-types flag with 'ailang run' for type inference debugging:")
		fmt.Println()
		...
		fmt.Println("  - Substitution Map: Type variable → resolved type")
		fmt.Println("  - Constraints: Type class constraints and resolution")
		fmt.Println("  - CoreTI Entries: Type for each Core AST node")
		fmt.Println()
		fmt.Println("For programmatic access, use TypeReport API (internal/types/type_report.go):")
		fmt.Println("  report := tc.TypeReport(nodeID)")
		fmt.Print("  fmt.Printf(\"Raw: %", "s, Resolved: %", "s\", report.Raw, report.Resolved)\n")
