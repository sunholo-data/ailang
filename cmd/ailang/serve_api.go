...
func printServeAPIHelp() {
	...
	fmt.Println("  Use @route(\"METHOD\", \"/path\") on exported functions for custom URL paths:")
	fmt.Println("    @route(\"POST\", \"/api/v1/parse\")")
	fmt.Println("    export func parse(content: string) -> Result ! {IO}")
	fmt.Println()
	fmt.Println("  Use @raw with @route to receive full HTTP request context (headers, body, etc.):")
	fmt.Println("    @raw")
	fmt.Println("    @route(\"POST\", \"/webhooks/stripe\")")
	fmt.Println("    export func handle(req: {body: string, method: string, path: string}) -> string ! {IO}")
	fmt.Println("  @raw record fields: body, headers, method, path, query")
	fmt.Println()
	...
	fmt.Println("  Deploy to Cloud Run with default concurrency (80 requests per instance).")
	fmt.Println("  Test with: ./tools/test-concurrency.sh ./api/")
	fmt.Println("  Debug with: DEBUG_CONCURRENCY=1 ailang serve-api ./api/")
	fmt.Println()
