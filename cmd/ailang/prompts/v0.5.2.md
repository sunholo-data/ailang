# AILANG v0.5.2 - AI Teaching Prompt
...
**NEW in v0.5.2:** **Relaxed Module Matching** - Files in temp directories auto-relax MOD010 validation. For prototyping: `--relax-modules` flag or `AILANG_RELAX_MODULES=1` env var allows module path mismatches with warnings instead of errors.

**Also in v0.5.0:** **AI effect** for pluggable AI/ML oracle. Call external AI providers (Anthropic, OpenAI, Google) with a simple string→string interface. Import `std/ai (call)` and use `AI.call(prompt)` to get AI responses. Requires `! {AI}` effect and `--caps AI` at runtime.
...
**Relaxed Module Matching (v0.5.2):** For prototyping, you can relax module path validation:
```bash
# CLI flag
ailang run --relax-modules --caps IO --entry main file.ail
# Environment variable
AILANG_RELAX_MODULES=1 ailang run --caps IO --entry main file.ail
# Auto-relaxation: Files in /tmp/ or /var/folders/ auto-relax with warning
```
