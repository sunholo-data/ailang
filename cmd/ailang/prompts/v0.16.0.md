...
## WASM-Friendly Patterns (v0.22.x+)

**Status**: Required for modules targeting the browser via `wasm/ailang.wasm`. Optional but recommended for all AILANG code.
**Since**: v0.22.x (M-WASM-TYPECHECK-LIMITS)
**Applies to**: code that will be loaded into AILANG-WASM in a browser demo.

The AILANG WASM type-checker runs on the host JavaScript engine's call stack
(~10–15K frames). Certain code shapes that work on the CLI in milliseconds
hang the browser main thread for 80+ seconds before tripping a structured
error. Code-gen for WASM-targeted modules should avoid the five compounding
patterns below.

**These patterns work fine on the CLI.** They are a WASM-specific concern.
But the workarounds make all code shorter and more idiomatic anyway, so
prefer them even for CLI-only code.

### Pattern 1: Triple-nested `match`

DON'T (catches the cliff):
```ailang
-- Three matches stacked inside each other
match decode(raw) {
  Err(e) => Err(e),
...
DO (flatten with helpers + sequential let-bindings):
```ailang
...
### Pattern 2: Repeated matches on the same tagged-union with `.field` access

DON'T:
```ailang
-- Three separate matches on the same Result; each `s.x` / `s.y` triggers
-- fresh tagged-union receiver analysis on `s`.
...
DO (one match → record with all fields → read fields downstream):
```ailang
...
-- now score.has / score.x / score.y are plain record-field reads,
-- no repeated tagged-union analysis.
```
...
DON'T:
```ailang
-- 11 imports, 8 destructured names including 3 sum constructors —
-- each constructor brings type-scheme work for ADT-head preservation.
...
DO (import only what you actually use; prefer qualified module use):
```ailang
import cognitive_commons/services/consensus
  ( CommonsState, applyUtterance, applyManifestoEdit, RejectedTooShort )
-- ...and call other functions via `consensus.currentLeader(...)` if needed.
```

### Pattern 4: Multi-effect annotations on hot-path functions

DON'T (every effect adds a row constraint to the solver):
```ailang
export func speak(...) -> Result[CommonsState, string] ! {AI, DOM, Msg, IO} = {
  -- 60 lines of work mixing AI calls, DOM patches, and Msg sends
}
```

DO (split orchestration into smaller single-effect or two-effect functions):
```ailang
...
export func speak(...) -> Result[CommonsState, string] ! {AI, DOM, Msg} = {
  -- Just orchestration, no heavy work in the multi-effect function body.
  match compose_only(...) {
    Err(e)    => Err(e),
...
DON'T (`.sentiment.x` inside string interpolation, four times):
```ailang
...
DO (let-bind the intermediate record once):
```ailang
let s = st.sentiment;
...
### What goes wrong without these workarounds

When a WASM-loaded module trips the budget guard, the user sees a clear error
naming the module and pointing back here. The previous failure mode (before
v0.22.x) was a silent 80–120-second browser freeze. Either is better than
the freeze, but the workarounds above prevent the error from firing in the
first place.

For the full diagnostic background see
[`docs/docs/reference/limitations.md` § "WASM Type-Checker Depth Limit"]
and [`design_docs/planned/v0_22_0/m-wasm-typecheck-limits.md`].
