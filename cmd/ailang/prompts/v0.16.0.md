...
## WASM-Friendly Patterns (cross-reference)

**Targeting browser/WASM?** Five code shapes can hang the browser type-checker
even though they work fine on the CLI (triple-nested `match`, repeated
matches on the same tagged-union with field access, heavy intra-package
imports with sum-constructor destructuring, multi-effect annotations on
hot-path functions, deep record-field access in tight expressions).

Full guide + bad/good examples for each pattern lives in the **devtools
prompt** (toolchain reference) where the rest of the WASM toolchain docs are:

```bash
ailang devtools-prompt | grep -B 1 -A 80 "WASM-Friendly Patterns"
```

Or see [`docs/docs/reference/limitations.md` § "WASM Type-Checker Depth Limit"]
for the diagnostic background.
