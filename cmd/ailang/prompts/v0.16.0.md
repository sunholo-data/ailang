...
## Idiomatic AILANG: Pure Core, Effectful Shell

AILANG's effect rows make the "functional core, imperative shell"
architecture (Bernhardt 2012) **visible in the type system**. In Haskell,
every effectful function is `IO a` — you can't tell from the signature
whether a function only reads files or also touches the network, AI
oracles, and global state. AILANG's `! {FS}` vs `! {FS, Net, AI}` is
right there in the signature, so a function with too many effects is
visibly mixed-concern.

**Aim for this shape in every non-trivial AILANG module:**

  - **Pure helpers everywhere possible**: `pure func`. Use the
    annotation explicitly even when the body has no effects — the
    annotation documents intent and the compiler verifies it.
  - **Single-effect leaves**: each I/O-touching function with the
    smallest effect row that actually fits — `! {AI}`, `! {DOM}`,
    `! {Msg}`, `! {FS}`. One effect each. Easier to test, easier to
    replace per-target, easier to reason about.
  - **Multi-effect orchestrators at the top**: ONE outer function
    that sequences single-effect leaves. Its body reads like a
    storyboard ("compose → judge → patch → broadcast") with no
    business logic of its own.

Read a well-styled AILANG module and you can describe its architecture
just from the function signatures — without opening any function body.

### Five concrete patterns

These flow from the pure-core/effectful-shell discipline. Each has a
BAD-style (works but mixed-concern) and a GOOD-style (idiomatic, also
easier to test). The GOOD versions are also faster to type-check, which
matters when targeting WASM (see *Bonus: WASM type-check budget* below).

#### Pattern 1 — Flatten nested `match`

BAD (three matches stacked, nested arms hide the control flow):
```ailang
match decode(raw) {
  Err(e) => Err(e),
...
GOOD (one match per layer, helper + sequential lets reveal the
algorithm: parse → check error → extract text):
```ailang
...
#### Pattern 2 — Destructure tagged unions once

BAD (three separate matches on the same `Result`, each `s.x` / `s.y`
re-runs tagged-union receiver analysis on `s`):
```ailang
...
GOOD (one match → flat record → plain field reads downstream):
```ailang
...
let score = unpack_score(score_result);
-- now score.has / score.x / score.y are ordinary record-field reads
```

#### Pattern 3 — Import only what you use

BAD (8 destructured names, half unused — clutters the interface and
the type-checker has to instantiate each ADT-head):
```ailang
...
GOOD (drop unused symbols; trims the implicit dependency surface):
```ailang
import cognitive_commons/services/consensus
  ( CommonsState, applyUtterance, applyManifestoEdit, currentLeader, RejectedTooShort )
```

#### Pattern 4 — Single-effect leaves, multi-effect orchestrator

This is the most important pattern. The effect-row in the signature
documents what each piece can do; mixing them in one function defeats
the discipline.

BAD (one function doing everything — invisible to the type-checker as
a code-smell, but visible in the row):
```ailang
export func speak(...) -> Result[CommonsState, string] ! {AI, DOM, Msg, IO} = {
  -- 60 lines mixing AI calls, DOM patches, Msg sends, println logging
}
```

GOOD (single-effect leaves + orchestrator):
```ailang
func compose_only(...)   -> Result[AuthorReply, string] ! {AI}  = { ... }
func render_only(reply, region: string) -> () ! {DOM} = { ... }
func broadcast_only(reply) -> () ! {Msg} = { ... }

export func speak(...) -> Result[CommonsState, string] ! {AI, DOM, Msg} = {
  match compose_only(...) {
    Err(e)    => Err(e),
    Ok(reply) => { render_only(reply, region); broadcast_only(reply); Ok(...) }
  }
}
```

The orchestrator's body reads like a storyboard. Each leaf is testable
with one effect mocked. You can swap renderer implementations (DOM →
SSR string) without touching compose or broadcast.

#### Pattern 5 — Let-bind chained path access

BAD (`.sentiment.x` four times — same chain walked four times):
```ailang
...
GOOD (let-bind the intermediate record once):
```ailang
let s = st.sentiment;
...
### Bonus: WASM type-check budget

The patterns above are good style regardless of target. On WASM
specifically, they're also **required** to stay inside the 2-second
type-check budget (v0.22.x+). Code that violates them works on the
CLI in milliseconds but can hang the WASM type-checker for 80+
seconds. The budget guard catches the worst cases at runtime, but
prevention beats catching.

For the toolchain side of WASM (build flags, harness, budget-error
diagnostics), see `ailang devtools-prompt | grep -A 20 "WASM Type-Checker Budget"`.

For runnable bad/good worked examples of each pattern, see
[`examples/wasm_friendly_patterns.ail`](https://github.com/sunholo-data/ailang/blob/dev/examples/wasm_friendly_patterns.ail).

For the diagnostic background on the failure mode that motivated this
section, see [`docs/docs/reference/limitations.md` § "WASM Type-Checker
Depth Limit"](https://ailang.sunholo.com/docs/reference/limitations).
