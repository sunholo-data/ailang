...
**Foldl with curried callback:**
```ailang
module benchmark/solution
import std/io (println)

-- Multi-arg func - call foldl(f, acc, xs)
func foldl(f: int -> int -> int, acc: int, xs: [int]) -> int =
  match xs {
    [] => acc,
    x :: rest => foldl(f, f(acc)(x), rest)  -- f is curried: f(acc)(x)
  }

export func main() -> () ! {IO} {
  let add = \a. \b. a + b;           -- Curried lambda
  let sum = foldl(add, 0, [1,2,3,4,5]);  -- foldl is multi-arg
  println(show(sum))
}
```
...
import std/list (map, filter, foldl, length, concat)  -- NO flatten! Use foldl(concat, [], lists)
