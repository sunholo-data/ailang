# AILANG v0.5.1 - AI Teaching Prompt

## ⚠️ MANDATORY STRUCTURE (adapt to your task)

**Every AILANG program MUST start with this structure:**
...
**CRITICAL:**
- `print` expects a **string**. Use `show(value)` to convert numbers/records to strings!
- `show` is a builtin - no import needed
- **READ THE TASK** and write code that solves IT, not the examples below!

**ABSOLUTE RULES:**
1. **Line 1 MUST be `module benchmark/solution`** - No exceptions!
2. **Use `print(show(value))` for numbers**, `print(str)` for strings
3. **Semicolons ONLY inside `{ }` blocks**
4. **No loops** - Use recursion instead

## Reference Examples (learn the patterns, don't copy verbatim)

### Print a calculation (modulo, arithmetic):
```ailang
module benchmark/solution
...
### Filter a list recursively (NO for/filter/where):
```ailang
module benchmark/solution

export func filterAge(people: [{name: string, age: int}], minAge: int) -> [{name: string, age: int}] {
  match people {
    [] => [],
    p :: rest => if p.age >= minAge
                 then p :: filterAge(rest, minAge)
                 else filterAge(rest, minAge)
  }
}

export func main() -> () ! {IO} {
  let data = [{name: "Alice", age: 30}, {name: "Bob", age: 25}];
  print(show(filterAge(data, 28)))
}
```

---

**CRITICAL: Write code in AILANG syntax. NOT Python, NOT Rust, NOT JavaScript, NOT pseudo-code.**

AILANG is a pure functional language with Hindley-Milner type inference and algebraic effects.

**NEW in v0.5.0:** **AI effect** for pluggable AI/ML oracle. Call external AI providers (Anthropic, OpenAI, Google) with a simple string→string interface. Import `std/ai (call)` and use `AI.call(prompt)` to get AI responses. Requires `! {AI}` effect and `--caps AI` at runtime.

**Also in v0.5.0:** **Array type** with O(1) indexed access for game grids and performance-critical code. Syntax: `#[1, 2, 3]` (literal), `Array[int]` (type). Import `std/array` for operations.

**Also includes v0.4.8 features:** Stronger template enforcement, clearer structure requirements. **Import aliasing** for module and symbol renaming.

**Also includes v0.4.7 features:** Inline tests with cross-function dependency support (M-TESTING-DEPS):
- **Inline Tests** - Executable documentation with `tests [...]` on function definitions
- **Cross-Function Dependencies** - Test functions that call other user-defined functions
- **Mutual Recursion Support** - Test isEven/isOdd and similar patterns
- **Self-Documenting Code** - Expected inputs/outputs visible in function definition

**Also includes v0.4.6 features:**
- **What AILANG Does NOT Have** - Clear examples of multiple modules per file, semicolons outside blocks, imperative constructs
- **Multi-Module Projects** - Complete example showing myapp/data.ail, myapp/storage.ail, myapp/main.ail
- **Operators & Functions Reference** - Quick lookup table for all operators (including `%`), list/string/IO/FS/Net/JSON functions
...
- **Import Aliasing (v0.4.8+):**
  - Module alias: `import std/list as List` → use `List.length(xs)`, `List.map(f, xs)`
...
- Effect: `! {IO, FS, Net, Env, Debug, AI, Rand, Clock}` after return type
...
- **Array (v0.4.10+):** `#[1, 2, 3]` (literal) | `import std/array as A` → `A.get(arr, i)` (O(1)), `A.getOpt(arr, i)` (safe), `A.set(arr, i, v)` (O(n) copy), `A.length(arr)`
- Type: `int -> bool` (sugar v0.4.1+) or `funcType int bool` (canonical)
- **Testing (v0.3.20+):** `test "name" = expr` | `property "name" (x: int) = expr`
- **Inline Tests (v0.4.7+):** `pure func f(x: int) -> int tests [(0, 0), (5, 25)] { x * x }` - executable documentation!
...
## Effect Module APIs (v0.5.1)

**⚠️ CRITICAL: Know the function signatures before using!**

**💡 Discover any function signature:**
```bash
...
### std/rand - Random Numbers (Rand effect)
```ailang
import std/rand (rand_int, rand_float, rand_bool, rand_seed)

-- Generate random integer in [min, max] inclusive
rand_int(min: int, max: int) -> int ! {Rand}
-- Example: rand_int(1, 6) -- Roll a d6

-- Generate random float in [min, max)
rand_float(min: float, max: float) -> float ! {Rand}
-- Example: rand_float(0.0, 1.0)

-- Generate random boolean (50/50)
rand_bool() -> bool ! {Rand}

-- Set seed for deterministic results
rand_seed(seed: int) -> () ! {Rand}
-- Example: rand_seed(42) -- Same seed = same sequence
```
...
export func main() -> () ! {IO, Rand} {
  rand_seed(42);  -- Deterministic!
  let roll = rand_int(1, 6);
  print("You rolled: " ++ show(roll))
}
```
**Run:** `ailang run --caps IO,Rand --entry main file.ail`

### std/debug - Debug Tracing (Debug effect)
```ailang
import std/debug (log, check)

-- Log a debug message
log(msg: string) -> () ! {Debug}

-- Check a condition (assertions are collected, not thrown)
check(cond: bool, msg: string) -> () ! {Debug}
```

### std/clock - Time (Clock effect)
```ailang
...
### std/ai - AI Oracle (AI effect)
```ailang
import std/ai (call)

-- Call AI with string input, get string output
call(input: string) -> string ! {AI}
-- Input/output typically JSON by convention
```

### std/game - Game Timing (Clock effect)
```ailang
import std/game (delta_time, total_time, frame_count)

-- Time since last frame (seconds)
delta_time() -> float ! {Clock}

-- Total game time (seconds)
total_time() -> float ! {Clock}

-- Current frame number
frame_count() -> int ! {Clock}
```
...
❌ **WRONG - COBOL/pseudo-code:**
```
PROGRAM Remainder
    DISPLAY 5 MOD 3
END PROGRAM
```
This is NOT AILANG. Use the template above!

❌ **WRONG - Custom DSL syntax:**
```
HTTP POST "url" WITH HEADERS [...] WITH BODY {...} STORE IN response
```
AILANG is NOT a DSL. Use actual AILANG function calls!

❌ **WRONG - Uppercase keywords:**
```
PRINT 5 % 3
```
AILANG uses lowercase: `print`, `let`, `if`, `match`

❌ **WRONG - Python for/comprehensions:**
```python
filtered = filter people where person.age >= 30
for p in filtered:
    print(p.name)
```
AILANG has NO `for`, `filter...where`, `in`. Use `match` and recursion!
...
```ailang
let remainder = 5 % 3  -- ✅ Works! Returns 2
let check = 10 % 3     -- ✅ Returns 1
```
**Note:** The `%` operator works for both `int` and `float` types.
...
❌ **WRONG - Nested function definitions with `func` keyword:**
```ailang
func outer(x: int) -> int {
  func inner(y: int) -> int {  -- ❌ Parse error! Can't use 'func' inside expressions
    y * 2
  }
  inner(x) + 1
}
```
The `func` keyword ONLY works at module level. For nested functions, use lambda expressions:
```ailang
export func outer(x: int) -> int {
  let inner = \y. y * 2 in  -- ✅ Correct - lambda expression
  inner(x) + 1
}

-- Or inline the lambda:
export func processItems(xs: [int]) -> [int] {
  let double = \x. x * 2 in
  map(double, xs)
}
```
...
Modules, functions, lambdas, pattern matching, ADTs, records, effects (`! {IO, FS, Net, Env, Debug, AI}`), recursion, numeric conversions, HTTP with headers, **JSON encoding/decoding**, **property-based testing**, **environment variables**, **syntactic sugar** (`::`, `->`, `f()`), **Debug effect** for game/simulation tracing, **AI effect** for pluggable AI oracle.
...
- `%` - Modulo (remainder) - Example: `10 % 3` returns `1`
- `**` - Exponentiation
...
- `length(xs: [a]) -> int` - List length
- `head(xs: [a]) -> Option[a]` - First element (returns None for empty list)
- `tail(xs: [a]) -> [a]` - All elements except first (returns [] for empty list)
- `reverse(xs: [a]) -> [a]` - Reverse list
- `concat(xs: [a], ys: [a]) -> [a]` - Concatenate two lists (NOT `append`!)
- `zip(xs: [a], ys: [b]) -> [(a, b)]` - Zip two lists into list of pairs
- `map(f: (a) -> b, xs: [a]) -> [b]` - Apply function to each element
- `filter(p: (a) -> bool, xs: [a]) -> [a]` - Keep elements matching predicate
- `foldl(f: (b, a) -> b, acc: b, xs: [a]) -> b` - Left fold (reduce)
- `foldr(f: (a, b) -> b, acc: b, xs: [a]) -> b` - Right fold
...
- `split(s: string, delimiter: string) -> [string]` - Split string by delimiter (**v0.4.7+**)
  - Empty delimiter splits into individual characters (UTF-8 codepoints)
  - Preserves empty fields: `split("a,,c", ",")` => `["a", "", "c"]`
  - `split("", "")` => `[]` (only case returning empty list)
  - Examples: `split("a,b,c", ",")` => `["a", "b", "c"]`, `split("hello", "")` => `["h", "e", "l", "l", "o"]`
...
let chars = split("hi", "")  -- ["h", "i"]

-- Compose split + trim for CSV with whitespace
let rawFields = split("a, b , c", ",")  -- ["a", " b ", " c"]
let cleanFields = map(trim, rawFields)  -- ["a", "b", "c"]

-- Config file parsing: split → fixed-arity pattern match
let parts = split("key: value", ":")
match parts {
  [k, v] => {key: trim(k), value: trim(v)},
  _ => defaultConfig
}
```
...
| `std/debug` | `log`, `check` | `import std/debug as Debug` |
| `std/ai` | `call` | `import std/ai (call)` |
...
-- Recursive sum with inline tests
export pure func sum(xs: List[int]) -> int
  tests [
    ([], 0),
    ([5], 5),
    ([1, 2, 3], 6),
...
export pure func length[a](xs: List[a]) -> int
  tests [
    ([], 0),
    ([42], 1),
    ([1, 2, 3], 3)
...
## Array Type (v0.4.10+) - O(1) Indexed Access

**Arrays provide O(1) random access** - use when you need fast indexed lookups (game grids, lookup tables).

**Syntax:**
- Literal: `#[1, 2, 3, 4, 5]` (hash + brackets)
- Type: `Array[int]`, `Array[string]`, `Array[{x: int, y: int}]`

**Import:** `import std/array as A`

**Indexing:** Arrays use **0-based indexing** (first element at index 0, last at `length - 1`).

**Operations:**
| Function | Signature | Cost | Description |
|----------|-----------|------|-------------|
| `A.make(n, v)` | `(int, a) -> Array[a]` | O(n) | Create array of size n with default v |
| `A.get(arr, i)` | `(Array[a], int) -> a` | O(1) | Get element at index (error if out of bounds) |
| `A.getOpt(arr, i)` | `(Array[a], int) -> Option[a]` | O(1) | Safe get: Some(elem) or None |
| `A.set(arr, i, v)` | `(Array[a], int, a) -> Array[a]` | **O(n)** | Return NEW array (copy-on-write) |
| `A.length(arr)` | `Array[a] -> int` | O(1) | Get array length |
| `A.fromList(xs)` | `[a] -> Array[a]` | O(n) | Convert list to array |
| `A.toList(arr)` | `Array[a] -> [a]` | O(n) | Convert array to list |

**Cost Model:**
- `get`, `getOpt`, `length` are O(1) - direct access
- `set` is **O(n)** because arrays are immutable (creates a full copy)
- `fromList`/`toList` are O(n) - avoid in hot loops

**Safe Access with getOpt:**
```ailang
import std/array as A
import std/option (Option, Some, None)

let arr = A.fromList([10, 20, 30])
let safe = A.getOpt(arr, 0)    -- Some(10)
let oob = A.getOpt(arr, 99)    -- None (out of bounds)
let neg = A.getOpt(arr, -1)    -- None (negative index)

match A.getOpt(arr, idx) {
  Some(val) => val,
  None => defaultValue
}
```
...
pure func idx(x: int, y: int) -> int { y * WIDTH + x }

pure func getCell(grid: Array[int], x: int, y: int) -> int {
  A.get(grid, idx(x, y))
}

pure func setCell(grid: Array[int], x: int, y: int, val: int) -> Array[int] {
  A.set(grid, idx(x, y), val)
}

export func main() -> () ! {IO} =
  let grid = A.make(WIDTH * HEIGHT, 0) in
  let grid2 = setCell(grid, 2, 1, 42) in
  print("Cell (2,1): " ++ show(getCell(grid2, 2, 1)))
```

**Key points:**
- **0-based indexing**: First element at index 0
- Arrays are **immutable** - `set` returns a NEW array (O(n) copy)
- Use `getOpt` for safe access returning `Option[a]`
- Use for O(1) **read** access when index is known
- Use Lists for sequential processing (pattern matching, recursion)
- **Performance tip**: For many updates, build a list and use `fromList` once
...
**Functions requiring `! {AI}`:**
- `call(...)` from `std/ai`
- Any function that calls these

**Multiple effects**: Combine with commas: `! {IO, FS}`, `! {Net, IO}`, `! {IO, AI}`, etc.
...
### Debug Effect (v0.4.10+) - Structured Runtime Tracing

**Debug is a "ghost effect"** - it's erasable in release mode for zero-cost tracing:
...
-- Debug.log: Trace execution (write-only, host collects)
-- Debug.check: Record assertions (collected, not thrown)
...
export func step(world: World, input: Input) -> (World, Output) ! {RNG, Debug} {
    Debug.log("Starting tick " ++ show(world.tick));
    -- ... game logic
}
```

**Key features:**
- **Write-only from AILANG**: `Debug.log` and `Debug.check` write to context, but AILANG code cannot read it
- **Host collects**: Only the Go host calls `DebugContext.Collect()` after step returns
- **Assertions don't throw**: `Debug.check(false, msg)` records failure but continues execution
- **Release mode erases**: With `--release`, Debug becomes zero-cost no-ops

**Why "check" not "assert"?** `assert` is a reserved keyword in AILANG.

**Import pattern:**
```ailang
import std/debug as Debug  -- Module alias for qualified access
-- Then use: Debug.log("message"), Debug.check(cond, "message")
```

### AI Effect (v0.5.0+) - Pluggable AI Oracle

**AI effect** provides a general-purpose AI oracle for calling external AI/ML systems. Uses a simple string→string interface (JSON by convention).
...
-- Ask AI for NPC decision
export func decideAction(context: NPCContext) -> Action ! {AI} {
...
export func main() -> () ! {IO, AI} {
    print("Asking AI: What is the capital of France?");
    let response = call("What is the capital of France? Reply in one word.");
    print("AI says: " ++ response)
}
```

**Running with AI effect:**
```bash
# With stub handler (testing - returns default responses)
ailang run --caps IO,AI --ai-stub --entry main module.ail

# With Claude Haiku (requires ANTHROPIC_API_KEY)
ailang run --caps IO,AI --ai claude-haiku-4-5 --entry main module.ail

# With GPT-5 Mini (requires OPENAI_API_KEY)
ailang run --caps IO,AI --ai gpt5-mini --entry main module.ail

# With Gemini Flash (requires GOOGLE_API_KEY)
ailang run --caps IO,AI --ai gemini-2-5-flash --entry main module.ail
```

**Key features:**
- **String→string interface**: JSON by convention, not enforced
- **Multi-provider**: Anthropic, OpenAI, Google supported via `--ai MODEL`
- **Stub handler**: Use `--ai-stub` for deterministic testing
- **No silent fallback**: Nil handler returns error (explicit configuration required)
- **Pluggable**: Swap providers without code changes

**Import pattern:**
```ailang
import std/ai (call)  -- Import call function
-- Then use: call("your prompt here")
```
...
```ailang
-- Simple pure function with inline tests
...
## Cross-Function Dependencies (M-TESTING-DEPS v0.4.7)

**Inline tests work for functions that call other functions!**
...
export pure func gcd(a: int, b: int) -> int
  tests [
    ((12, 8), 4), ((15, 5), 5), ((7, 3), 1), ((100, 25), 25)
  ]
{ if b == 0 then a else gcd(b, a % b) }

-- LCM calls gcd - tests work because M-TESTING-DEPS includes dependencies!
...
export pure func isOdd(n: int) -> bool
  tests [(0, false), (1, true), (4, false), (7, true)]
{ if n == 0 then false else isEven(n - 1) }
```

**Key points:**
- Tests for `lcm` automatically include `gcd` in the test harness
- Mutual recursion (`isEven`/`isOdd`) works via SCC analysis
- Run tests: `ailang test file.ail`
...
-- Inline tests show expected behavior for different tree shapes
export pure func treeSum(t: Tree) -> int
  tests [
    (Leaf(5), 5),
    (Node(Leaf(1), 2, Leaf(3)), 6),
    (Node(Leaf(0), 10, Node(Leaf(1), 2, Leaf(3))), 16)
  ]
{
  match t {
    Leaf(v) => v,
...
```ailang
-- Pure functions can include inline tests for documentation!
...
**This is v0.5.0 - Focus on: AI Effect, Array Type, Debug Effect, JSON Decode, Effect System, State Threading**
