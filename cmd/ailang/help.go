...
func printHelp() {
	...
	fmt.Printf("    %s <chain-id>       View chain stages and details (--remote reads cloud)\n", cyan("chains view"))
	fmt.Printf("    %s <chain-id>       ASCII tree with chat history\n", cyan("chains tree"))
	fmt.Printf("    %s          Cost summary (add --by-mission for per-mission budget rollup)\n", cyan("chains stats"))
	fmt.Printf("    %s  Post a mission iteration chain (JSON stdin; fail-soft, dual-writes cloud when configured)\n", cyan("chains post-iteration"))
