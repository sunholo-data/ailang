...
| `prepush-gate.ts` | Blocks `git push` when gofmt, or the repo's own `make lint` / `make check-file-sizes` (each only if the Makefile defines it), fails. Operator escape hatch: `AILANG_SKIP_PREPUSH=1` in pi's launch env |
