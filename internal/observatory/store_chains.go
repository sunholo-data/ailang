...
func (s *Store) GetChain(ctx context.Context, id string, opts ChainReadOptions) (*ExecutionChain, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		...
		FROM execution_chains WHERE id = ?
	`, id).Scan(
		&chain.ID, &chain.SourceType, &chain.SourceRef, &chain.GitHubRepo, &chain.GitHubIssueNumber,
...
func (s *Store) CreateStage(ctx context.Context, req *StageCreateRequest) (*ChainStage, error) {
	...
	var maxStage int
	err := s.db.QueryRowContext(ctx, `
		SELECT COALESCE(MAX(stage_number), 0) FROM chain_stages WHERE chain_id = ?
	`, req.ChainID).Scan(&maxStage)
	if err != nil {
		return nil, fmt.Errorf("failed to get max stage number: %w", err)
	}
	...
	`,
		stage.ID, stage.ChainID, stage.StageNumber, stage.AgentID, stage.Provider,
