...
func stubClaude(t *testing.T, out string, err error) {
	...
	claudeReviewCall = func(_ context.Context, model, sys, user string) ([]byte, error) {
		if model != ClaudeReviewerModel || sys != systemPrompt || !strings.Contains(user, `"verdict"`) {
...
func TestRunClaudeSubscriptionReviewer_ParsesVerdictAndTokens(t *testing.T) {
	...
	o := RunClaudeSubscriptionReviewer(ClaudeReviewerModel, "doc.md", "body")
...
func TestClaudeReviewCmd_SubscriptionOnlyAndToolless(t *testing.T) {
	...
	cmd := claudeReviewCmd(context.Background(), ClaudeReviewerModel, "sys", "user")
	...
	for _, want := range []string{"-p", "--tools\x00\x00", "--setting-sources\x00\x00", "--strict-mcp-config", "--model\x00" + ClaudeReviewerModel} {
