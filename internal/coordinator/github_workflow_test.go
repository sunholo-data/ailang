...
func TestGitHubWorkflow_DesignDocStage(t *testing.T) {
	...
	if err := store.MarkTaskPendingApproval(ctx, task.ID, "/tmp/wt-design-1", "branch-design-1", "main", "", result); err != nil {
...
func TestGitHubWorkflow_SprintPlanning(t *testing.T) {
	...
	if err := store.MarkTaskPendingApproval(ctx, task.ID, "/tmp/wt-sprint-1", "branch-sprint-1", "main", "", result); err != nil {
		t.Fatalf("failed to mark pending approval: %v", err)
	}
...
func TestGitHubWorkflow_Implementation(t *testing.T) {
	...
	if err := store.MarkTaskPendingApproval(ctx, task.ID, "/tmp/wt-impl-1", "branch-impl-1", "main", "", result); err != nil {
		t.Fatalf("failed to mark pending approval: %v", err)
	}
...
func TestGitHubWorkflow_RejectionAndRevision(t *testing.T) {
	...
	if err := store.MarkTaskPendingApproval(ctx, task.ID, "/tmp/wt-rev-1", "branch-rev-1", "main", "", result); err != nil {
		t.Fatalf("failed to mark pending approval: %v", err)
	}
	...
	if err := store.MarkTaskPendingApproval(ctx, newTask.ID, "/tmp/wt-rev-2", "branch-rev-2", "main", "", result); err != nil {
...
func TestGitHubWorkflow_ConcurrentApprovals(t *testing.T) {
	...
	for i := 0; i < numTasks; i++ {
		...
		go func(idx int) {
			...
			if err := store.MarkTaskPendingApproval(ctx, task.ID, "/tmp/wt", "branch-test", "main", "", result); err != nil {
				errors <- err
				return
			}
