...
**Common Mistakes** (`prompts/v0.16.6.md`) + **`docs/LIMITATIONS.md`**, maps each to its
...
| strict `take` after `flatMap` materialises all outputs (#617) | `take(n, flatMap(f, xs))` | none — file checks clean | Phase 3 note `LIST_TAKE_AFTER_FLATMAP`: explain strict materialisation and suggest `takeFlatMap(n, f, xs)` | `footgun_fixtures_test.go:TestTakeAfterFlatMapWarningFixtures` (module pipeline; four warning fixtures) | `prompts/v0.16.6.md`: Strict take-after-flatMap/map Common-Mistakes block | shipped-this-sprint — blind spots: value-level aliases (V10); allocating-`f` `take(n, map(f, xs))` (V25/V26), where `takeMap` is the fix this note cannot deliver; **nested composition — only the OUTERMOST trap fires when one trap is an argument to another, e.g. `take(2, flatMap(g, take(3, flatMap(h, xs))))` reports 1 of 2 (measured V1 iteration 186 via `ailang check`; ANF wraps the inner trap in an extra `Let` the detector does not unwrap) — see #680** |

**Count:** 22 rows (≥ 10 required).
