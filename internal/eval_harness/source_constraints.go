...
type SourceConstraints struct {
	...
	MinBytes         int      `yaml:"min_bytes,omitempty"`         // normalized source must be at least this many bytes (0 = unchecked)
	MaxCharRun       int      `yaml:"max_char_run,omitempty"`      // no character may repeat more than this many times consecutively (0 = unchecked)
...
func (sc *SourceConstraints) Check(code string) []string {
	...
	if sc.MinBytes > 0 && len(src) < sc.MinBytes {
		violations = append(violations, fmt.Sprintf(
			"source must be at least %d bytes after normalization; yours is %d bytes (%d short)",
			sc.MinBytes, len(src), sc.MinBytes-len(src)))
	}
	if sc.MaxCharRun > 0 {
		runLen := 0
		var prev rune = -1
		for i, ch := range src {
			...
			if runLen == sc.MaxCharRun+1 {
				line := 1 + strings.Count(src[:i], "\n")
				violations = append(violations, fmt.Sprintf(
					"no character may appear more than %d times in a row; %q repeats %d+ times on line %d",
					sc.MaxCharRun, ch, sc.MaxCharRun+1, line))
				break
			}
		}
	}
...
func (sc *SourceConstraints) Validate() error {
	if sc.ExactBytes < 0 || sc.MaxBytes < 0 || sc.MinBytes < 0 || sc.MaxCharRun < 0 {
		return fmt.Errorf("source_constraints: counts must be non-negative")
	}
	if sc.ExactBytes > 0 && (sc.MaxBytes > 0 || sc.MinBytes > 0) {
		return fmt.Errorf("source_constraints: exact_bytes is mutually exclusive with max_bytes/min_bytes")
	}
	if sc.MinBytes > 0 && sc.MaxBytes > 0 && sc.MinBytes > sc.MaxBytes {
		return fmt.Errorf("source_constraints: min_bytes exceeds max_bytes")
	}
	if sc.ExactBytes == 0 && sc.MaxBytes == 0 && sc.MinBytes == 0 && sc.MaxCharRun == 0 && sc.BannedChars == "" && len(sc.BannedSubstrings) == 0 {
