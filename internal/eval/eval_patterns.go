...
func (e *CoreEvaluator) evalDictRef(ref *core.DictRef) (Value, error) {
	...
	for _, method := range methodNames {
		...
		if !ok {
			...
			if ref.ClassName == "Eq" && ref.TypeName == types.StructuralEqTypeName {
				methods[method] = &BuiltinFunction{
					Name: "derived_structural_" + method,
					Fn:   makeStructuralEqualityFn(method == "eq"),
				}
				continue
			}
			if ref.ClassName == "Eq" {
		...
		// M-EQ-DERIVE-CONTAINERS: synthesized Eq compares any two values structurally
		if _, ok := entry.Impl.(*types.DerivedStructuralEquality); ok {
			methods[method] = &BuiltinFunction{
				Name: "derived_structural_" + method,
				Fn:   makeStructuralEqualityFn(method == "eq"),
			}
